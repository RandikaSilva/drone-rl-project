"""Crazyflie Waypoint Navigation - SAC Training, Play & Evaluation.
...
Usage (from IsaacLab directory):
    ...
    # Train:
    python ~/Desktop/Lasitha/drone_rl_project/waypoint/waypoint_sac.py \
        --mode train --num_envs 512 --total_timesteps 500000 --headless

    # Play trained policy:
    python ~/Desktop/Lasitha/drone_rl_project/waypoint/waypoint_sac.py \
        --mode play --checkpoint /path/to/sac_final.zip --num_envs 1

    # Evaluate:
    python ~/Desktop/Lasitha/drone_rl_project/waypoint/waypoint_sac.py \
        --mode eval --checkpoint /path/to/sac_final.zip --num_episodes 50 --headless
"""
...
parser = argparse.ArgumentParser(description="Crazyflie Waypoint Nav - SAC")
...
gym.register(
    id="Isaac-CrazyflieWaypoint-SAC-v0",
...
def train():
    ...
    log_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logs", "sac", "crazyflie_waypoint_sac")
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] SAC policy loaded! Drone is flying.")
    ...
    try:
        while simulation_app.is_running():
            ...
            if step_count % 50 == 0:
                ...
                print(f"  Step {step_count} | Phase: {phase_names[phase]} | Pos: ({pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.2f}) | Reward: {rewards[0].item():.3f}")
