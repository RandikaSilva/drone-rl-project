"""UNSEEN ENV #5 — "NZ Fern Gully (under-canopy)" — play/eval ONLY for v10 checkpoint.

Purpose: test whether the v10 policy generalises to UNDER-CANOPY flight
beneath a dense NZ bush understory — the most ecologically-relevant scenario
for DOC conservation UAV work in native forest. Unlike v10 training
(35 mixed trees + 8 bushes), this env has 43 non-flyable vertical obstacles
(canopy + sub-canopy + ponga/tree-ferns) plus 27 flyable ground-level fern
clumps, simulating the cluttered understory beneath a podocarp/broadleaf
canopy.

NZ relevance
------------
  * NZ native bush understory is dominated by ponga / silver fern (Cyathea
    dealbata) and mamaku tree ferns — thin vertical trunks 2.5-5m tall that
    form exactly the kind of collision hazard a hovering drone must avoid
    at 1.5m cruise altitude.
  * Under-canopy drone flight is an active research topic precisely because
    GPS is blocked and the ground-level fern cover dominates LiDAR returns:
      - Kuronen et al. (2025) "Towards autonomous photogrammetric forest
        inventory using a lightweight under-canopy robotic drone"
        (arXiv:2501.12073)
      - ISPRS (2025) "Under-canopy UAV Solutions for Forest Inventory –
        Challenges and Opportunities"
      - Zhou et al. (2024) "Forest in situ observations through a fully
        automated under-canopy unmanned aerial vehicle"
  * DOC deploys UAVs for native-bush pest monitoring (stoats, possums,
    rats) directly beneath podocarp-broadleaf canopy — this is the target
    operational biome.

Environment details
-------------------
  Name             : NZ Fern Gully (under-canopy podocarp-broadleaf bush)
  Style            : Dense understory — mixed canopy/sub-canopy/ponga + ferns
  Total obstacles  : 70
    Canopy emergents (8): rimu/rata/kahikatea-scale, r=0.55-0.75, h=7-8m
    Sub-canopy saplings(15): thin stems, r=0.28-0.35, h=4.0-5.0m
    Ponga tree-ferns (15): very thin trunks, r=0.18-0.22, h=2.5-3.5m
                           (at cruise 1.5m the drone still clips the trunk)
    Fern groundcover (28): flyable bushes, r=0.50-0.80, h=0.7-1.2m
    Corner anchors (4)  : arena boundary markers
  Arena            : x in [-11, 11], y in [-11, 11]
...
Usage (from IsaacLab directory):
    ...
    # Play (visual, 1 env):
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/v10_unseen5_labyrinth_walls.py \
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    # ── Obstacles (UNSEEN #5 "NZ Fern Gully — under-canopy": 70 obstacles) ───
    # Simulates the dense UNDERSTORY beneath a NZ podocarp/broadleaf canopy —
    # ponga (tree fern) trunks, sub-canopy saplings, emergent stems above,
    # and thick ground-level fern cover. Under-canopy UAV research targets
    # precisely this biome (Kuronen et al. 2025, arXiv:2501.12073; ISPRS 2025
    # Under-canopy UAV Solutions for Forest Inventory).
    # Format: (x, y, radius, height). Bushes: height <= 1.5m (flyable-over at cruise_altitude=1.5m)
    obstacle_positions: list = [
        # ── Canopy emergents (8) — tall rimu/rata/kahikatea-scale trunks ──
        (-8.0, -6.0, 0.70, 7.5),
        (-5.5,  3.5, 0.65, 7.0),
        (-2.0,  7.0, 0.55, 7.5),
        ( 1.5, -4.5, 0.60, 7.0),
        ...
        ( 8.5,  5.5, 0.70, 7.0),
        (-4.0, -8.0, 0.60, 7.5),
        # ── Sub-canopy saplings (15) — thin tall stems, collision hazards ──
        (-7.0, -1.5, 0.30, 4.5),
        (-6.5,  6.0, 0.28, 4.0),
        (-4.0,  1.5, 0.32, 5.0),
        (-2.5, -6.5, 0.30, 4.5),
        ...
        ( 2.5, -7.5, 0.28, 4.0),
        (-6.0, -4.0, 0.35, 5.0),
        # ── Ponga tree-fern trunks (15) — VERY thin stems, 2.5-3.5m tall ──
        # (At cruise alt 1.5m the drone clips the trunk — not flyable.)
        (-7.5,  4.5, 0.20, 3.0),
        (-5.0, -2.5, 0.22, 2.8),
        (-3.5,  3.0, 0.18, 3.5),
        (-1.5,  1.0, 0.20, 3.0),
        ...
        ( 6.5,  4.5, 0.22, 2.8),
        ( 8.0, -2.0, 0.20, 3.0),
        # ── Understory ferns / ground cover (28 bushes) — flyable at cruise ──
        (-8.0,  2.5, 0.70, 1.0),
        (-7.0,  0.5, 0.60, 0.9),
        (-6.0, -6.5, 0.80, 1.2),
        (-5.0,  5.5, 0.55, 0.8),
...
def play():
    ...
    print(f"\n{'='*70}")
    print(f"[ENV]  UNSEEN #5 — NZ Fern Gully (under-canopy podocarp-broadleaf bush)")
    print(f"[ENV]  Total obstacles : {len(env_cfg.obstacle_positions)}")
    _bushes = sum(1 for (_x, _y, _r, h) in env_cfg.obstacle_positions if h <= env_cfg.bush_height_threshold)
    print(f"[ENV]  Canopy emergents: 8   (rimu/rata/kahikatea-scale, r=0.55-0.75, h=7-8m)")
    print(f"[ENV]  Sub-canopy saplings: 15  (thin tall stems, r=0.28-0.35, h=4-5m)")
    print(f"[ENV]  Ponga tree-ferns : 15  (very thin trunks, r=0.18-0.22, h=2.5-3.5m)")
    print(f"[ENV]  Fern groundcover : {_bushes}  (flyable bushes, h<={env_cfg.bush_height_threshold}m)")
    print(f"[ENV]  Arena corners    : 4")
    print(f"[ENV]  Arena          : x in [-11, 11], y in [-11, 11]; start (-10, 0, 0.2)")
    if agent:
        print(f"[INFO] High-level: trained SAC policy (v10 checkpoint)")
    else:
...
def evaluate():
    ...
    print(f"[ENV]  UNSEEN #5 — NZ Fern Gully (under-canopy podocarp-broadleaf bush)")
    print(f"[ENV]  Total obstacles : {len(env_cfg.obstacle_positions)}")
    print(f"[ENV]  Canopy emergents: 8   (rimu/rata-scale, r=0.55-0.75, h=7-8m)")
    print(f"[ENV]  Sub-canopy saplings: 15  (thin stems, r=0.28-0.35, h=4-5m)")
    print(f"[ENV]  Ponga tree-ferns : 15  (very thin trunks, r=0.18-0.22, h=2.5-3.5m)")
    print(f"[ENV]  Fern groundcover : {_bushes}  (flyable at cruise alt 1.5m)")
    print(f"[ENV]  Arena corners    : 4")
    print(f"[ENV]  Arena          : x in [-11, 11], y in [-11, 11]; start (-10, 0, 0.2)")
