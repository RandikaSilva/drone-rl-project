...
VERSION 7 — Targeted corridor crash fixes over v6 (82% success).

v6 baseline: 82% success, 14% crash (all NAVIGATE corridor collisions), 70.2% waypoint completion.
v6 crashes are from tight corridor passages where speed reduction alone isn't enough.

Key changes from v6 (MINIMAL — 4 targeted fixes):
    1. Lateral avoidance in safety layer — push velocity AWAY from nearest obstacle, not just slow down
    2. Wider reactive safety trigger (0.6m -> 0.9m) — more reaction time in corridors
    3. Corridor detection — when 2+ obstacles are close, reduce speed more aggressively
    4. Slightly wider safe_distance (2.7 -> 3.0m) — earlier penalty activation
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Obstacle Nav — SAC v7")
...
# v7: Enhanced corridor safety — lateral avoidance, corridor detection, wider safety trigger
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v7: Targeted corridor crash fixes over v6. Lateral avoidance + wider safety + corridor detection."""
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print("[INFO] v7: v6 + lateral avoidance + corridor detection + wider safety. Low-level: deterministic proportional controller (no checkpoint)")
    ...
    def _run_low_level_policy(self):
        ...
        # v7: Enhanced reactive safety — speed reduction + lateral avoidance + corridor detection
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v7",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v7", "crazyflie_hierarchical_obstacle_sac_v7")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v7", cfg=env_cfg)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v7: v6 + lateral avoidance + corridor detection (19 trees) + deterministic proportional LL controller")
    if agent:
