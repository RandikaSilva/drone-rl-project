"""Hierarchical Crazyflie obstacle avoidance + perching (trunk landing) with SAC — V3.

V3 goals (target >90% success):
    1. Complex environment with varied trunk sizes (large/medium/small + 8cm perch candidates)
    2. Multiple 8cm landing-candidate trees — agent must choose the CORRECT target
    3. Perching precision: tolerance 0.15m (tight) to land 20cm from 8cm trunk
    4. Optimizations from v10: stagnation penalty, command EMA smoothing, moderate DR (wind + vel scale)
    5. Target trunk added to observation (body-frame direction + normalized distance)

Environment:
    - 10 border trees (non-landable, radius 0.4-0.6)
    - 6 large interior trees (non-landable, radius 0.5-0.75)
    - 8 small perching trees (LANDABLE, radius 0.08 = 8cm, height 3.5m)
    Per episode:
      * TAKEOFF tree: random 8cm tree  (drone spawns 20cm from its surface)
      * LAND target: nearest 8cm tree to final waypoint (NOT same as takeoff)
      * All other trees remain collision obstacles — drone must avoid wrong 8cm candidates too

Two-level control architecture:
    HIGH-LEVEL (trained):  LiDAR 1347 rays (3 vert ch x 449 horiz) + state + target_trunk -> goal modifier
...
Usage (from IsaacLab directory):
    ...
    # Train high-level:
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/hierarchical_obstacle_trunk_sac_v3.py \
        --mode train --num_envs 256 --total_timesteps 15000000 --headless
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Perch/Trunk Nav — SAC v3")
parser.add_argument("--mode", type=str, default="train", choices=["train", "play", "eval"])
parser.add_argument("--num_envs", type=int, default=256)
parser.add_argument("--total_timesteps", type=int, default=15000000)
...
# v9: Tree trunk takeoff & landing — drone spawns near trunk, lands by approaching trunk
...
class LidarCNNExtractor(BaseFeaturesExtractor):
    ...
    Observation layout (v3): [lidar_1347, lin_vel_b(3), ang_vel_b(3), gravity_b(3),
                               goal_pos_b(3), phase_one_hot(5), nearest_obstacle(3),
                               target_trunk(3)]
    => 1347 lidar + 23 state = 1370
    ...
    def __init__(self, observation_space, features_dim=128, lidar_dim=None,
        ...
        # v3: state is 23 dims (20 from v2 + 3 target_trunk)
        if lidar_dim is None:
            lidar_dim = observation_space.shape[0] - 23
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v3: Multi-size trees + 8cm perching. Drone must select CORRECT 8cm trunk to land on."""

    episode_length_s = 55.0  # v3: longer — target selection + precision perching need more time
    ...
    # High-level spaces
    # obs v3: lidar_1347 + lin_vel_b(3) + ang_vel_b(3) + gravity_b(3) + goal_pos_b(3)
    #         + phase_one_hot(5) + nearest_obstacle(3) + target_trunk(3) = 1347 + 23 = 1370
    ...
    # v9: Enhanced reactive safety — stronger lateral push + wider trigger
    reactive_safety_distance: float = 1.2  # v9: wider trigger (was 0.9) — more reaction time
    reactive_safety_min_speed: float = 0.25  # v9: can slow more (was 0.3)
    reactive_safety_lateral_gain: float = 1.2  # v9: doubled lateral push (was 0.6) — stronger avoidance
    reactive_safety_corridor_threshold: int = 2  # v7: if N+ obstacles close, extra slowdown
    reactive_safety_corridor_speed: float = 0.15  # v9: slower in corridors (was 0.2)
    ...
    stabilize_duration: float = 3.0  # v9: longer stabilize (was 2.0) — more time to settle
    hover_position_tolerance: float = 0.5
    hover_duration: float = 2.0

    # ── v3: Tree Trunk Perching (8cm landing trunks) ────────────────────────
    trunk_hover_distance: float = 0.2     # 20cm from trunk surface
    trunk_start_height: float = 1.0       # height on trunk where drone starts
    trunk_hold_duration: float = 2.0      # seconds to hold on trunk before ascending
    trunk_approach_tolerance: float = 0.15  # v3: tighter (was 0.3) — 8cm trunk needs precision
    trunk_land_speed_threshold: float = 0.35  # v3: slower (was 0.5) — gentle perching
    landing_trunk_radius: float = 0.08    # v3: 8cm landing trunks
    # landing_tree_indices & interior_tree_indices set below after obstacle_positions
    wrong_trunk_penalty_scale: float = -15.0   # v3: penalty for loitering near non-target 8cm trees

    # v3: Trunk approach rewards (scaled up for precision)
    trunk_approach_progress_scale: float = 15.0   # v3: stronger (was 10) — faster approach
    trunk_xy_precision_scale: float = 300.0       # v3: tighter precision (was 200)
    trunk_altitude_hold_scale: float = -3.0
    trunk_stability_scale: float = 25.0           # v3: stronger (was 20) — reward low speed near trunk
    trunk_reached_bonus: float = 1500.0           # v3: bigger bonus for precise perch

    # ── Multi-Waypoint ───────────────────────────────────────────────────────
    static_start_pos: tuple = (-10.0, 0.0, 0.2)  # fallback only — v9 starts near trunk
    ...
    # ── Obstacles (v3: 32 trees — varied sizes + clusters + 8cm perch candidates) ───
    # Format: (x, y, radius, height).
    # Layout has tree clusters (close-together trees) and small-tree groves to add
    # navigation complexity. Each 8cm landing candidate also has nearby distractor trees.
    obstacle_positions: list = [
        # ── Border large trees (10) — perimeter, non-landable ────────────────
        (-6.0, 10.0, 0.55, 5.5),   # 0
        ( 0.0, 10.0, 0.60, 6.0),   # 1
        ( 6.0, 10.0, 0.50, 5.0),   # 2
        (-6.0, -10.0, 0.55, 5.5),  # 3
        ...
        ( 9.0, -7.0, 0.50, 5.0),   # 8
        ( 9.0,  7.0, 0.50, 5.0),   # 9
        # ── Center large tree + two close small neighbors (cluster) ──────────
        ( 0.0,  0.0, 0.70, 6.5),   # 10 center
        (-1.3,  0.6, 0.22, 3.8),   # 11 close small NW of center
        ( 1.3, -0.6, 0.22, 3.8),   # 12 close small SE of center
        # ── Cluster A — trio of medium trees grouped tightly (upper-left) ───
        (-5.0,  4.5, 0.45, 5.2),   # 13
        (-4.0,  5.2, 0.38, 4.6),   # 14 close to 13 (~1.2m)
        (-5.7,  3.3, 0.42, 5.0),   # 15 close to 13 (~1.5m)
        # ── Cluster B — pair of medium trees close (lower-right) ─────────────
        ( 5.0, -4.5, 0.50, 5.2),   # 16
        ( 4.0, -5.4, 0.40, 4.6),   # 17 close to 16 (~1.3m)
        # ── Small-tree grove (lower-left) — 3 small trees close together ────
        (-3.3, -3.6, 0.20, 3.4),   # 18
        (-2.6, -4.5, 0.18, 3.2),   # 19 close to 18 (~1.1m)
        (-4.0, -3.0, 0.22, 3.5),   # 20 close to 18 (~1.0m)
        # ── Mid-arena distractors (small) — sparse but block diagonals ──────
        (-7.2,  0.0, 0.32, 4.2),   # 21 left midfield
        ( 7.2,  0.0, 0.30, 4.2),   # 22 right midfield
        # ── 8cm landing/perching trees (8) — THIN, landable. Some have ──────
        # nearby small "distractor" trees so target selection matters.
        (-3.0,  6.5, 0.08, 3.8),   # 23 upper-left perch — distractor 24 nearby
        (-2.0,  7.2, 0.20, 3.4),   # 24 distractor near 23 (~1.2m)
        ( 3.0,  6.5, 0.08, 3.8),   # 25 upper-right perch — distractor 26 nearby
        ( 4.2,  7.0, 0.18, 3.3),   # 26 distractor near 25 (~1.3m)
        (-3.0, -6.8, 0.08, 3.5),   # 27 lower-left perch — distractor 28 nearby
        (-1.8, -7.4, 0.22, 3.5),   # 28 distractor near 27 (~1.3m)
        ( 3.0, -6.8, 0.08, 3.5),   # 29 lower-right perch — ISOLATED
        ( 6.5,  2.5, 0.08, 3.6),   # 30 right corridor perch — ISOLATED
        (-1.5,  2.5, 0.08, 3.4),   # 31 near-center-upper perch (close to center cluster — distractor)
        ( 1.5, -2.0, 0.08, 3.4),   # 32 near-center-lower perch (close to small grove — distractor)
        # ── v3.1: ISOLATED 8cm perching trees — preferred LAND targets (>2.5m clear) ──
        (-5.0,  1.5, 0.08, 3.5),   # 33 mid-left isolated perch
        (-5.5, -7.5, 0.08, 3.4),   # 34 lower-left isolated perch
        ( 2.5,  4.0, 0.08, 3.5),   # 35 upper-mid isolated perch
        # ── v3.2: NARROW GAP — two medium trees forming a 1.5m corridor ───────
        (-0.75, 5.5, 0.30, 4.5),   # 36 gap-left  (forces upper-mid threading)
        ( 0.75, 5.5, 0.30, 4.5),   # 37 gap-right
        # ── v3.2: MID-RIGHT GROVE — 3 close trees blocking direct E-W path ───
        ( 4.0, -2.5, 0.25, 3.8),   # 38 grove A
        ( 4.7, -1.8, 0.20, 3.5),   # 39 grove B (~1.0m from 38)
        ( 3.3, -3.0, 0.22, 3.7),   # 40 grove C (~0.9m from 38)
    ]
    # v3: all 8cm trunks (used for collision radius, canopy, and wrong-trunk-penalty)
    landing_tree_indices: list = [23, 25, 27, 29, 30, 31, 32, 33, 34, 35]
    # v3.1: ISOLATED 8cm trunks — the only valid TAKEOFF and LAND targets.
    # Clustered 8cm trees (23, 25, 27, 31, 32) act as distractors only — drone must
    # ignore them and find an isolated trunk for perching.
    isolated_landing_indices: list = [29, 30, 33, 34, 35]
    # legacy alias (kept for compatibility with v3 callsites)
    interior_tree_indices: list = [29, 30, 33, 34, 35]
    obstacle_collision_radius: float = 0.5
    obstacle_safe_distance: float = 2.5   # v3: slightly tighter to fit denser forest
    # v3: 8cm trunks have smaller collision radius so drone can approach closely
    landing_trunk_collision_radius: float = 0.15  # 8cm radius + ~7cm slack
    ...
    obstacle_proximity_penalty_scale: float = -30.0   # v9: reduced (was -50) — less overcautious
    ...
    crash_penalty: float = -1000.0                    # v9: increased from -900
    ...
    nav_lateral_penalty_scale: float = -2.0            # v9: stronger lateral penalty (was -1.0)
    nav_stability_scale: float = 2.0                   # reward low ang-vel + upright
    # v5: NEW — reward for maintaining clearance from obstacles
    obstacle_clearance_bonus_scale: float = 10.0      # v9: much stronger clearance reward (was 3.0)
    ...
    # ── v3: Stagnation penalty (from v10) — prevent permanent stalls ─────────
    stagnation_timeout: float = 6.0          # seconds without waypoint advance before penalty kicks in
    stagnation_penalty_scale: float = -8.0   # penalty grows linearly after timeout

    # ── v3: Command smoothing (from v10) — simulates motor/control lag ──────
    cmd_smoothing_alpha: float = 0.06   # EMA alpha for LL velocity command smoothing

    # ── v3: Domain Randomization (moderate, from v10) ───────────────────────
    dr_wind_enabled: bool = True
    dr_wind_max_speed: float = 0.5       # m/s per axis (conservative vs v10's 0.8)
    dr_wind_theta: float = 0.5           # OU mean-reversion rate
    dr_wind_sigma: float = 0.25          # OU walk intensity (m/s)

    dr_vel_scale_enabled: bool = True
    dr_vel_scale_range: tuple = (0.90, 1.10)  # per-env uniform sample (tighter than v10)

    dr_drag_enabled: bool = True
    dr_drag_range: tuple = (0.03, 0.10)       # per-env drag_tilt_coeff range

    dr_lidar_noise_enabled: bool = True
    dr_lidar_noise_std: float = 0.04          # std of noise on [0,1] normalized lidar

    dr_obs_noise_enabled: bool = True
    dr_obs_vel_noise_std: float = 0.04        # std added to lin_vel_b (m/s)
    dr_obs_angvel_noise_std: float = 0.025    # std added to ang_vel_b (rad/s)
    dr_obs_pos_noise_std: float = 0.03        # std added to goal/target body-frame xy (m)
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # v3: Tree trunk takeoff & landing buffers (target trunk = 8cm landing candidate)
        self._start_trunk_idx = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self._land_trunk_idx = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self._land_trunk_pos_w = torch.zeros(self.num_envs, 3, device=self.device)  # 20cm from target trunk surface
        self._prev_trunk_dist = torch.zeros(self.num_envs, device=self.device)
        ...
        # v3: Stagnation tracking (from v10) — episode timer + last waypoint-advance timestamp
        ...
        # v3: Command smoothing state (body-frame velocity cmds after EMA)
        ...
        self._dr_drag_coeff = torch.full(
            (self.num_envs,), self.cfg.drag_tilt_coeff, device=self.device,
        )

        # Phase state machine
        self._phase = torch.zeros(self.num_envs, dtype=torch.int32, device=self.device)
        self._takeoff_timer = torch.zeros(self.num_envs, device=self.device)
        ...
        # ── v3: Multi-size trees + 8cm perching with target selection ────────
        num_landing = len(self.cfg.landing_tree_indices)
        num_isolated = len(self.cfg.isolated_landing_indices)
        print(f"[INFO] v3: {self._num_obstacles} trees ({num_landing} 8cm trunks: "
              f"{num_isolated} isolated targets + {num_landing - num_isolated} clustered distractors)"
              f" + deterministic proportional LL controller")
        ...
        print(f"[INFO]   perching: landing_trunk_radius={self.cfg.landing_trunk_radius}m, tolerance={self.cfg.trunk_approach_tolerance}m")
        dr_features = []
        ...
        if self.cfg.dr_obs_noise_enabled: dr_features.append(
            f"obs_noise(vel={self.cfg.dr_obs_vel_noise_std},angvel={self.cfg.dr_obs_angvel_noise_std})"
        )
        print(f"[INFO]   DR: {', '.join(dr_features) if dr_features else 'disabled'}")

        # Episode logging
        self._episode_sums = {
            key: torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
            for key in [
                "trunk_hold_reward", "takeoff_ascent", "takeoff_alt_reward", "takeoff_drift",
                ...
                "trunk_approach_progress", "trunk_precision", "trunk_altitude_hold",
                "trunk_stability", "trunk_reached", "wrong_trunk_penalty", "stagnation",
    ...
    def _setup_scene(self):
        ...
        for i, (ox, oy, radius, height) in enumerate(self.cfg.obstacle_positions):
            ...
            # v3: 8cm landing trees get a fixed reasonable canopy (visual only — no
            # collision needed since hit_obstacle uses trunk radius).
            is_landing = i in self.cfg.landing_tree_indices
            if is_landing:
                canopy_radius = 0.45  # visible foliage on a thin trunk
                canopy_height = 1.0
            else:
                canopy_radius = radius * (1.3 + (i % 4) * 0.15)
                canopy_height = 1.2 + (i % 3) * 0.3
            canopy_cfg = sim_utils.CylinderCfg(
                radius=canopy_radius, height=canopy_height, axis="Z",
                collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=not is_landing),
                rigid_props=sim_utils.RigidBodyPropertiesCfg(kinematic_enabled=True, disable_gravity=True),
    ...
    def _run_low_level_policy(self):
        ...
        # v9 TAKEOFF: hold on trunk, then move away + ascend to cruise altitude
        takeoff = (phase == self.TAKEOFF)
        if takeoff.any():
            holding = takeoff & (self._takeoff_timer < self.cfg.trunk_hold_duration)
            departing = takeoff & (self._takeoff_timer >= self.cfg.trunk_hold_duration)
            # Phase 1: Hold position on trunk (zero velocity)
            if holding.any():
                vx_cmd[holding] = 0.0
                vy_cmd[holding] = 0.0
                vz_cmd[holding] = 0.0
                yaw_rate_cmd[holding] = 0.0
            # Phase 2: Depart — move away from trunk + ascend
            if departing.any():
                alt_err = self.cfg.cruise_altitude - self._robot.data.root_pos_w[departing, 2]
                vx_cmd[departing] = K_xy * goal_b[departing, 0] * 0.8  # v9: stronger departure (was 0.5)
                vy_cmd[departing] = K_xy * goal_b[departing, 1] * 0.8
                vz_cmd[departing] = K_z * alt_err
                yaw_rate_cmd[departing] = 0.0
        ...
        # v9 LAND: approach trunk position (lateral movement, maintain cruise altitude)
        land = (phase == self.LAND)
        if land.any():
            # goal_b points toward trunk landing position (set in phase transition)
            vx_cmd[land] = K_xy * goal_b[land, 0] * 0.5  # gentle approach
            vy_cmd[land] = K_xy * goal_b[land, 1] * 0.5
            # Hold cruise altitude — don't descend
            alt_err_land = self.cfg.cruise_altitude - self._robot.data.root_pos_w[land, 2]
            vz_cmd[land] = K_z * alt_err_land
            yaw_rate_cmd[land] = K_yaw * torch.atan2(goal_b[land, 1], goal_b[land, 0]) * 0.3
        ...
        # v7/v9: Enhanced reactive safety — active in NAVIGATE + TAKEOFF + STABILIZE (trunk proximity)
        nav_mask = (phase == self.NAVIGATE) | (phase == self.TAKEOFF) | (phase == self.STABILIZE)
        if nav_mask.any():
            drone_xy = self._robot.data.root_pos_w[:, :2].unsqueeze(1)  # (N, 1, 2)
            obs_delta = drone_xy - self._obstacle_pos_w  # (N, num_obs, 2) — drone minus obstacle
            obs_dists = torch.linalg.norm(obs_delta, dim=2)  # (N, num_obs)
            obs_surface = obs_dists - self._obstacle_radii.unsqueeze(0)  # (N, num_obs)
            min_surf, min_idx = obs_surface.min(dim=1)

            # Speed reduction (same as v6 but with wider trigger distance)
            safety_scale = (min_surf / self.cfg.reactive_safety_distance).clamp(
                self.cfg.reactive_safety_min_speed, 1.0
            )

            # v7: Corridor detection — count obstacles within safety distance
            num_close_obstacles = (obs_surface < self.cfg.reactive_safety_distance).sum(dim=1)
            in_corridor = num_close_obstacles >= self.cfg.reactive_safety_corridor_threshold
            # Extra speed reduction in corridors
            corridor_scale = torch.where(
                in_corridor,
                safety_scale.clamp(min=self.cfg.reactive_safety_corridor_speed),
                safety_scale,
            )

            apply_mask = nav_mask & (min_surf < self.cfg.reactive_safety_distance)
            if apply_mask.any():
                # Apply speed reduction
                self._ll_vx_cmd[apply_mask] *= corridor_scale[apply_mask]
                self._ll_vy_cmd[apply_mask] *= corridor_scale[apply_mask]

                # v7: Lateral avoidance — push velocity AWAY from nearest obstacle
                batch_idx = torch.arange(self.num_envs, device=self.device)
                nearest_delta = obs_delta[batch_idx, min_idx]  # (N, 2) drone-obstacle direction
                nearest_dir = nearest_delta / (torch.linalg.norm(nearest_delta, dim=1, keepdim=True) + 1e-6)
                # Avoidance strength: stronger when closer (inversely proportional)
                avoidance_strength = (1.0 - min_surf / self.cfg.reactive_safety_distance).clamp(0.0, 1.0)
                lateral_push_w = nearest_dir * avoidance_strength.unsqueeze(1) * self.cfg.reactive_safety_lateral_gain
                # Convert world-frame push to body-frame
                ...
                push_bx = cos_neg_yaw * lateral_push_w[:, 0] - sin_neg_yaw * lateral_push_w[:, 1]
                push_by = sin_neg_yaw * lateral_push_w[:, 0] + cos_neg_yaw * lateral_push_w[:, 1]
                self._ll_vx_cmd[apply_mask] += push_bx[apply_mask]
                self._ll_vy_cmd[apply_mask] += push_by[apply_mask]

                # Re-clamp after lateral push
                self._ll_vx_cmd[apply_mask] = self._ll_vx_cmd[apply_mask].clamp(
                    -self.cfg.max_velocity_xy, self.cfg.max_velocity_xy
                )
                self._ll_vy_cmd[apply_mask] = self._ll_vy_cmd[apply_mask].clamp(
                    -self.cfg.max_velocity_xy, self.cfg.max_velocity_xy
                )
    ...
    def _kinematic_update(self):
        ...
        # ── v3: Advance wind OU process (world-frame, 3D) ──
        if self.cfg.dr_wind_enabled:
            sqrt_dt = dt ** 0.5
            self._dr_wind += (-self.cfg.dr_wind_theta * self._dr_wind * dt
                              + self.cfg.dr_wind_sigma * sqrt_dt
                              * torch.randn_like(self._dr_wind))
            self._dr_wind.clamp_(-self.cfg.dr_wind_max_speed, self.cfg.dr_wind_max_speed)
            self._dr_wind[:, 2].mul_(0.3)  # weaker updrafts/downdrafts

        # ── v3: EMA command smoothing (simulates motor/control lag) ──
        a = self.cfg.cmd_smoothing_alpha
        self._smooth_vx = (1 - a) * self._smooth_vx + a * self._ll_vx_cmd
        self._smooth_vy = (1 - a) * self._smooth_vy + a * self._ll_vy_cmd
        self._smooth_vz = (1 - a) * self._smooth_vz + a * self._ll_vz_cmd
        self._smooth_yaw_rate = (1 - a) * self._smooth_yaw_rate + a * self._ll_yaw_rate_cmd

        # ── v3: Apply velocity-scaling DR (simulates mass/motor variation) ──
        ...
        noisy_vz = eff_vz + self._noise_vel[:, 2] * 0.5  # less vertical
        ...
        # ── v3: Add wind disturbance in world frame ──
        if self.cfg.dr_wind_enabled:
        ...
        # Update tracked yaw (with random walk noise) — use smoothed yaw rate
        ...
        # v3: per-env drag coefficient (DR)
        k_drag = self._dr_drag_coeff if self.cfg.dr_drag_enabled else self.cfg.drag_tilt_coeff
    ...
    def _get_observations(self) -> dict:
        ...
        # v3 DR: add Gaussian noise to lidar (post-normalization)
        ...
        # v3: target trunk relative position in body frame (which 8cm tree to land on)
        target_delta_w = self._land_trunk_pos_w - root_pos  # (N, 3)
        cos_neg_yaw_t, sin_neg_yaw_t = torch.cos(-yaw), torch.sin(-yaw)
        target_body_x = cos_neg_yaw_t * target_delta_w[:, 0] - sin_neg_yaw_t * target_delta_w[:, 1]
        target_body_y = sin_neg_yaw_t * target_delta_w[:, 0] + cos_neg_yaw_t * target_delta_w[:, 1]
        target_xy_dist = torch.linalg.norm(torch.stack([target_body_x, target_body_y], dim=1), dim=1)
        target_dir_norm = torch.stack(
            [target_body_x / target_xy_dist.clamp(min=1e-6),
             target_body_y / target_xy_dist.clamp(min=1e-6)], dim=1,
        )
        # Normalize distance by arena span (~20m max)
        target_dist_normalized = (target_xy_dist / 20.0).clamp(0.0, 1.0).unsqueeze(1)

        # v3 DR: state observation noise (sensor inaccuracy)
        lin_vel_b_obs = self._robot.data.root_lin_vel_b
        ang_vel_b_obs = self._robot.data.root_ang_vel_b
        goal_pos_b_obs = goal_pos_b
        target_dir_obs = target_dir_norm
        if self.cfg.dr_obs_noise_enabled:
            ...
            pos_n = self.cfg.dr_obs_pos_noise_std
            goal_pos_b_obs = goal_pos_b_obs + pos_n * torch.randn_like(goal_pos_b_obs)
            target_dir_obs = (target_dir_obs + pos_n * torch.randn_like(target_dir_obs)).clamp(-1.0, 1.0)

        # Layout v3: [lidar_1347, lin_vel_b(3), ang_vel_b(3), gravity_b(3), goal_pos_b(3),
        #             phase(5), nearest_obstacle(3), target_trunk(3)] = 1370
        obs = torch.cat([
            hl_lidar_norm,                          # 1347 (3ch × 449)
            lin_vel_b_obs,                           # 3
            ang_vel_b_obs,                           # 3
            self._robot.data.projected_gravity_b,    # 3 (no DR — gravity is reliable)
            goal_pos_b_obs,                          # 3
            phase_one_hot,                           # 5
            nearest_body_dir,                        # 2
            nearest_dist_normalized,                 # 1
            target_dir_obs,                          # 2 — v3: target 8cm trunk direction
            target_dist_normalized,                  # 1 — v3: normalized distance to target
        ], dim=-1)  # Total: 1370
        return {"policy": obs}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # v3: advance episode timer (used for stagnation penalty)
        ...
        # ── Phase 0: TAKEOFF (includes trunk hold + ascend) ─────────────────
        self._takeoff_timer[is_takeoff] += dt
        is_holding = is_takeoff & (self._takeoff_timer < self.cfg.trunk_hold_duration)
        is_departing = is_takeoff & (self._takeoff_timer >= self.cfg.trunk_hold_duration)
        alt_progress = altitude - self._prev_alt
        self._prev_alt = altitude.clone()
        # During hold: reward staying still at trunk
        trunk_hold_reward = torch.where(is_holding, torch.exp(-speed) * 2.0 * dt, torch.zeros_like(speed))
        # During depart: reward ascending
        takeoff_ascent = torch.where(is_departing, alt_progress * self.cfg.takeoff_ascent_scale, torch.zeros_like(alt_progress))
        alt_fraction = (altitude / self.cfg.cruise_altitude).clamp(0.0, 1.0)
        takeoff_alt_reward = torch.where(is_departing, alt_fraction * self.cfg.takeoff_altitude_reward_scale * dt, torch.zeros_like(altitude))
        ...
        lateral_scale = torch.where(
            ...
            torch.full_like(xy_dist, self.cfg.nav_lateral_penalty_scale * 0.5),  # v9: 50% for turns (was 30%)
        )
        ...
        # v3: update last-advance timestamp when waypoint is reached
        if just_advanced.any():
            self._last_wp_advance_time[just_advanced] = self._episode_time[just_advanced]

        # v3: Stagnation penalty — grows after timeout without advancing
        time_since_advance = (self._episode_time - self._last_wp_advance_time).clamp(min=0.0)
        stagnation_excess = (time_since_advance - self.cfg.stagnation_timeout).clamp(min=0.0)
        stagnation_penalty = torch.where(
            is_navigate, stagnation_excess * self.cfg.stagnation_penalty_scale * dt,
            torch.zeros_like(stagnation_excess),
        )
        ...
        # ── Phase 4: LAND (v9: trunk approach instead of ground descent) ─────
        # Keep old land rewards zeroed (for logging compatibility), add trunk rewards
        land_descent = torch.zeros_like(speed)
        land_xy_stability = torch.zeros_like(speed)
        land_drift = torch.zeros_like(speed)
        land_precision = torch.zeros_like(speed)
        land_descent_control = torch.zeros_like(speed)
        land_controlled_descent = torch.zeros_like(speed)
        land_altitude_penalty = torch.zeros_like(speed)

        # v9: Trunk approach rewards
        trunk_delta = self._land_trunk_pos_w - root_pos
        trunk_xy_dist = torch.linalg.norm(trunk_delta[:, :2], dim=1)
        trunk_dist_3d = torch.linalg.norm(trunk_delta, dim=1)

        # Progress toward trunk
        trunk_progress = self._prev_trunk_dist - trunk_xy_dist
        self._prev_trunk_dist = trunk_xy_dist.clone()
        trunk_approach_progress = torch.where(
            is_land, trunk_progress * self.cfg.trunk_approach_progress_scale,
            torch.zeros_like(trunk_progress)
        )

        # Precision near trunk (exponential reward for being close)
        trunk_precision = torch.where(
            is_land, torch.exp(-trunk_dist_3d * 5.0) * self.cfg.trunk_xy_precision_scale * dt,
            torch.zeros_like(trunk_dist_3d)
        )

        # Altitude hold at cruise (penalize altitude deviation)
        trunk_alt_error = torch.abs(root_pos[:, 2] - self.cfg.cruise_altitude)
        trunk_altitude_hold = torch.where(
            is_land, trunk_alt_error * self.cfg.trunk_altitude_hold_scale * dt,
            torch.zeros_like(trunk_alt_error)
        )

        # Stability near trunk (reward low speed when close)
        trunk_stability = torch.where(
            is_land & (trunk_xy_dist < 1.0),
            torch.exp(-speed) * self.cfg.trunk_stability_scale * dt,
            torch.zeros_like(speed)
        )

        # Trunk reached bonus
        trunk_reached = is_land & (trunk_dist_3d < self.cfg.trunk_approach_tolerance) & (speed < self.cfg.trunk_land_speed_threshold)
        trunk_reached_bonus = trunk_reached.float() * self.cfg.trunk_reached_bonus

        # v3: Wrong-trunk penalty — penalize being close to a non-target 8cm tree
        # (forces the agent to actively choose the correct perch target)
        landing_idx_tensor = torch.tensor(self.cfg.landing_tree_indices, device=self.device, dtype=torch.long)
        landing_obs_pos = self._obstacle_pos_w[:, landing_idx_tensor, :]   # (N, K, 2)
        drone_xy_wp = root_pos[:, :2].unsqueeze(1)                         # (N, 1, 2)
        land_dists = torch.linalg.norm(drone_xy_wp - landing_obs_pos, dim=2)  # (N, K)
        # Mask out the TARGET index so only non-target 8cm trees get penalized
        target_mask = landing_idx_tensor.unsqueeze(0) == self._land_trunk_idx.unsqueeze(1)  # (N, K)
        land_dists_masked = torch.where(target_mask, torch.full_like(land_dists, 999.0), land_dists)
        min_wrong_dist, _ = land_dists_masked.min(dim=1)
        wrong_close = (min_wrong_dist < 0.6).float()   # within 60cm of a wrong 8cm tree
        wrong_trunk_penalty = torch.where(
            is_land | is_hover,
            wrong_close * torch.exp(-min_wrong_dist) * self.cfg.wrong_trunk_penalty_scale * dt,
            torch.zeros_like(min_wrong_dist),
        )

        # Goal reached uses trunk position (final_goal_pos_w is already set to trunk pos)
        final_delta = self._final_goal_pos_w - root_pos
        final_dist_3d = torch.linalg.norm(final_delta, dim=1)
        goal_reached = trunk_reached  # v9: trunk arrival = goal reached
        goal_bonus = goal_reached.float() * self.cfg.goal_reached_bonus

        # Crash penalty: detect if drone hit obstacle (v9: exclude start/landing trunks)
        ...
        # v9: Mask out relevant trunks — don't crash-penalize near start/landing trunk
        batch_idx_crash = torch.arange(self.num_envs, device=self.device)
        crash_surface_dists_masked = crash_surface_dists.clone()
        crash_surface_dists_masked[batch_idx_crash[is_land], self._land_trunk_idx[is_land].long()] = 999.0
        is_near_start = is_takeoff | is_stabilize
        crash_surface_dists_masked[batch_idx_crash[is_near_start], self._start_trunk_idx[is_near_start].long()] = 999.0
        min_crash_dist, min_crash_idx = crash_surface_dists_masked.min(dim=1)
        ...
        # v9: LAND goal is trunk position (at cruise altitude), not ground
        self._goal_pos_w[to_land] = self._land_trunk_pos_w[to_land]
        if to_land.any():
            trunk_delta = self._land_trunk_pos_w[to_land] - root_pos[to_land]
            self._prev_trunk_dist[to_land] = torch.linalg.norm(trunk_delta[:, :2], dim=1)
            self._prev_z_dist[to_land] = torch.abs(trunk_delta[:, 2])
        ...
        reward = (
            ...
            + land_descent_control + land_controlled_descent + land_altitude_penalty
            + trunk_approach_progress + trunk_precision + trunk_altitude_hold
            + trunk_stability + trunk_reached_bonus + wrong_trunk_penalty
            + stagnation_penalty
            + goal_bonus + crash_penalty
            + time_penalty + ang_vel_penalty + yaw_penalty + upright_reward
        )

        # Logging
        self._episode_sums["trunk_hold_reward"] += trunk_hold_reward
        ...
        self._episode_sums["trunk_approach_progress"] += trunk_approach_progress
        self._episode_sums["trunk_precision"] += trunk_precision
        self._episode_sums["trunk_altitude_hold"] += trunk_altitude_hold
        self._episode_sums["trunk_stability"] += trunk_stability
        self._episode_sums["trunk_reached"] += trunk_reached_bonus
        self._episode_sums["wrong_trunk_penalty"] += wrong_trunk_penalty
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        too_low = (
            ...
            # v9: LAND is now airborne (trunk hover), so too_low still applies
        )
        too_high = self._robot.data.root_pos_w[:, 2] > self.cfg.max_flight_height
        flipped = self._robot.data.projected_gravity_b[:, 2] > 0.7

        # Obstacle collision (v3: per-obstacle collision radius — tighter for 8cm trunks)
        is_land = (self._phase == self.LAND)
        is_near_start_trunk = (self._phase == self.TAKEOFF) | (self._phase == self.STABILIZE)
        ...
        # Mask out relevant trunks to avoid false collisions
        obs_surface_masked = obs_surface_dists.clone()
        batch_idx = torch.arange(self.num_envs, device=self.device)
        obs_surface_masked[batch_idx[is_land], self._land_trunk_idx[is_land].long()] = 999.0
        obs_surface_masked[batch_idx[is_near_start_trunk], self._start_trunk_idx[is_near_start_trunk].long()] = 999.0
        min_obs_dist, min_obs_idx = obs_surface_masked.min(dim=1)
        nearest_height = self._obstacle_heights[min_obs_idx]
        # v3: per-obstacle collision radius — 8cm trunks use landing_trunk_collision_radius
        is_landing_trunk = self._obstacle_radii[min_obs_idx] <= self.cfg.landing_trunk_radius + 1e-3
        collision_radius = torch.where(
            is_landing_trunk,
            torch.full_like(min_obs_dist, self.cfg.landing_trunk_collision_radius),
            torch.full_like(min_obs_dist, self.cfg.obstacle_collision_radius),
        )
        hit_obstacle = (
            (min_obs_dist < collision_radius)
            & (self._robot.data.root_pos_w[:, 2] < nearest_height)
        )

        # v9: Goal reached = trunk position reached (not ground landing)
        trunk_delta = self._land_trunk_pos_w - self._robot.data.root_pos_w
        trunk_dist_3d = torch.linalg.norm(trunk_delta, dim=1)
        speed = torch.linalg.norm(self._robot.data.root_lin_vel_b, dim=1)
        goal_reached = (
            is_land
            & (trunk_dist_3d < self.cfg.trunk_approach_tolerance)
            & (speed < self.cfg.trunk_land_speed_threshold)
        )

        # v9: No ground-touch termination — drone stays airborne for trunk landing
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Update obstacle world positions (must be before trunk selection)
        env_origins_xy = self._terrain.env_origins[env_ids, :2]
        ...
        num_reset = len(env_ids)
        # v3.1: takeoff and landing use ISOLATED 8cm trees only
        # (clustered 8cm trees in landing_tree_indices remain distractors)
        isolated_idx = torch.tensor(self.cfg.isolated_landing_indices, device=self.device, dtype=torch.long)
        num_isolated = len(isolated_idx)

        # v3.1: Pick random ISOLATED 8cm tree for TAKEOFF start
        random_tree_sel = torch.randint(0, num_isolated, (num_reset,), device=self.device)
        start_tree_idx = isolated_idx[random_tree_sel]
        self._start_trunk_idx[env_ids] = start_tree_idx

        # Compute start position: 20cm from trunk surface, at trunk_start_height
        start_tree_xy = self._obstacle_pos_w[env_ids][
            torch.arange(num_reset, device=self.device), start_tree_idx
        ]  # (num_reset, 2)
        start_tree_radii = self._obstacle_radii[start_tree_idx]  # (num_reset,)
        # Random direction away from tree center
        start_angles = torch.rand(num_reset, device=self.device) * 2 * math.pi
        offset_dist = start_tree_radii + self.cfg.trunk_hover_distance
        start_pos = torch.zeros(num_reset, 3, device=self.device)
        start_pos[:, 0] = start_tree_xy[:, 0] + offset_dist * torch.cos(start_angles)
        start_pos[:, 1] = start_tree_xy[:, 1] + offset_dist * torch.sin(start_angles)
        start_pos[:, 2] = self.cfg.trunk_start_height
        self._start_pos_w[env_ids] = start_pos

        # v3.1: Pick LAND target — nearest ISOLATED 8cm tree to final waypoint, BUT different from takeoff
        final_wp_xy = self._final_goal_pos_w[env_ids, :2]  # (num_reset, 2)
        landing_obs_xy = self._obstacle_pos_w[env_ids][:, isolated_idx, :]  # (num_reset, num_isolated, 2)
        delta_to_trees = final_wp_xy.unsqueeze(1) - landing_obs_xy   # (num_reset, num_isolated, 2)
        dist_to_trees = torch.linalg.norm(delta_to_trees, dim=2)     # (num_reset, num_isolated)
        # Mask out the takeoff tree so the agent must navigate to a different isolated perch
        takeoff_mask = isolated_idx.unsqueeze(0) == start_tree_idx.unsqueeze(1)   # (num_reset, num_isolated)
        dist_to_trees_masked = torch.where(takeoff_mask, torch.full_like(dist_to_trees, 999.0), dist_to_trees)
        _, nearest_landing_sel = dist_to_trees_masked.min(dim=1)
        land_tree_idx = isolated_idx[nearest_landing_sel]
        self._land_trunk_idx[env_ids] = land_tree_idx

        # Compute landing position: 20cm from 8cm trunk surface toward the final waypoint
        land_tree_xy = self._obstacle_pos_w[env_ids][
            torch.arange(num_reset, device=self.device), land_tree_idx
        ]  # (num_reset, 2)
        land_tree_radii = self._obstacle_radii[land_tree_idx]  # (num_reset,) — should all be ~0.08
        land_dir = final_wp_xy - land_tree_xy  # direction from tree to waypoint
        land_dir_norm = torch.linalg.norm(land_dir, dim=1, keepdim=True).clamp(min=1e-6)
        land_dir = land_dir / land_dir_norm  # unit direction
        # Fallback if waypoint is exactly at the tree (rare): use random direction
        zero_dir = (land_dir_norm.squeeze(1) < 1e-3)
        if zero_dir.any():
            rand_ang = torch.rand(int(zero_dir.sum()), device=self.device) * 2 * math.pi
            land_dir[zero_dir, 0] = torch.cos(rand_ang)
            land_dir[zero_dir, 1] = torch.sin(rand_ang)
        land_offset = land_tree_radii.unsqueeze(1) + self.cfg.trunk_hover_distance
        self._land_trunk_pos_w[env_ids, 0] = land_tree_xy[:, 0] + land_dir[:, 0] * land_offset.squeeze(1)
        self._land_trunk_pos_w[env_ids, 1] = land_tree_xy[:, 1] + land_dir[:, 1] * land_offset.squeeze(1)
        self._land_trunk_pos_w[env_ids, 2] = self.cfg.cruise_altitude  # stay at cruise altitude

        # Override final_goal to trunk position (for goal_reached checks)
        self._final_goal_pos_w[env_ids] = self._land_trunk_pos_w[env_ids]
        ...
        self._prev_trunk_dist[env_ids] = torch.linalg.norm(
            start_pos[:, :2] - self._land_trunk_pos_w[env_ids, :2], dim=1
        )

        self._phase[env_ids] = self.TAKEOFF
        self._takeoff_timer[env_ids] = 0.0
        ...
        # v3: reset stagnation timers, command smoothing state, DR state
        ...
        if self.cfg.dr_vel_scale_enabled:
            ...
            self._dr_vel_scale[env_ids] = lo + (hi - lo) * torch.rand(
                len(env_ids), device=self.device,
            )
        if self.cfg.dr_drag_enabled:
            lo, hi = self.cfg.dr_drag_range
            self._dr_drag_coeff[env_ids] = lo + (hi - lo) * torch.rand(
                len(env_ids), device=self.device,
            )
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        if debug_vis:
            ...
            # v3.2: Red cube marker at the perching/landing point
            if not hasattr(self, "landing_pos_visualizer"):
                land_marker_cfg = CUBOID_MARKER_CFG.copy()
                land_marker_cfg.markers["cuboid"] = sim_utils.CuboidCfg(
                    size=(0.25, 0.25, 0.25),
                    visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(1.0, 0.0, 0.0)),
                )
                land_marker_cfg.prim_path = "/Visuals/Command/landing_position"
                self.landing_pos_visualizer = VisualizationMarkers(land_marker_cfg)
            ...
            self.landing_pos_visualizer.set_visibility(True)
        else:
            ...
            if hasattr(self, "landing_pos_visualizer"):
                self.landing_pos_visualizer.set_visibility(False)
    ...
    def _debug_vis_callback(self, event):
        ...
        # v3.2: Red cube at the perching/landing point (always visible)
        self.landing_pos_visualizer.visualize(self._land_trunk_pos_w)
...
gym.register(
    id="Isaac-HierarchicalObstacleTrunk-SAC-v3",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_trunk_sac_v3", "crazyflie_hierarchical_obstacle_trunk_sac_v3")
    ...
    env = gym.make("Isaac-HierarchicalObstacleTrunk-SAC-v3", cfg=env_cfg)
...
def play():
    ...
    env_cfg.min_num_waypoints = args_cli.min_waypoints
    env_cfg.max_num_waypoints = args_cli.max_waypoints
    # v3: disable DR for clean playback
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v3: multi-size trees + 8cm perching with target selection + deterministic LL controller")
    if agent:
...
def evaluate():
    ...
    env_cfg.min_num_waypoints = args_cli.min_waypoints
    env_cfg.max_num_waypoints = args_cli.max_waypoints
    # v3: disable DR for clean evaluation
    env_cfg.dr_wind_enabled = False
    env_cfg.dr_vel_scale_enabled = False
    env_cfg.dr_drag_enabled = False
    env_cfg.dr_lidar_noise_enabled = False
    ...
    # Track phase+cause combos for detailed crash breakdown
    crash_details = []  # list of (phase, cause) tuples
            ...
            if dones_np[i] and total_episodes < args_cli.num_episodes:
                ...
                if hasattr(env, 'unwrapped') and hasattr(env.unwrapped, 'extras'):
                    ...
                    if died_count > 0:
                        ...
                        # Record phase+cause combo for this crash
                        ep_phase = None
                        for pname in phase_crashes:
                            if extras.get(f"Episode_Termination/died_{pname}", 0) > 0:
                                ep_phase = pname
                                break
                        ep_cause = None
                        for cname in cause_crashes:
                            if extras.get(f"Episode_Termination/cause_{cname}", 0) > 0:
                                ep_cause = cname
                                break
