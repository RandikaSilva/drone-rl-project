"""UNSEEN ENV #4 — "NZ Kauri Grove (Waipoua-style)" — play/eval ONLY for v10 checkpoint.

Purpose: test whether the v10 policy generalises to UNSEEN TRUNK SIZES. v10
training max trunk radius is 0.75m; this env has 10 giant pillars at radius
0.90-1.10m — mimicking mature kauri (Agathis australis) stems, which in
Waipoua Forest reach 1.5-5m diameter (e.g. Tāne Mahuta, 4.4m diameter at
base). Surrounding medium trees mimic kauri associates (rimu, miro, taraire).

NZ relevance
------------
  * Kauri is NZ's largest native tree; mature stems commonly 1-3m diameter,
    giants reaching >4m (Te Matua Ngahere 5.2m, Tāne Mahuta 4.4m).
  * Waipoua Forest (Northland) is the largest remaining kauri sanctuary,
    actively monitored for kauri dieback (Phytophthora agathidicida) —
    DOC runs autonomous/semi-autonomous survey ops there.
  * Kauri-grove navigation is an underexplored UAV scenario: wide trunks mean
    few-but-large obstacles, very different LiDAR signature vs. plantation.
  * Relevant literature: DOC (n.d.) "Our work protecting kauri" programme;
    Meiforth et al. (2019) using UAVs for kauri dieback detection.

Environment details
-------------------
  Name             : NZ Kauri Grove (Waipoua-style)
  Style            : few-but-huge kauri stems + medium native associates + bushes
  Total obstacles  : 52
    Kauri giants (10): radius 0.90 - 1.10m (2-stem ≈ Tāne Mahuta half-scale),
                       height 7.0 - 7.5m  (UNSEEN SIZE vs v10 training)
    Associates (25)  : rimu/miro-scale, radius 0.35 - 0.55m, height 4.5 - 5.5m
    Undergrowth (17) : ferns / kauri-grass, flyable-over at cruise alt 1.5m
...
Usage (from IsaacLab directory):
    ...
    # Play (visual, 1 env):
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/v10_unseen4_giant_pillars.py \
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    # ── UNSEEN env #4: Giant Pillars — 52 obstacles ──────────────────────────
    # 10 giants (radius 0.90-1.10 — BIGGER than any v10 training obstacle)
    # + 25 medium filler + 17 bushes.
    obstacle_positions: list = [
        # ── Giants (10) — radius >= 0.90, height 7.0-7.5 ──
        ( -6.0,  5.0, 1.00, 7.5),
        ( -6.0, -5.0, 1.10, 7.5),
        ( -2.0,  2.5, 0.95, 7.0),
        ( -2.0, -2.5, 1.00, 7.0),
        ...
        (  0.0,  8.0, 0.95, 7.0),
        (  0.0, -8.0, 0.95, 7.0),
        # ── Medium filler trees (25) — radius 0.35-0.55 ──
        ( -8.0,  7.0, 0.48, 5.2), ( -8.0, -7.0, 0.42, 4.8), ( -8.0,  3.0, 0.50, 5.0), ( -8.0, -3.0, 0.40, 4.7),
        ( -4.0,  7.0, 0.45, 5.0), ( -4.0, -7.0, 0.47, 5.1), ( -4.0,  0.0, 0.53, 5.3),
        (  0.0,  0.0, 0.52, 5.2), (  0.0,  4.0, 0.43, 4.9), (  0.0, -4.0, 0.49, 5.0),
        (  4.0,  7.0, 0.44, 5.0), (  4.0, -7.0, 0.46, 5.1), (  4.0,  0.0, 0.51, 5.3),
        (  8.0,  7.0, 0.38, 4.8), (  8.0, -7.0, 0.55, 5.4), (  8.0,  5.0, 0.41, 4.9), (  8.0, -5.0, 0.47, 5.1),
        ( -6.0,  0.0, 0.54, 5.3), (  6.0,  0.0, 0.50, 5.2),
        (-10.0,  6.0, 0.45, 5.0), (-10.0, -6.0, 0.48, 5.1), ( 10.0,  6.0, 0.43, 4.9), ( 10.0, -6.0, 0.51, 5.2),
        (-10.0,  9.0, 0.50, 5.3), ( 10.0,  9.0, 0.50, 5.3),
        # ── Bushes (17) — short wide cylinders, flyable-over at cruise alt 1.5m ──
        ( -7.5,  2.0, 0.75, 1.0), ( -7.5, -2.0, 0.75, 1.0),
        ( -3.0,  4.5, 0.75, 1.0), ( -3.0, -4.5, 0.75, 1.0),
        (  3.0,  4.5, 0.75, 1.0), (  3.0, -4.5, 0.75, 1.0),
        (  7.5,  4.5, 0.75, 1.0), (  7.5, -4.5, 0.75, 1.0),
        (  5.0,  0.0, 0.75, 1.0), ( -5.0,  0.0, 0.75, 1.0),
        (  9.5,  0.0, 0.75, 1.0), ( -9.5,  0.0, 0.75, 1.0),
        ( -1.0,  6.5, 0.75, 1.0), (  1.0, -6.5, 0.75, 1.0),
        (  5.0,  8.5, 0.75, 1.0), ( -5.0, -8.5, 0.75, 1.0),
        (  0.0, -10.5, 0.75, 1.0),
    ]
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[UNSEEN ENV #4] NAME        : NZ Kauri Grove (Waipoua-style)")
    print(f"[UNSEEN ENV #4] STYLE       : 10 kauri giants (r=0.90-1.10, UNSEEN) + 25 native associates + 17 undergrowth")
    print(f"[UNSEEN ENV #4] OBSTACLES   : {len(env_cfg.obstacle_positions)}  (10 kauri + 25 rimu/miro-scale + 17 ferns)")
    print(f"[UNSEEN ENV #4] ARENA       : x in [-11,11], y in [-11,11]  |  START (-10, 0, 0.2)")
...
def evaluate():
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION -- UNSEEN ENV #4: NZ Kauri Grove (Waipoua-style)")
    print(f"  Style    : mature kauri stand — few-but-huge stems (r 0.90-1.10) + native associates")
    print(f"  Obstacles: {len(env_cfg.obstacle_positions)}  (10 kauri giants + 25 rimu/miro-scale + 17 ferns/undergrowth)")
