...
VERSION 8 — 96% success rate (48/50). Velocity-projection safety + predictive avoidance.

v7 baseline: 94% success, 2% crash, 4% timeout.
v8 result:   96% success, 2% crash, 2% timeout. (Best so far, needs retraining for 100%)

Key changes from v7:
    1. Velocity-projection safety — blocks approach velocity toward obstacles, allows tangential sliding
       (prevents crashes without causing timeouts like uniform speed reduction)
    2. Predictive avoidance — projects velocity forward to detect fast approaches early
    3. Push-away force proportional to closeness (gain=0.5)
    4. Episode time 40s→120s — extra buffer for slow cautious navigation
...
# v8: Velocity-projection safety + predictive avoidance for 96% success
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v8: 96% success. Velocity-projection safety + predictive avoidance."""
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print("[INFO] v8: velocity-projection safety + predictive avoidance. Low-level: deterministic proportional controller (no checkpoint)")
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v8", "crazyflie_hierarchical_obstacle_sac_v8")
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v8: velocity-projection safety + predictive avoidance (19 trees) + deterministic proportional LL controller")
    if agent:
