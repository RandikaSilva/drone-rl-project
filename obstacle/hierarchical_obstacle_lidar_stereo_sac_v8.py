...
VERSION 8 — ZED 2i stereo camera depth + LiDAR sensor fusion over v7 (94% success).

v7 baseline: 94% success, 2% crash (1/50 in NAVIGATE), 72.1% waypoint completion.
v7's remaining failures: tight corridor crashes where sparse LiDAR misses obstacles directly ahead,
and timeout failures where the drone can't find efficient paths.

Key changes from v7 (SENSOR ADDITION ONLY — requires retraining):
    1. ZED 2i depth camera (forward-facing, 110° HFOV, 48x64 depth image)
       — Dense forward obstacle detection complements sparse 360° LiDAR
    2. Dual-branch CNN feature extractor (1D CNN for LiDAR + 2D CNN for depth)
       — Fuses both modalities with state for richer spatial understanding
    3. Larger fusion network (features_dim=192) for dual-modality processing
    Everything else (rewards, safety layers, PID, config) is IDENTICAL to v7.

    NOTE: Requires retraining — observation space changes from 1367 to 4439.
    v7 checkpoint is INCOMPATIBLE due to different obs/feature extractor.

Two-level control architecture:
    HIGH-LEVEL (trained):  LiDAR 1347 rays + ZED 2i depth (48x64) + state -> goal modifier
    LOW-LEVEL  (deterministic): Goal -> proportional velocity commands (vx, vy, vz, yaw_rate)

ZED 2i Camera Specs (simulated):
    - Resolution: 48x64 (downsampled for RL efficiency, real: up to 2208x1242)
    - HFOV: 110° (matching real ZED 2i)
    - Depth range: 0.3m - 20.0m
    - Mount: Forward-facing on drone body, 0.15m ahead of center
    - Output: distance_to_camera (Euclidean depth)

Usage (from IsaacLab directory):
    cd ~/projects/isaac/IsaacLab
    source ~/projects/isaac/env_isaaclab/bin/activate

    # Train high-level (MUST train from scratch — new obs space):
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Obstacle Nav — LiDAR+Stereo SAC v8 (LiDAR + ZED 2i)")
...
from isaaclab.sensors import RayCaster, RayCasterCfg, TiledCamera, TiledCameraCfg
...
# v8: LiDAR + ZED 2i depth camera sensor fusion for improved obstacle avoidance


# ═══════════════════════════════════════════════════════════════════════════════
# Dual-Branch CNN Feature Extractor: LiDAR (1D) + Depth (2D) + State
# ═══════════════════════════════════════════════════════════════════════════════

class LidarDepthCNNExtractor(BaseFeaturesExtractor):
    """1D CNN for multi-channel LiDAR + 2D CNN for ZED depth + MLP for state, fused together.

    Observation layout: [lidar_1347, depth_3072, lin_vel_b(3), ang_vel_b(3), gravity_b(3),
                         goal_pos_b(3), phase_one_hot(5), nearest_obstacle(3)]

    LiDAR is reshaped to (batch, n_channels, rays_per_channel) for Conv1d.
    Depth is reshaped to (batch, 1, height, width) for Conv2d.
    """

    def __init__(self, observation_space, features_dim=192, lidar_dim=None,
                 depth_height=48, depth_width=64, cnn_out_dim=32,
                 depth_cnn_out_dim=32, lidar_channels=3):
        self._depth_height = depth_height
        self._depth_width = depth_width
        self._depth_dim = depth_height * depth_width

        if lidar_dim is None:
            lidar_dim = observation_space.shape[0] - 20 - self._depth_dim
        super().__init__(observation_space, features_dim)

        self.lidar_dim = lidar_dim
        self.state_dim = observation_space.shape[0] - lidar_dim - self._depth_dim
        ...
        # 2D CNN for ZED depth image: (batch, 1, 48, 64) -> (batch, depth_cnn_out_dim)
        self.depth_cnn = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=5, stride=2, padding=2),   # 48x64 -> 24x32
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=3, stride=2, padding=1),  # 24x32 -> 12x16
            nn.ReLU(),
            nn.Conv2d(32, depth_cnn_out_dim, kernel_size=3, stride=2, padding=1),  # 12x16 -> 6x8
            nn.ReLU(),
            nn.AdaptiveAvgPool2d(1),  # -> (depth_cnn_out_dim, 1, 1)
            nn.Flatten(),             # -> (depth_cnn_out_dim,)
        )
        ...
        # Fusion — combines LiDAR (360° sparse) + depth (110° dense) + state
        self._features_dim = features_dim
        self.fusion = nn.Sequential(
            nn.Linear(cnn_out_dim + depth_cnn_out_dim + 64, features_dim),
            nn.ReLU(),
        )

    def forward(self, observations):
        lidar_flat = observations[:, :self.lidar_dim]
        depth_flat = observations[:, self.lidar_dim:self.lidar_dim + self._depth_dim]
        state = observations[:, self.lidar_dim + self._depth_dim:]
        ...
        # Reshape: (batch, 3072) -> (batch, 1, 48, 64)
        depth = depth_flat.view(-1, 1, self._depth_height, self._depth_width)
        depth_features = self.depth_cnn(depth)

        state_features = self.state_mlp(state)

        return self.fusion(torch.cat([lidar_features, depth_features, state_features], dim=1))
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v8: LiDAR + ZED 2i depth camera sensor fusion. Requires retraining."""
    ...
    # High-level spaces
    # obs: lidar_1347 + depth_3072 + lin_vel_b(3) + ang_vel_b(3) + gravity_b(3) + goal_pos_b(3)
    #      + phase_one_hot(5) + nearest_obstacle(3) = 1347 + 3072 + 20 = 4439
    # 3 vertical channels × 449 horiz rays = 1347 (horizontal_res=0.8 on 0-360°)
    # ZED 2i depth: 48 × 64 = 3072 pixels
    action_space = 3   # goal modifier: yaw_offset, speed_factor, altitude_offset
    observation_space = 4439
    state_space = 0
    debug_vis = True
    ...
    # ── ZED 2i Stereo Camera (forward-facing depth) ─────────────────────────
    # Simulates Stereolabs ZED 2i: 110° HFOV, depth 0.3-20m
    # Mounted on drone body, facing forward (+X direction)
    # Provides dense depth in the forward direction to complement sparse 360° LiDAR
    zed_depth_height: int = 48
    zed_depth_width: int = 64
    zed_max_depth: float = 20.0
    zed_min_depth: float = 0.3

    zed_camera = TiledCameraCfg(
        prim_path="/World/envs/env_.*/Robot/body/ZedCamera",
        update_period=0,
        height=48,
        width=64,
        data_types=["distance_to_camera"],
        spawn=sim_utils.PinholeCameraCfg(
            focal_length=2.12,        # mm — gives ~110° HFOV with aperture=6.0
            horizontal_aperture=6.0,  # mm — 2*2.12*tan(55°) ≈ 6.06
            clipping_range=(0.3, 20.0),
        ),
        offset=TiledCameraCfg.OffsetCfg(
            pos=(0.15, 0.0, 0.0),     # mounted 0.15m ahead of body center
            rot=(0.5, -0.5, 0.5, -0.5),  # rotate camera to face forward (+X)
            convention="ros",
        ),
    )

    # ── Goal Modifier (high-level adjusts direction/speed toward real goal) ──
    max_yaw_offset: float = 1.0    # radians (~57°)
    speed_factor_range: tuple = (0.0, 1.2)
    max_altitude_offset: float = 0.8
    goal_modifier_smoothing: float = 0.4

    # v8: Multi-obstacle velocity-projection safety — blocks approach velocity, allows tangential sliding
    ...
    obstacle_safe_distance: float = 2.7
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    """Hierarchical obstacle avoidance with LiDAR + ZED 2i depth fusion.
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Ornstein-Uhlenbeck random walk states
        ...
        # v8: ZED 2i depth buffer for rewards and safety
        self._min_depth_forward = torch.full(
            (self.num_envs,), self.cfg.zed_max_depth, device=self.device
        )

        # Debug draw
        self._draw = None

        print("[INFO] v8: LiDAR + ZED 2i depth camera sensor fusion. Low-level: deterministic proportional controller")
        print(f"[INFO]   LiDAR: {self.cfg.hl_lidar_n_channels}ch × 449 rays = 1347 dims (360°)")
        print(f"[INFO]   ZED 2i: {self.cfg.zed_depth_height}x{self.cfg.zed_depth_width} depth = {self.cfg.zed_depth_height * self.cfg.zed_depth_width} dims (110° forward)")
        print(f"[INFO]   Total obs: {self.cfg.observation_space} (lidar + depth + state)")
    ...
    def _setup_scene(self):
        ...
        # v8: ZED 2i depth camera (110° forward dense)
        self._zed_camera = TiledCamera(self.cfg.zed_camera)
        self.scene.sensors["zed_camera"] = self._zed_camera
    ...
    # ── ZED 2i Depth Processing ──────────────────────────────────────────────

    def _process_zed_depth(self) -> torch.Tensor:
        """Process ZED 2i depth image: extract, clamp, normalize.

        Returns:
            depth_norm: (N, H*W) normalized depth values [0, 1] where 0=close, 1=far
        """
        # TiledCamera returns (N, H, W, 1) for distance_to_camera
        depth_raw = self._zed_camera.data.output["distance_to_camera"]  # (N, H, W, 1)
        depth = depth_raw.squeeze(-1)  # (N, H, W)

        # Clamp to valid range and handle inf/nan
        depth = torch.where(
            torch.isfinite(depth),
            depth.clamp(self.cfg.zed_min_depth, self.cfg.zed_max_depth),
            torch.full_like(depth, self.cfg.zed_max_depth),
        )

        # Compute min depth in central FOV for monitoring
        H, W = depth.shape[1], depth.shape[2]
        frac = 0.5  # 50% central crop
        h_start = int(H * (1 - frac) / 2)
        h_end = int(H * (1 + frac) / 2)
        w_start = int(W * (1 - frac) / 2)
        w_end = int(W * (1 + frac) / 2)
        central_depth = depth[:, h_start:h_end, w_start:w_end]
        self._min_depth_forward = central_depth.reshape(self.num_envs, -1).min(dim=1).values

        # Normalize to [0, 1]: 0=min_depth (close), 1=max_depth (far)
        depth_norm = (depth - self.cfg.zed_min_depth) / (self.cfg.zed_max_depth - self.cfg.zed_min_depth)
        depth_norm = depth_norm.clamp(0.0, 1.0)

        # Flatten: (N, H, W) -> (N, H*W)
        return depth_norm.reshape(self.num_envs, -1)
    ...
    def _run_low_level_policy(self):
        """Deterministic proportional controller with LiDAR + depth safety layers."""
    ...
    def _kinematic_update(self):
        ...
        noisy_vz = self._ll_vz_cmd + self._noise_vel[:, 2] * 0.5
    ...
    def _get_observations(self) -> dict:
        """High-level obs: LiDAR (1347) + ZED depth (3072) + state (20) = 4439 dims."""
        root_pos = self._robot.data.root_pos_w
        root_quat = self._robot.data.root_quat_w
        ...
        # v8: ZED 2i depth image (48x64 = 3072 dims)
        depth_norm = self._process_zed_depth()

        # Phase one-hot
        phase_one_hot = torch.nn.functional.one_hot(self._phase.long(), 5).float()

        # Nearest obstacle in body frame (3 dims)
        ...
        # Layout: [lidar_1347, depth_3072, lin_vel_b, ang_vel_b, gravity_b, goal_pos_b, phase, nearest_obstacle]
        obs = torch.cat([
            hl_lidar_norm,                          # 1347 (3ch × 449)
            depth_norm,                             # 3072 (48 × 64 ZED depth)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Obstacle proximity penalty (LiDAR-based)
        drone_xy = root_pos[:, :2].unsqueeze(1)
        ...
        reward = (
            ...
            + obstacle_proximity + obstacle_clearance + lidar_penalty
            + subgoal_reachability + subgoal_magnitude
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._min_depth_forward[env_ids] = self.cfg.zed_max_depth
...
gym.register(
    id="Isaac-HierarchicalObstacle-LidarStereo-SAC-v8",
...
def setup_cameras(env_unwrapped):
    ...
    # v8: ZED 2i FPV camera — uses the sensor camera prim attached to the drone body
    # This shows what the ZED 2i sees in real-time (110° HFOV, forward-facing)
    # The camera is already spawned by TiledCameraCfg at env_0's robot body
    zed_cam_path = "/World/envs/env_0/Robot/body/ZedCamera"

    # Also create a higher-res render camera co-located with the ZED sensor
    # for a cleaner viewport (the TiledCamera prim renders at 48x64 internally)
    zed_render_path = "/World/Cameras/ZED2i_View"
    zed_render = UsdGeom.Camera.Define(stage, Sdf.Path(zed_render_path))
    zed_render.GetClippingRangeAttr().Set(Gf.Vec2f(0.3, 100.0))
    # Match ZED 2i optics: focal_length=2.12mm, horizontal_aperture=6.0mm → ~110° HFOV
    zed_render.GetFocalLengthAttr().Set(2.12)
    zed_render.GetHorizontalApertureAttr().Set(6.0)
    ...
    # ZED 2i drone FPV viewport — shows the forward-facing depth camera perspective
    vp_zed = create_viewport_window("ZED 2i FPV", width=640, height=400)
    vp_zed.viewport_api.camera_path = zed_render_path

    print("[INFO] 5 camera views created (Main=FREE, Follow, Top-Down, Side, ZED 2i FPV)")
    return {
        "follow": cam1_path, "topdown": cam2_path, "side": cam3_path,
        "zed_render": zed_render_path, "stage": stage,
    }


def update_cameras(cameras, drone_pos, drone_quat=None):
    from pxr import UsdGeom, Gf
    stage = cameras["stage"]
    x, y, z = float(drone_pos[0]), float(drone_pos[1]), float(drone_pos[2])

    # Update follow / topdown / side cameras (track drone position)
    ...
    # Update ZED 2i render camera — co-located with drone, facing forward
    if drone_quat is not None:
        qw = float(drone_quat[0])
        qx = float(drone_quat[1])
        qy = float(drone_quat[2])
        qz = float(drone_quat[3])
        yaw = math.atan2(2 * (qw * qz + qx * qy), 1 - 2 * (qy * qy + qz * qz))
    else:
        yaw = 0.0

    zed_cam = UsdGeom.Camera.Get(stage, cameras["zed_render"])
    zed_xform = UsdGeom.Xformable(zed_cam.GetPrim())
    zed_xform.ClearXformOpOrder()
    # Camera position: 0.15m ahead of drone center (matching sensor mount)
    cam_offset = 0.15
    eye_x = x + cam_offset * math.cos(yaw)
    eye_y = y + cam_offset * math.sin(yaw)
    eye = Gf.Vec3d(eye_x, eye_y, z)
    # Look forward along drone heading
    look_dist = 10.0
    target = Gf.Vec3d(
        eye_x + look_dist * math.cos(yaw),
        eye_y + look_dist * math.sin(yaw),
        z,
    )
    up = Gf.Vec3d(0, 0, 1)
    mat = Gf.Matrix4d()
    mat.SetLookAt(eye, target, up)
    zed_xform.AddTransformOp().Set(mat.GetInverse())
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_lidar_stereo_sac_v8", "crazyflie_hierarchical_obstacle_lidar_stereo_sac_v8")
    ...
    print(f"[INFO] High-level: 10 Hz, LiDAR ({env_cfg.hl_lidar_n_channels}ch × 449 = 1347 rays) + ZED 2i ({env_cfg.zed_depth_height}x{env_cfg.zed_depth_width} depth)")
    print(f"[INFO] Obs space: {env_cfg.observation_space} (lidar 1347 + depth {env_cfg.zed_depth_height * env_cfg.zed_depth_width} + state 20)")

    env = gym.make("Isaac-HierarchicalObstacle-LidarStereo-SAC-v8", cfg=env_cfg)
    ...
    if args_cli.checkpoint:
        ...
        agent = SAC(
            ...
            target_entropy="auto",
            policy_kwargs=dict(
                features_extractor_class=LidarDepthCNNExtractor,
                features_extractor_kwargs=dict(
                    cnn_out_dim=32,
                    depth_cnn_out_dim=32,
                    features_dim=192,  # v8: larger fusion for dual-modality (was 128)
                    lidar_channels=env_cfg.hl_lidar_n_channels,
                    depth_height=env_cfg.zed_depth_height,
                    depth_width=env_cfg.zed_depth_width,
                ),
                net_arch=[512, 256],
            ),
            verbose=1,
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v8: LiDAR + ZED 2i depth fusion (19 trees) + deterministic proportional LL controller")
    if agent:
        print(f"[INFO] High-level: trained SAC policy")
    else:
        print(f"[INFO] High-level: NONE (zero actions = neutral goal modifiers)")
    print(f"[INFO] Sensors: LiDAR (1347 rays, 360°) + ZED 2i ({env_cfg.zed_depth_height}x{env_cfg.zed_depth_width} depth, 110° forward)")
    ...
    try:
        while simulation_app.is_running():
            ...
            drone_quat = env.unwrapped._robot.data.root_quat_w[0].cpu().numpy()
            update_cameras(cameras, drone_pos, drone_quat)
            ...
            if step_count % 20 == 0:
                ...
                min_depth = unwrapped._min_depth_forward[0].item()
                phase_names = ["TAKEOFF", "STABILIZE", "NAVIGATE", "HOVER", "LAND"]
...
def evaluate():
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION: Hierarchical LiDAR+Stereo SAC v8 (LiDAR + ZED 2i) | {args_cli.num_episodes} episodes")
    print(f"HL Checkpoint: {os.path.basename(args_cli.checkpoint)}")
    print(f"Sensors: LiDAR (1347 rays, 360°) + ZED 2i ({env_cfg.zed_depth_height}x{env_cfg.zed_depth_width} depth, 110° forward)")
    ...
    try:
        ...
        print(f"\n{'='*60}", flush=True)
        print(f"RESULTS -- Hierarchical LiDAR+Stereo SAC v8 ({n} episodes)", flush=True)
