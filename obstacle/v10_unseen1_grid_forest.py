"""UNSEEN ENV #1 — "NZ Pine Plantation (Kaingaroa-style)" — play/eval ONLY for v10 checkpoint.

Purpose: test how well the v10 policy (trained on an IRREGULAR 35-obstacle
forest) generalises to a REGULARLY-PLANTED commercial pine plantation,
representative of NZ's Pinus radiata estate (~1.7M ha, largest plantation
species in NZ). Typical initial stocking is 600–1500 stems/ha at spacings
such as 3.16x3.16m (1000/ha) or 4x5m (500/ha) — this env uses 4m spacing
to approximate pre-thinning stocking scaled to the 22x22m arena
(~1,033 stems/ha, equivalent to NZFFA default plantation density).

NZ relevance
------------
  * Pinus radiata covers ~90% of NZ commercial forest (Kaingaroa, Kinleith).
  * Autonomous under-canopy drones are actively used for plantation inventory
    (e.g. Salmond et al. 2022 "Autonomous Surveying of Plantation Forests
    Using Multi-Rotor UAVs", MDPI Drones 6(9):256) — highly regular stem
    placement is the defining feature.
  * NZFFA stocking guide: 1000 stems/ha at 3.16x3.16m initial spacing.

Environment details
-------------------
  Name             : NZ Pine Plantation
  Style            : Regularly-planted 4m grid (pre-thinning density)
  Total obstacles  : 50
    Trees (24)     : 5x5 grid at x in {-8,-4,0,4,8}, y in {-8,-4,0,4,8}
                     (takeoff cell (-8, 0) skipped — leaves the start clear)
    Perimeter (10) : arena anchors at x=+/-10.5 and y=+/-10.5
                     (simulates plantation edge / shelter-belt)
    Bushes (16)    : midpoints between grid nodes — regenerating undergrowth
                     (flyable over at cruise alt 1.5m)
  Arena            : x in [-11, 11], y in [-11, 11]  (22m x 22m ≈ 0.048 ha)
  Effective stocking: ~1,033 stems/ha (scaled) — pre-thinning Pinus radiata
...
Usage (from IsaacLab directory):
    ...
    # Play (visual, 1 env):
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/v10_unseen1_grid_forest.py \
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    # ── UNSEEN env #1: Dense Grid Forest — 54 obstacles ──────────────────────
    # Format: (x, y, radius, height). Bushes: height <= 1.5m (flyable-over at cruise_altitude=1.5m)
    # 24 grid trees + 10 perimeter anchors + 20 bushes.
    obstacle_positions: list = [
        # ── Grid trees (24) — 5x5 grid at x,y in {-8,-4,0,4,8}, skip (-8,0) ──
        (-8.0, -8.0, 0.45, 5.0), (-8.0, -4.0, 0.45, 5.0), (-8.0,  4.0, 0.45, 5.0), (-8.0,  8.0, 0.45, 5.0),
        (-4.0, -8.0, 0.45, 5.0), (-4.0, -4.0, 0.45, 5.0), (-4.0,  0.0, 0.45, 5.0), (-4.0,  4.0, 0.45, 5.0), (-4.0,  8.0, 0.45, 5.0),
        ( 0.0, -8.0, 0.45, 5.0), ( 0.0, -4.0, 0.45, 5.0), ( 0.0,  0.0, 0.45, 5.0), ( 0.0,  4.0, 0.45, 5.0), ( 0.0,  8.0, 0.45, 5.0),
        ( 4.0, -8.0, 0.45, 5.0), ( 4.0, -4.0, 0.45, 5.0), ( 4.0,  0.0, 0.45, 5.0), ( 4.0,  4.0, 0.45, 5.0), ( 4.0,  8.0, 0.45, 5.0),
        ( 8.0, -8.0, 0.45, 5.0), ( 8.0, -4.0, 0.45, 5.0), ( 8.0,  0.0, 0.45, 5.0), ( 8.0,  4.0, 0.45, 5.0), ( 8.0,  8.0, 0.45, 5.0),
        # ── Perimeter anchors (10) ──
        (-10.5, -10.0, 0.55, 5.0), (-10.5,  10.0, 0.55, 5.0), ( 10.5, -10.0, 0.55, 5.0), ( 10.5,  10.0, 0.55, 5.0),
        (  0.0, -10.5, 0.50, 5.0), (  0.0,  10.5, 0.50, 5.0),
        (-10.5,   5.0, 0.45, 5.0), (-10.5,  -5.0, 0.45, 5.0), ( 10.5,   5.0, 0.45, 5.0), ( 10.5,  -5.0, 0.45, 5.0),
        # ── Bushes (16) — midpoints between grid nodes, flyable-over at cruise alt 1.5m ──
        (-6.0, -6.0, 0.70, 1.0), (-6.0, -2.0, 0.70, 1.0), (-6.0,  2.0, 0.70, 1.0), (-6.0,  6.0, 0.70, 1.0),
        (-2.0, -6.0, 0.70, 1.0), (-2.0, -2.0, 0.70, 1.0), (-2.0,  2.0, 0.70, 1.0), (-2.0,  6.0, 0.70, 1.0),
        ( 2.0, -6.0, 0.70, 1.0), ( 2.0, -2.0, 0.70, 1.0), ( 2.0,  2.0, 0.70, 1.0), ( 2.0,  6.0, 0.70, 1.0),
        ( 6.0, -6.0, 0.70, 1.0), ( 6.0, -2.0, 0.70, 1.0), ( 6.0,  2.0, 0.70, 1.0), ( 6.0,  6.0, 0.70, 1.0),
    ]
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[UNSEEN ENV #1] NAME        : NZ Pine Plantation (Pinus radiata, Kaingaroa-style)")
    print(f"[UNSEEN ENV #1] STYLE       : regular 4m-spaced grid (~1,033 stems/ha pre-thinning)")
    print(f"[UNSEEN ENV #1] OBSTACLES   : {len(env_cfg.obstacle_positions)}  (24 plantation trees + 10 shelter-belt + 16 regen bushes)")
    print(f"[UNSEEN ENV #1] ARENA       : x in [-11,11], y in [-11,11]  |  START (-10, 0, 0.2)")
...
def evaluate():
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION -- UNSEEN ENV #1: NZ Pine Plantation (Pinus radiata)")
    print(f"  Style    : regular 4m-spaced grid (~1,033 stems/ha pre-thinning)")
    print(f"  Obstacles: {len(env_cfg.obstacle_positions)}  (24 plantation trees + 10 shelter-belt + 16 regen bushes)")
