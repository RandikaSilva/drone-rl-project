...
Usage (from IsaacLab directory):
    ...
    # Train high-level:
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/hierarchical_obstacle_trunk_sac_v1.py \
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Obstacle Nav — SAC v8")
...
# v8: Tree trunk takeoff & landing — drone spawns near trunk, lands by approaching trunk
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    # v7: Enhanced reactive safety — speed reduction + lateral avoidance push
    reactive_safety_distance: float = 0.9  # v7: wider trigger for more reaction time (was 0.6)
    reactive_safety_min_speed: float = 0.3  # v7: can slow more (was 0.4)
    reactive_safety_lateral_gain: float = 0.6  # v7: NEW — lateral push strength away from obstacle
    reactive_safety_corridor_threshold: int = 2  # v7: NEW — if N+ obstacles close, extra slowdown
    reactive_safety_corridor_speed: float = 0.2  # v7: NEW — min speed when in corridor
    ...
    # v8: Trunk approach rewards (replace ground descent rewards)
    ...
    # ── Multi-Waypoint ───────────────────────────────────────────────────────
    static_start_pos: tuple = (-10.0, 0.0, 0.2)  # fallback only — v8 starts near trunk
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # v8: Tree trunk takeoff & landing buffers
        ...
        print("[INFO] v8: v7 + tree trunk takeoff & landing. Low-level: deterministic proportional controller (no checkpoint)")
    ...
    def _run_low_level_policy(self):
        ...
        # v8 TAKEOFF: hold on trunk, then move away + ascend to cruise altitude
        ...
        if takeoff.any():
            ...
            if departing.any():
                ...
                vx_cmd[departing] = K_xy * goal_b[departing, 0] * 0.5
                vy_cmd[departing] = K_xy * goal_b[departing, 1] * 0.5
        ...
        # v8 LAND: approach trunk position (lateral movement, maintain cruise altitude)
        ...
        # v7/v8: Enhanced reactive safety — active in NAVIGATE + TAKEOFF + STABILIZE (trunk proximity)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # ── Phase 4: LAND (v8: trunk approach instead of ground descent) ─────
        ...
        goal_reached = trunk_reached  # v8: trunk arrival = goal reached
        goal_bonus = goal_reached.float() * self.cfg.goal_reached_bonus

        # Crash penalty: detect if drone hit obstacle (v8: exclude start/landing trunks)
        ...
        # v8: Mask out relevant trunks — don't crash-penalize near start/landing trunk
        ...
        # v8: LAND goal is trunk position (at cruise altitude), not ground
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        too_low = (
            ...
            # v8: LAND is now airborne (trunk hover), so too_low still applies
        )
        too_high = self._robot.data.root_pos_w[:, 2] > self.cfg.max_flight_height
        flipped = self._robot.data.projected_gravity_b[:, 2] > 0.7

        # Obstacle collision (v8: exclude start trunk during TAKEOFF/STABILIZE, landing trunk during LAND)
        ...
        # v8: Goal reached = trunk position reached (not ground landing)
        ...
        # v8: No ground-touch termination — drone stays airborne for trunk landing
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # v8: Pick random interior tree for TAKEOFF start position
        ...
        # v8: Find nearest interior tree to final waypoint for LAND target
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v8",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_trunk_sac_v1", "crazyflie_hierarchical_obstacle_trunk_sac_v1")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v8", cfg=env_cfg)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v8: v7 + tree trunk takeoff & landing (19 trees) + deterministic proportional LL controller")
    if agent:
