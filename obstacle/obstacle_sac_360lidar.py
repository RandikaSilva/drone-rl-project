"""All-in-one Crazyflie obstacle avoidance + multi-waypoint navigation with SAC (Stable-Baselines3).
...
LiDAR: 360° horizontal ring at 1° resolution (359 beams, single horizontal channel).
       Combined with analytical ray-cylinder intersection for obstacle detection.

Modes:
    --mode train   Train SAC policy from scratch (or resume from checkpoint)
...
Usage (from IsaacLab directory):
    ...
    source ~/projects/isaac/env_isaaclab/bin/activate && python \
        ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/obstacle_sac_360lidar.py \
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Crazyflie Obstacle+MultiWP Nav — SAC (SB3)")
...
# Freespace-sim style debug draw (optional — may not be available in all Isaac Sim setups)
try:
...
class ObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    #    + lidar_360(359) + phase_one_hot(5) + nearest_obstacle_dir(2) + nearest_obstacle_dist(1) = 379
    # LiDAR: 1 horizontal channel, 1° resolution, 360° sweep → 359 unique beams
    action_space = 4
    observation_space = 379
    state_space = 0
    debug_vis = True
    ...
    # ── LiDAR Sensor — 360° horizontal ring at 1° resolution ────────────────
    # Single horizontal channel (elevation=0°), 359 unique beams around the drone.
    # Obstacles are NOT in the mesh so they are detected analytically via
    # _augment_lidar_with_obstacles() (ray-cylinder intersection).
    lidar = RayCasterCfg(
        ...
        pattern_cfg=patterns.LidarPatternCfg(
            channels=1,
            vertical_fov_range=(0.0, 0.0),   # pure horizontal ring
            horizontal_fov_range=(0.0, 360.0),
            horizontal_res=1.0,               # 1° per beam → 359 unique beams
        ),
        max_distance=20.0,
    ...
    static_start_pos: tuple = (-7.0, 0.0, 0.2)   # was -10.0 — moved away from wall trees at X=-9
    ...
    obstacle_safe_distance: float = 1.5    # was 2.5 — tighter zone so drone can navigate through gaps

    # ── Reward Scales ────────────────────────────────────────────────────────
    # Phase 0 — TAKEOFF
    takeoff_ascent_scale: float = 20.0
    takeoff_altitude_reward_scale: float = 5.0   # continuous reward for being higher
    ...
    # Phase 2 — NAVIGATE (obstacle-aware but still goal-driven)
    nav_xy_progress_scale: float = 8.0       # was 3.0 — must dominate obstacle penalties
    nav_velocity_align_scale: float = 12.0   # was 6.0 — strong pull toward waypoint
    nav_lateral_penalty_scale: float = -0.5  # was -1.0 — allow wider turns around obstacles
    nav_altitude_scale: float = -1.5         # was -2.0 — less punishing
    nav_stability_scale: float = 2.0         # was 3.0 — stability shouldn't discourage movement
    nav_max_speed: float = 2.0               # was 1.5 — allow faster flight
    nav_speed_penalty_scale: float = -2.0    # was -3.0 — softer speed cap
    # Multi-waypoint bonuses
    intermediate_waypoint_bonus: float = 150.0   # was 100 — bigger carrot for reaching waypoints
    speed_carry_bonus_scale: float = 8.0         # was 5.0 — reward momentum through waypoints
    waypoint_progress_scale: float = 3.0         # was 2.0 — more reward for fraction complete
    # Phase 3 — HOVER
    ...
    goal_reached_bonus: float = 1000.0    # was 750 — bigger incentive to complete the mission
    # Obstacle avoidance (NAVIGATE phase) — balanced penalties (must NOT dominate nav rewards)
    obstacle_proximity_penalty_scale: float = -5.0     # was -15.0 — gentle nudge, not a wall
    lidar_obstacle_penalty_scale: float = -5.0         # was -15.0 — same
    lidar_danger_distance: float = 2.5                 # was 5.0 — only penalize when actually close
    # All phases
...
class ObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: ObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Per-env world-frame obstacle XY positions: (num_envs, num_obstacles, 2)
        self._obstacle_pos_w = torch.zeros(
            self.num_envs, self._num_obstacles, 2, device=self.device
        )

        # Nearest obstacle tracking (for debug visualization)
        ...
        # LiDAR ray direction cache (lazy init on first obs call)
        self._lidar_ray_dirs = None
        self._lidar_horiz_indices = None
        # Min horizontal LiDAR distance (updated in _get_observations, used in reward)
    ...
    def _setup_scene(self):
        ...
        # LiDAR sensor — attached to robot body
        ...
        for i, (ox, oy, radius, height) in enumerate(self.cfg.obstacle_positions):
            # Trunk: brown cylinder, full height
            trunk_cfg = sim_utils.CylinderCfg(
                ...
                visual_material=sim_utils.PreviewSurfaceCfg(
                    diffuse_color=(0.36, 0.25, 0.13),  # brown trunk
                ),
            )
            trunk_cfg.func(
                f"/World/envs/env_0/Tree_{i}_Trunk",
                trunk_cfg,
                translation=(ox, oy, height / 2.0),
            )

            # Canopy: green cylinder, shorter and wider, sitting on top of trunk
            canopy_height = 1.5
            ...
            canopy_cfg = sim_utils.CylinderCfg(
                ...
                visual_material=sim_utils.PreviewSurfaceCfg(
                    diffuse_color=(0.15, 0.4, 0.12),  # green canopy
                ),
            )
            canopy_cfg.func(
                f"/World/envs/env_0/Tree_{i}_Canopy",
                canopy_cfg,
        ...
        # ── Natural lighting: Sun + Sky HDRI ─────────────────────────────────
        sun_cfg = sim_utils.DistantLightCfg(
            intensity=2500.0, color=(0.9, 0.85, 0.7), angle=0.53,
        )
    ...
    def _generate_waypoints(self, env_ids: torch.Tensor):
        ...
        X via sorted linspace+jitter, Y via alternating-sign offsets.
        Final waypoint Z = cruise_altitude. Unused slots padded with final WP copy.
        Waypoints too close to obstacles get nudged away.
        """
        ...
        # obstacle_local is (num_obstacles, 2) — local XY positions
        obs_xy = self._obstacle_local  # (num_obs, 2)
        obs_r = self._obstacle_radii   # (num_obs,)
        clearance = self.cfg.waypoint_obstacle_clearance

        for wi in range(max_wps):
            # (num_reset, 2) waypoint XY
            wp_xy = torch.stack([wp_x[:, wi], wp_y[:, wi]], dim=1)
            # Distance from each waypoint to each obstacle: (num_reset, num_obs)
            delta = wp_xy.unsqueeze(1) - obs_xy.unsqueeze(0)  # (num_reset, num_obs, 2)
            dists = torch.linalg.norm(delta, dim=2)  # (num_reset, num_obs)
            surface_dists = dists - obs_r.unsqueeze(0)

            # Find minimum distance per waypoint
            min_surf_dist, min_idx = surface_dists.min(dim=1)

            # Nudge waypoints that are too close
            too_close = min_surf_dist < clearance
            if too_close.any():
                batch_idx = torch.arange(num_reset, device=device)
                closest_delta = delta[batch_idx[too_close], min_idx[too_close]]  # (K, 2)
    ...
    def _init_lidar_rays(self, n_rays: int):
        """Precompute LiDAR ray unit-directions from pattern config (yaw-aligned frame)."""
        ...
        for el in elevations:
            ...
            for az in azimuths:
                dirs.append(torch.stack([cos_el * torch.cos(az),
                                         cos_el * torch.sin(az),
                                         sin_el]))
        self._lidar_ray_dirs = torch.stack(dirs)  # (N_rays, 3)
        # All rays are horizontal (single 0° elevation channel) → all indices selected
        self._lidar_horiz_indices = torch.where(
            self._lidar_ray_dirs[:, 2].abs() < 0.1
        )[0]
    ...
    def _augment_lidar_with_obstacles(
        ...
        N = self.num_envs
        proc_dirs = self._lidar_ray_dirs[horiz_idx]  # (N_proc, 3)

        # Extract yaw for ray rotation (ray_alignment="yaw")
        qw, qx, qy, qz = root_quat[:, 0], root_quat[:, 1], root_quat[:, 2], root_quat[:, 3]
        yaw = torch.atan2(2 * (qw * qz + qx * qy), 1 - 2 * (qy * qy + qz * qz))
        cos_y, sin_y = torch.cos(yaw), torch.sin(yaw)

        # Rotate ray XY directions by yaw -> world frame  (N, N_proc)
        ...
        # Analytical ray-cylinder intersection for each obstacle
        for oi in range(self._num_obstacles):
            cx = self._obstacle_pos_w[:, oi, 0:1]  # (N, 1)
            ...
            # Check both intersection points (entry & exit)
            for t_cand in [(-b - sqrt_d) / (2 * a + 1e-8),
                           (-b + sqrt_d) / (2 * a + 1e-8)]:
    ...
    def _get_observations(self) -> dict:
        """Observations: lin_vel(3)+ang_vel(3)+gravity(3)+goal(3)+lidar_360(359)+phase(5)+nearest_obs(3) = 379."""
        root_pos = self._robot.data.root_pos_w
        root_quat = self._robot.data.root_quat_w

        goal_pos_b, _ = subtract_frame_transforms(
            root_pos, root_quat, self._goal_pos_w,
        )

        # LiDAR: compute distances from sensor to hit points, normalize to [0, 1]
        lidar_hits = self._lidar.data.ray_hits_w          # (num_envs, N_rays, 3)
        lidar_pos = self._lidar.data.pos_w.unsqueeze(1)   # (num_envs, 1, 3)
        lidar_distances = torch.linalg.norm(lidar_hits - lidar_pos, dim=-1)  # (num_envs, N_rays)
        # Augment horizontal rays with analytical cylinder obstacle detection
        ...
        # Store augmented distances for debug visualization
        ...
        drone_xy = root_pos[:, :2].unsqueeze(1)             # (N, 1, 2)
        obs_delta_w = self._obstacle_pos_w - drone_xy       # (N, num_obs, 2)
        obs_center_dists = torch.linalg.norm(obs_delta_w, dim=2)  # (N, num_obs)
        obs_surface_dists = obs_center_dists - self._obstacle_radii.unsqueeze(0)  # (N, num_obs)

        # Find nearest obstacle per env
        min_idx = obs_surface_dists.argmin(dim=1)            # (N,)
        batch_idx = torch.arange(self.num_envs, device=self.device)

        # Direction to nearest obstacle in world frame
        nearest_delta_w = obs_delta_w[batch_idx, min_idx]    # (N, 2)
        nearest_dist = obs_surface_dists[batch_idx, min_idx] # (N,)

        # Transform direction to body frame using yaw rotation
        qw, qx, qy, qz = root_quat[:, 0], root_quat[:, 1], root_quat[:, 2], root_quat[:, 3]
        yaw = torch.atan2(2.0 * (qw * qz + qx * qy), 1.0 - 2.0 * (qy * qy + qz * qz))
        cos_yaw = torch.cos(-yaw)  # negative for world-to-body
        ...
        nearest_body_xy = torch.stack([nearest_body_x, nearest_body_y], dim=1)  # (N, 2)
        body_dir_norm = torch.linalg.norm(nearest_body_xy, dim=1, keepdim=True).clamp(min=1e-6)
        nearest_body_dir = nearest_body_xy / body_dir_norm  # (N, 2) unit direction

        # Normalize distance: 0 = touching surface, 1 = at safe_distance or beyond
        nearest_dist_normalized = torch.clamp(
            nearest_dist / self.cfg.obstacle_safe_distance, 0.0, 1.0
        ).unsqueeze(1)  # (N, 1)

        # Store for debug visualization
        nearest_obs_xy = self._obstacle_pos_w[batch_idx, min_idx]  # (N, 2)
        ...
        # Store min horizontal LiDAR distance for reward computation
        if self._lidar_horiz_indices is not None and len(self._lidar_horiz_indices) > 0:
            horiz_dists = lidar_distances[:, self._lidar_horiz_indices]  # (N, 4)
            self._min_horiz_lidar_dist = horiz_dists.min(dim=1).values  # (N,)
        else:
            self._min_horiz_lidar_dist = torch.full(
                (self.num_envs,), self.cfg.lidar_max_distance, device=self.device
            )
        ...
        obs = torch.cat([
            ...
            goal_pos_b,                             # 3
            lidar_normalized,                       # 359 (360° ring at 1° res)
            phase_one_hot,                          # 5
            nearest_body_dir,                       # 2
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Distance from start position (XY only, for STABILIZE phase)
        ...
        # Continuous reward: higher altitude = more reward (prevents getting stuck at ground)
        alt_fraction = (altitude / self.cfg.cruise_altitude).clamp(0.0, 1.0)
        takeoff_alt_reward = torch.where(
            is_takeoff,
            alt_fraction * self.cfg.takeoff_altitude_reward_scale * dt,
        ...
        # ── Phase 2: NAVIGATE (multi-waypoint + obstacle aware) ─────────────
        ...
        # ── Obstacle proximity penalty (NAVIGATE phase) ──────────────────
        drone_xy = root_pos[:, :2].unsqueeze(1)           # (num_envs, 1, 2)
        obstacle_xy = self._obstacle_pos_w                 # (num_envs, num_obstacles, 2)
        obstacle_dists = torch.linalg.norm(drone_xy - obstacle_xy, dim=2)  # (num_envs, num_obstacles)
        ...
        proximity_raw = torch.exp(-min_obstacle_dist) * (
            min_obstacle_dist < self.cfg.obstacle_safe_distance
        ).float()
        obstacle_proximity = torch.where(
            is_navigate,
            proximity_raw * self.cfg.obstacle_proximity_penalty_scale * dt,
            torch.zeros_like(proximity_raw),
        )
        ...
        lidar_penalty = torch.where(
            is_navigate,
            lidar_danger * lidar_closeness * self.cfg.lidar_obstacle_penalty_scale * dt,
            torch.zeros_like(lidar_dist),
        )
        ...
        # Controlled descent: penalize vertical speed exceeding land_max_descent_speed
        ...
        # Reward descending at ideal speed (0.2-0.5 m/s)
        descent_vel = -lin_vel_w[:, 2]  # positive when descending
        ...
        # Altitude penalty: encourage descent
        ...
        # ── All-phase rewards ────────────────────────────────────────────────
        # Reduce time penalty during early phases so takeoff/stabilize aren't overwhelmed
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Goal reached (LAND phase only, using final goal)
        ...
        # Touched ground during HOVER or LAND
        touched_ground = (
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Generate random waypoint sequence (with obstacle clearance)
        ...
        # Update obstacle world positions (local + env origin offset)
        env_origins_xy = self._terrain.env_origins[env_ids, :2]
        self._obstacle_pos_w[env_ids] = (
            self._obstacle_local.unsqueeze(0) + env_origins_xy.unsqueeze(1)
        )
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        if debug_vis:
            ...
            # Acquire debug draw interface (freespace-sim style thin lines)
            if self._draw is None and _debug_draw is not None:
                self._draw = _debug_draw.acquire_debug_draw_interface()

            # LiDAR thresholds for color coding
            self._lidar_close_thresh = 2.0   # red: danger
            self._lidar_mid_thresh = 6.0     # yellow: caution
    ...
    def _debug_vis_callback(self, event):
        """Show goal marker + waypoints + freespace-sim style colored LiDAR lines."""
        # ── Goal and waypoint markers ────────────────────────────────────
        ...
        # ── Freespace-sim style LiDAR debug draw (env 0 only) ───────────
        ...
        # Only visualize env 0 for performance
        root_pos = self._robot.data.root_pos_w[0].cpu()      # (3,)
        root_quat = self._robot.data.root_quat_w[0].cpu()    # (4,)
        lidar_dists = self._last_lidar_distances[0].cpu()     # (N_rays,)

        n_rays = self._lidar_ray_dirs.shape[0]
        ray_dirs = self._lidar_ray_dirs.cpu()  # (N_rays, 3)

        # Compute yaw for ray rotation
        qw, qx, qy, qz = root_quat[0], root_quat[1], root_quat[2], root_quat[3]
        ...
        for ri in range(n_rays):
            ...
            # Rotate by yaw (ray_alignment="yaw")
            dx_w = cos_y * dx_l - sin_y * dy_l
            dy_w = sin_y * dx_l + cos_y * dy_l
            dz_w = dz_l

            dist = float(lidar_dists[ri])
            dist = min(dist, self.cfg.lidar_max_distance)
            ...
            # Hit point dot (only for horizontal rays that hit something)
        ...
        # Draw all rays as thin lines
        if starts:
            self._draw.draw_lines(starts, ends, colors, widths)

        # Draw hit points as dots
        if hit_points:
...
gym.register(
    id="Isaac-CrazyflieObstacle-SAC-360Lidar-v0",
...
def setup_cameras(env_unwrapped):
    """Create 3 camera views: follow-cam, top-down, and side view."""
    ...
    # Main viewport stays FREE for manual zoom/orbit/pan
    # Follow cam goes in its own window instead
    ...
    print("[INFO] 4 camera views created:")
    print("       1. Main viewport  -> FREE (zoom/orbit/pan with mouse)")
    print("       2. Follow Cam     -> Auto-tracks drone")
    print("       3. Top-Down View  -> Bird's eye view")
    print("       4. Side View      -> Side perspective")
...
def update_cameras(cameras, drone_pos):
    """Update all camera positions to track the drone each frame."""
    from pxr import UsdGeom, Gf

    stage = cameras["stage"]
    x, y, z = float(drone_pos[0]), float(drone_pos[1]), float(drone_pos[2])

    # Camera 1: Follow cam (behind and slightly above)
    follow_cam = UsdGeom.Camera.Get(stage, cameras["follow"])
    xform = UsdGeom.Xformable(follow_cam.GetPrim())
    xform.ClearXformOpOrder()
    eye = Gf.Vec3d(x - 8.0, y - 5.0, z + 5.0)
    target = Gf.Vec3d(x, y, z)
    up = Gf.Vec3d(0, 0, 1)
    cam_matrix = Gf.Matrix4d()
    cam_matrix.SetLookAt(eye, target, up)
    cam_transform = cam_matrix.GetInverse()
    xform.AddTransformOp().Set(cam_transform)

    # Camera 2: Top-down
    topdown_cam = UsdGeom.Camera.Get(stage, cameras["topdown"])
    xform2 = UsdGeom.Xformable(topdown_cam.GetPrim())
    xform2.ClearXformOpOrder()
    eye2 = Gf.Vec3d(x - 5.0, y - 5.0, 20.0)
    target2 = Gf.Vec3d(x, y, z)
    up2 = Gf.Vec3d(0, 1, 0)
    cam_matrix2 = Gf.Matrix4d()
    cam_matrix2.SetLookAt(eye2, target2, up2)
    cam_transform2 = cam_matrix2.GetInverse()
    xform2.AddTransformOp().Set(cam_transform2)

    # Camera 3: Side view
    side_cam = UsdGeom.Camera.Get(stage, cameras["side"])
    xform3 = UsdGeom.Xformable(side_cam.GetPrim())
    xform3.ClearXformOpOrder()
    eye3 = Gf.Vec3d(x, y - 15.0, z + 3.0)
    target3 = Gf.Vec3d(x, y, z)
    up3 = Gf.Vec3d(0, 0, 1)
    cam_matrix3 = Gf.Matrix4d()
    cam_matrix3.SetLookAt(eye3, target3, up3)
    cam_transform3 = cam_matrix3.GetInverse()
    xform3.AddTransformOp().Set(cam_transform3)
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "sac", "crazyflie_obstacle_nav_sac_360lidar")
    ...
    env = gym.make("Isaac-CrazyflieObstacle-SAC-360Lidar-v0", cfg=env_cfg)
...
def play():
    """Evaluate a SAC policy with multi-camera visualization."""
    ...
    print(f"[INFO] 4 camera views active:", flush=True)
    print(f"       Main   -> FREE (zoom/orbit/pan with mouse)", flush=True)
    print(f"       Window -> Follow Cam (auto-tracks drone)", flush=True)
    print(f"       Window -> Top-Down (bird's eye)", flush=True)
    print(f"       Window -> Side View", flush=True)
        ...
        while simulation_app.is_running():
            ...
            if step_count % 50 == 0:
                ...
                print(f"  Step {step_count} | Phase: {phase_names[phase]} | WP: {wp_idx}/{num_wps} | "
                      ...
                      f"Reward: {rewards[0].item():.3f}", flush=True)
...
def evaluate():
    """Run N episodes and report performance metrics."""
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION: SAC Obstacle+MultiWP | {args_cli.num_episodes} episodes")
    ...
    print(f"Obstacles: {len(env_cfg.obstacle_positions)} trees")
    print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS -- SAC Obstacle+MultiWP ({n} episodes)")
    print(f"{'='*60}")
