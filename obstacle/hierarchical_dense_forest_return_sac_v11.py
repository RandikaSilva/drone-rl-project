"""Hierarchical Crazyflie obstacle avoidance + return-to-start with SAC.
VERSION 11 — v10 env + middle wall of trees + cross-arena mission + return-home on stuck.

Built on v10 (99% success baseline on dense forest).
Mission: fly from the LEFT side of the arena to the RIGHT side.
The middle of the arena (x ~ 0) is BLOCKED by a solid wall of densely packed trees.
If the drone gets stuck (cannot advance waypoints for too long), the final goal
is automatically redirected back to the starting position — the drone learns
to give up and fly home rather than crash or time out in the tree wall.

v11 changes vs v10:
    - Start on the LEFT side (-11, 0). Waypoints move left -> right across arena.
    - Middle wall of tightly-packed trees (impassable / very tight gaps).
    - Return-home trigger: after `return_home_stuck_timeout` seconds without
      advancing a waypoint while navigating, final goal is swapped to the
      start position. Drone must fly home and land.
    - `return_home_bonus` reward on successful return-to-start.
...
Usage (from IsaacLab directory):
    ...
    # Train high-level:
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/hierarchical_dense_forest_return_sac_v11.py \
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Dense Forest Return-to-Start — SAC v11")
...
parser.add_argument("--min_waypoints", type=int, default=2)
...
# v10: Dense forest with varied trees + bushes (built on v9 smoothing + v8 safety)
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v11: Middle-wall forest + cross-arena mission + return-home on stuck. Built on v10."""

    episode_length_s = 180.0  # v8+: extended episode time for cautious navigation
    ...
    # ── Domain Randomization (v9) ────────────────────────────────────────────
    # Wind disturbance — smooth OU-process wind in world frame
    dr_wind_enabled: bool = True
    dr_wind_max_speed: float = 0.8       # max wind speed per axis (m/s)
    dr_wind_theta: float = 0.5           # OU mean-reversion rate (higher = gustier)
    dr_wind_sigma: float = 0.3           # OU walk intensity (m/s)

    # Velocity response scaling — simulates mass/motor efficiency variation
    dr_vel_scale_enabled: bool = True
    dr_vel_scale_range: tuple = (0.85, 1.15)  # per-env uniform sample

    # Drag coefficient randomization
    dr_drag_enabled: bool = True
    dr_drag_range: tuple = (0.03, 0.10)  # per-env drag_tilt_coeff range

    # LiDAR noise — additive Gaussian on normalized distances
    dr_lidar_noise_enabled: bool = True
    dr_lidar_noise_std: float = 0.05     # std of noise added to [0,1] normalized lidar

    # Observation noise — simulates noisy IMU (velocity & angular velocity)
    dr_obs_noise_enabled: bool = True
    dr_obs_vel_noise_std: float = 0.05   # std added to lin_vel_b (m/s)
    dr_obs_angvel_noise_std: float = 0.03  # std added to ang_vel_b (rad/s)
    ...
    # ── Multi-Waypoint (v11: cross-arena, left -> right) ─────────────────────
    # Start on the LEFT side. Intermediate waypoints also on the LEFT side
    # (before the wall). Final waypoint on the RIGHT side (past the wall) —
    # the wall blocks the direct path, so the drone must find a gap or give up
    # and return home.
    static_start_pos: tuple = (-11.0, 0.0, 0.2)
    min_num_waypoints: int = 2
    max_num_waypoints: int = 3
    # Left (start) side x-range for intermediate waypoints
    near_x_range: tuple = (-8.0, -3.5)
    # Right (far) side x-range for final waypoint
    far_x_range: tuple = (4.0, 10.0)
    ...
    # ── Obstacles (v11: varied middle wall with a visual gap that's too narrow) ─
    # Format: (x, y, radius, height). Bushes: height <= 1.5m (flyable-over at cruise_altitude=1.5m)
    # Middle wall: varied-size trees at x≈0, slightly staggered. A visual gap
    # sits near y=0 where two small trees leave a ~0.6m surface gap — drone
    # (wall_radius 0.65m → needs 1.3m clearance) CANNOT squeeze through, but
    # the gap looks tempting to the policy.
    obstacle_positions: list = [
        # ── MIDDLE WALL (14 varied trees at x≈0, staggered) — LOOKS PASSABLE, ISN'T ──
        (-0.3, -10.0, 1.00, 6.5),
        ( 0.2,  -8.5, 0.70, 5.5),
        (-0.1,  -7.0, 0.90, 6.0),
        ( 0.3,  -5.5, 0.50, 4.8),
        (-0.2,  -4.0, 0.85, 5.8),
        ( 0.1,  -2.5, 0.60, 5.0),
        (-0.3,  -1.1, 0.35, 4.0),  # south edge of the "visual gap"
        ( 0.3,   0.4, 0.35, 4.0),  # north edge of the "visual gap" (surface-to-surface ~0.8m)
        (-0.1,   1.8, 0.55, 5.0),
        ( 0.2,   3.3, 0.80, 5.8),
        (-0.3,   4.8, 0.45, 4.5),
        ( 0.1,   6.3, 0.95, 6.2),
        (-0.2,   8.0, 0.65, 5.5),
        ( 0.3,  10.0, 1.05, 6.5),
        # ── LEFT SIDE scattered trees (7) — obstacles on start side ──
        (-7.0,  7.5, 0.50, 5.0),
        (-8.5, -7.0, 0.55, 5.5),
        (-6.0, -3.5, 0.45, 5.0),
        (-4.0,  2.5, 0.40, 4.5),
        ...
        (-5.5,  5.5, 0.38, 4.5),
        (-8.5,  3.0, 0.48, 5.0),
        # ── RIGHT SIDE scattered trees (7) — obstacles on goal side ──
        ( 7.0, -7.5, 0.50, 5.0),
        ( 8.5,  7.0, 0.55, 5.5),
        ( 6.0,  3.5, 0.45, 5.0),
        ( 4.0, -2.5, 0.40, 4.5),
        ...
        ( 5.5, -5.5, 0.38, 4.5),
        ( 8.5, -3.0, 0.48, 5.0),
        # ── Border trees (8) — arena perimeter ──
        (-11.5,  9.0, 0.50, 5.0),
        (-11.5, -9.0, 0.50, 5.0),
        ( 11.5,  9.0, 0.50, 5.0),
        ( 11.5, -9.0, 0.50, 5.0),
        ...
        ( 0.0,  11.5, 0.50, 5.0),
        ( 0.0, -11.5, 0.50, 5.0),
        # ── BUSHES (6) — short, flyable-over (for visual density) ──
        (-6.5,  0.0, 0.70, 1.0),
        ( 6.5,  0.0, 0.75, 1.1),
        (-3.0, -5.0, 0.65, 0.8),
        ( 3.0,  5.0, 0.80, 0.9),
    ...
    crash_penalty: float = -900.0                     # v6: slightly increased from -800
    ...
    # v10: Stagnation penalty — growing penalty when drone fails to advance waypoints
    ...
    # v11: force_hover fires AFTER return-home, so the drone first tries to return
    # home, and only if still stuck after returning does it force-land in place.
    final_wp_force_hover_timeout: float = 25.0
    # v11: Return-to-start when drone can't find a path through the middle wall.
    # After this many seconds without advancing a waypoint while navigating at
    # the final waypoint, the final goal is redirected to the starting position.
    # Must be > stagnation_timeout (stagnation penalty kicks in first) and
    # < final_wp_force_hover_timeout (so return-home fires before force-land).
    return_home_stuck_timeout: float = 10.0
    return_home_bonus: float = 500.0  # reward added once when return-home triggers
    return_home_success_bonus: float = 500.0  # additional reward when drone actually reaches start
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._actual_vel_w = torch.zeros(self.num_envs, 3, device=self.device)  # smoothed velocity (drone inertia)
        self._prev_vel_w = torch.zeros(self.num_envs, 3, device=self.device)
        # Smoothed body-frame commands (prevents oscillation from HL/safety jumps)
        ...
        self._smoothed_accel_w = torch.zeros(self.num_envs, 3, device=self.device)  # EMA-filtered acceleration for tilt
        ...
        # v9: Domain randomization buffers
        self._dr_wind = torch.zeros(self.num_envs, 3, device=self.device)  # OU wind state (world frame)
        self._dr_vel_scale = torch.ones(self.num_envs, device=self.device)  # per-env velocity scaling
        self._dr_drag_coeff = torch.full((self.num_envs,), self.cfg.drag_tilt_coeff, device=self.device)  # per-env drag
        ...
        # v11: Return-to-start state — True once redirected back to the start
        self._return_home_triggered = torch.zeros(
        ...
        print(f"[INFO] v11: middle-wall forest ({self._num_obstacles} obstacles, tree wall at x~0) + cross-arena mission + return-home on stuck. Low-level: deterministic proportional controller.")
        dr_features = []
        ...
        self._episode_sums = {
            ...
            for key in [
                ...
                "return_home_trigger", "return_home_success",
            ]
        }
    ...
    def _setup_scene(self):
        ...
        bush_colors = [
            (0.22, 0.45, 0.15),  # olive green
            (0.18, 0.38, 0.12),  # dark olive
            (0.28, 0.50, 0.18),  # bright olive
            (0.20, 0.42, 0.10),  # mossy green
        ]
        for i, (ox, oy, radius, height) in enumerate(self.cfg.obstacle_positions):
            is_bush = height <= self.cfg.bush_height_threshold

            if is_bush:
                # Bush: single short wide cylinder, no canopy
                b_color = bush_colors[i % len(bush_colors)]
                bush_cfg = sim_utils.CylinderCfg(
                    radius=radius, height=height, axis="Z",
                    collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
                    rigid_props=sim_utils.RigidBodyPropertiesCfg(kinematic_enabled=True, disable_gravity=True),
                    visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=b_color),
                )
                bush_cfg.func(
                    f"/World/envs/env_0/Bush_{i}", bush_cfg,
    ...
    def _generate_waypoints(self, env_ids: torch.Tensor):
        """v11: cross-arena waypoints. All intermediate waypoints on the LEFT
        (start) side of the middle wall; the FINAL waypoint on the RIGHT (goal)
        side. The drone must cross the tree wall to succeed, or give up and
        return home once stuck."""
        num_reset = len(env_ids)
        device = self.device
        max_wps = self.cfg.max_num_waypoints
        near_xmin, near_xmax = self.cfg.near_x_range
        far_xmin, far_xmax = self.cfg.far_x_range
        ...
        # Intermediate waypoints on near (left) side — random x in near_x_range,
        # zigzag y. Sorted left-to-right (x ascending, but still all negative).
        wp_x = torch.zeros(num_reset, max_wps, device=device)
        if max_wps > 1:
            inter_x = near_xmin + torch.rand(num_reset, max_wps - 1, device=device) * (near_xmax - near_xmin)
            inter_x, _ = inter_x.sort(dim=1)
            wp_x[:, :max_wps - 1] = inter_x
        # Final waypoint: far (right) side
        wp_x[:, -1] = far_xmin + torch.rand(num_reset, device=device) * (far_xmax - far_xmin)

        wp_y = torch.zeros(num_reset, max_wps, device=device)
        # Start the zig-zag from a small random y near the starting position
        ...
        # Obstacle clearance nudging — per-waypoint clamp ranges
        # (near-side intermediates clamp to near_x_range, final to far_x_range)
        ...
        for wi in range(max_wps):
            # Choose x-clamp range for this waypoint slot
            if wi == max_wps - 1:
                x_lo, x_hi = far_xmin, far_xmax
            else:
                x_lo, x_hi = near_xmin, near_xmax
            ...
            if too_close.any():
                ...
                wp_x[too_close, wi] = (wp_x[too_close, wi] + nudge[:, 0]).clamp(x_lo, x_hi)
        ...
        # v11: landing Z = takeoff Z (from static_start_pos). So drone lands where it took off.
        self._final_goal_pos_w[env_ids, 2] = self.cfg.static_start_pos[2]
    ...
    def _check_waypoint_advancement(self, xy_dist: torch.Tensor, speed: torch.Tensor) -> torch.Tensor:
        ...
        if reached.any():
            ...
            # v8: Track waypoint advance time for anti-stall
            self._last_wp_advance_time[reached] = self._episode_time[reached]

        return reached

    def _check_return_home_trigger(self) -> torch.Tensor:
        """v11: if drone is stuck at the final waypoint for `return_home_stuck_timeout`
        seconds while navigating, redirect the final goal to the starting position.
        Returns a bool tensor of envs that JUST triggered return-home this step."""
        is_navigate = self._phase == self.NAVIGATE
        is_at_final = self._current_wp_idx >= (self._num_waypoints - 1)
        time_since_advance = self._episode_time - self._last_wp_advance_time
        stuck = (time_since_advance > self.cfg.return_home_stuck_timeout)
        trigger = is_navigate & is_at_final & stuck & ~self._return_home_triggered

        if trigger.any():
            trig_ids = torch.where(trigger)[0]
            # Redirect final goal to start position (keep cruise altitude while flying,
            # landing Z is applied when transitioning to LAND).
            start_xy = self._start_pos_w[trig_ids, :2]
            max_wps = self.cfg.max_num_waypoints
            final_idx = (self._num_waypoints[trig_ids] - 1).long()
            batch_idx = torch.arange(len(trig_ids), device=self.device)
            # Replace the final waypoint with start (xy + cruise altitude for flying home)
            self._waypoints_w[trig_ids, final_idx, 0] = start_xy[:, 0]
            self._waypoints_w[trig_ids, final_idx, 1] = start_xy[:, 1]
            self._waypoints_w[trig_ids, final_idx, 2] = self.cfg.cruise_altitude
            # v11: Landing goal = actual takeoff point (XY + Z). Land where we took off —
            # ground if we started on ground, truck roof if we started on a truck, etc.
            self._final_goal_pos_w[trig_ids, 0] = start_xy[:, 0]
            self._final_goal_pos_w[trig_ids, 1] = start_xy[:, 1]
            self._final_goal_pos_w[trig_ids, 2] = self._start_pos_w[trig_ids, 2]
            # Update current goal and reset stagnation clock so the drone gets a fresh chance
            self._update_current_goal(trig_ids)
            new_delta = self._goal_pos_w[trig_ids] - self._robot.data.root_pos_w[trig_ids]
            self._prev_xy_dist[trig_ids] = torch.linalg.norm(new_delta[:, :2], dim=1)
            self._last_wp_advance_time[trig_ids] = self._episode_time[trig_ids]
            self._return_home_triggered[trig_ids] = True
    ...
    def _init_hl_lidar_rays(self, n_rays: int):
        ...
        # v10: include -22.5° elevation rays (abs(dz) ~ 0.38) to detect short bushes
    ...
    def _run_low_level_policy(self):
        ...
        # v8: Multi-obstacle velocity-projection safety + predictive avoidance
        # Handles ALL obstacles within range (not just nearest) to prevent dodging into a second obstacle
        nav_mask = (phase == self.NAVIGATE)
        if nav_mask.any():
            drone_pos = self._robot.data.root_pos_w
            drone_xy = drone_pos[:, :2].unsqueeze(1)  # (N, 1, 2)
            ...
            # --- Predictive avoidance: project velocity forward ---
            vel_w_actual = self._robot.data.root_lin_vel_w[:, :2]  # (N, 2)
            lookahead = self.cfg.predictive_avoidance_lookahead
            predicted_xy = drone_pos[:, :2] + vel_w_actual * lookahead  # (N, 2)
            pred_delta = predicted_xy.unsqueeze(1) - self._obstacle_pos_w  # (N, num_obs, 2)
            pred_dists = torch.linalg.norm(pred_delta, dim=2)
            pred_surface = pred_dists - self._obstacle_radii.unsqueeze(0)

            # Effective surface distance: min of current and predicted for EACH obstacle
            effective_surface = torch.min(obs_surface, pred_surface)  # (N, num_obs)

            safety_d = self.cfg.reactive_safety_distance
            # Find which drones have ANY obstacle within safety distance
            min_eff_surf = effective_surface.min(dim=1).values
            apply_mask = nav_mask & (min_eff_surf < safety_d)

            if apply_mask.any():
                # Compute yaw for body↔world transforms
                ...
                vel_w = torch.stack([vx_w, vy_w], dim=1)  # (N, 2)

                # Process ALL obstacles within safety distance (not just nearest)
                # Strength per obstacle: (N, num_obs), 0 if outside safety_d
                strength_all = (1.0 - effective_surface / safety_d).clamp(0.0, 1.0)  # (N, num_obs)
                # Direction away from each obstacle: (N, num_obs, 2)
                away_dirs = obs_delta / (obs_dists.unsqueeze(2) + 1e-6)  # unit vectors away from each obs

                # For each obstacle, compute approach speed along its away direction
                # vel_w: (N, 2) → (N, 1, 2), away_dirs: (N, num_obs, 2)
                approach_speeds = (vel_w.unsqueeze(1) * away_dirs).sum(dim=2)  # (N, num_obs), positive=away

                # Accumulate corrections from all threatening obstacles
                total_correction = torch.zeros_like(vel_w)  # (N, 2)

                # Mask: obstacles within safety distance
                in_range = effective_surface < safety_d  # (N, num_obs)

                # Remove approach velocity component for each threatening obstacle
                approaching = approach_speeds < 0  # (N, num_obs) — moving toward this obstacle
                remove_mask_obs = in_range & approaching  # (N, num_obs)
                # Correction per obstacle: remove approach component scaled by strength
                remove_frac = strength_all  # (N, num_obs)
                correction_mag = (-approach_speeds * remove_frac).clamp(min=0.0)  # (N, num_obs)
                correction_mag = correction_mag * remove_mask_obs.float()  # zero out non-threatening
                # Sum correction vectors from all obstacles
                correction_vecs = away_dirs * correction_mag.unsqueeze(2)  # (N, num_obs, 2)
                total_correction += correction_vecs.sum(dim=1)  # (N, 2)

                # Push-away from all obstacles within range
                push_mag = strength_all * self.cfg.reactive_safety_push_gain * in_range.float()  # (N, num_obs)
                push_vecs = away_dirs * push_mag.unsqueeze(2)  # (N, num_obs, 2)
                total_correction += push_vecs.sum(dim=1)  # (N, 2)

                # v10: Anti-stuck escape — when between multiple obstacles, push vectors
                # cancel out leaving near-zero correction. Detect this and inject tangential
                # escape velocity perpendicular to the goal direction.
                num_threatening = in_range.float().sum(dim=1)  # (N,) count of nearby obstacles
                correction_mag_total = torch.linalg.norm(total_correction, dim=1)  # (N,)
                # Stuck = 2+ obstacles nearby but correction vectors cancelled out
                is_stuck = apply_mask & (num_threatening >= 2) & (correction_mag_total < 0.3)
                if is_stuck.any():
                    # Compute tangential direction (perpendicular to goal direction)
                    ...
                    # Perpendicular: rotate 90° (pick consistent direction based on nearest obstacle side)
                    perp_dir = torch.stack([-goal_dir[:, 1], goal_dir[:, 0]], dim=1)  # (stuck, 2)
                    # Choose escape side: toward the direction with more clearance
                    # Use dot product of perp with away-from-nearest-obstacle direction
                    nearest_obs_idx = effective_surface[is_stuck].argmin(dim=1)  # (stuck,)
                    stuck_indices = torch.where(is_stuck)[0]  # absolute env indices
                    nearest_away = away_dirs[stuck_indices, nearest_obs_idx]  # (stuck, 2)
                    # Flip perp to point toward more open side
                    side_dot = (perp_dir * nearest_away).sum(dim=1, keepdim=True)
                    perp_dir = perp_dir * side_dot.sign()  # flip if needed
                    # Escape speed scales with stagnation time
    ...
    def _step_ou_noise(self, dt: float):
        ...
        # v9: Advance wind OU process (world-frame, 3D)
        if self.cfg.dr_wind_enabled:
            ...
            # Wind Z component is weaker (updrafts/downdrafts less common)
    ...
    def _kinematic_update(self):
        ...
        # ── Smooth body-frame commands (simulates motor/control lag) ──
        # Prevents oscillation from HL goal modifier jumps & safety velocity corrections
        # alpha=0.06 → 95% in ~490ms at 100Hz — matches real Crazyflie velocity response
        cmd_alpha = 0.06
        ...
        # v9: Apply velocity response scaling (simulates mass/motor variation)
        ...
        # v9: Add wind disturbance in world frame
        if self.cfg.dr_wind_enabled:
        ...
        # Integrate position (commands already smooth from body-frame EMA)
        ...
        # World-frame acceleration — EMA-filtered for smooth tilt transitions
        raw_accel_w = (self._desired_vel_w - self._prev_vel_w) / dt
        self._prev_vel_w = self._desired_vel_w.clone()
        accel_alpha = 0.15  # 95% in ~185ms — smooth banking without tilt snapping
        self._smoothed_accel_w = (1 - accel_alpha) * self._smoothed_accel_w + accel_alpha * raw_accel_w

        # Rotate smoothed acceleration into body frame (using yaw)
        ...
        # v9: Use per-env randomized drag coefficient if enabled
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # v8: Track episode time for anti-stall detection
    ...
    def _get_observations(self) -> dict:
        ...
        # v9: Add LiDAR noise (additive Gaussian on normalized distances)
        ...
        # v9: Noisy velocity/angular velocity observations (simulates noisy IMU)
        ...
        obs = torch.cat([
            ...
            lin_vel_b_obs,                          # 3 (v9: with noise)
            ang_vel_b_obs,                          # 3 (v9: with noise)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # v10: Stagnation penalty — grows linearly when drone hasn't advanced a waypoint
        time_since_advance = self._episode_time - self._last_wp_advance_time
        ...
        # v11: Return-to-start trigger — redirect goal to start if stuck too long
        return_home_just_triggered = self._check_return_home_trigger()
        return_home_trigger_bonus = return_home_just_triggered.float() * self.cfg.return_home_bonus
        ...
        # v11: Return-home success bonus — drone landed (or was about to land) near the
        # start after being redirected home.
        return_home_success_bonus = (
            goal_reached & self._return_home_triggered
        ).float() * self.cfg.return_home_success_bonus
        ...
        # v11: tighter tolerance when returning home so drone actually lands AT start, not 5m away
        final_wp_tol = torch.where(
            self._return_home_triggered,
            torch.full_like(current_xy_dist, 1.5),
            torch.full_like(current_xy_dist, self.cfg.final_waypoint_tolerance),
        )
        normal_hover = is_at_final_wp & (current_xy_dist < final_wp_tol)
        ...
        # v11: exclude return-home envs so they keep pushing toward start instead of giving up mid-route
        force_hover_landing = to_hover & force_hover & ~normal_hover & ~self._return_home_triggered
        ...
        reward = (
            ...
            + return_home_trigger_bonus + return_home_success_bonus
            + hover_position + hover_speed + hover_altitude
            + land_descent + land_xy_stability + land_drift + land_precision
        ...
        self._episode_sums["return_home_trigger"] += return_home_trigger_bonus
        self._episode_sums["return_home_success"] += return_home_success_bonus
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # v11: Return-home metrics
        rth = self._return_home_triggered[env_ids].float()
        self.extras["log"]["Metrics/return_home_triggered_rate"] = rth.mean().item()
        ...
        # v9: Re-sample domain randomization parameters for reset envs
        self._dr_wind[env_ids] = 0.0  # reset wind state
        if self.cfg.dr_vel_scale_enabled:
        ...
        # v8: Reset anti-stall timers
        self._last_wp_advance_time[env_ids] = 0.0
        self._episode_time[env_ids] = 0.0

        # v11: Reset return-home trigger
        self._return_home_triggered[env_ids] = False
...
gym.register(
    id="Isaac-HierarchicalDenseForestReturn-SAC-v11",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v11", "crazyflie_hierarchical_obstacle_sac_v11")
    ...
    print(f"[INFO] Obstacles: {len(env_cfg.obstacle_positions)} (trees + bushes)")
    ...
    env = gym.make("Isaac-HierarchicalDenseForestReturn-SAC-v11", cfg=env_cfg)
...
def play():
    ...
    env_cfg.episode_length_s = 180.0  # v8+: extended episode time
    env_cfg.debug_vis = True
    env_cfg.min_num_waypoints = args_cli.min_waypoints
    env_cfg.max_num_waypoints = args_cli.max_waypoints

    # Disable DR for play — v6 checkpoint was trained without DR, noise causes oscillation
    ...
    # v10: smoothing for stable flight (same as v9)
    env_cfg.cmd_smoothing_alpha = 0.06
    env_cfg.accel_smoothing_alpha = 0.15
    ...
    total_reward = 0.0
    step_count = 0
    base_step_dt = env_cfg.sim.dt * env_cfg.decimation
    playback_dt = base_step_dt * 3.0  # 3x slower in flight
    land_playback_dt = base_step_dt * 8.0  # 8x slower during HOVER/LAND so landing is visible

    print(f"\n{'='*60}")
    print(f"[INFO] v11: middle-wall forest + cross-arena mission + return-home on stuck. Deterministic proportional LL controller.")
    if agent:
    ...
    try:
        while simulation_app.is_running():
            ...
            current_phase = unwrapped._phase[0].item()
            # Print every step (user requested) so landing stage is visible
            if True:
                ...
                rth = bool(unwrapped._return_home_triggered[0].item())
                rth_tag = " [RETURN-HOME]" if rth else ""
                print(f"  Step {step_count} | {phase_names[phase]} | WP {wp_idx}/{num_wps}{rth_tag} | "
            ...
            # Slow down further during HOVER (3) and LAND (4) so landing is visible
            target_dt = land_playback_dt if current_phase in (3, 4) else playback_dt
...
def evaluate():
    ...
    # v10 eval: DR disabled (v6 checkpoint), smoothing enabled (v9/v10 feature)
    env_cfg.dr_wind_enabled = False
    env_cfg.dr_vel_scale_enabled = False
    env_cfg.dr_drag_enabled = False
    env_cfg.dr_lidar_noise_enabled = False
