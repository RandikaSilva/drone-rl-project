...
VERSION 10 — Dense forest with varied trees and bushes.

v9 baseline: 99% success, 0% crash, 1% timeout.
v10 changes: More complex environment — 35 obstacles (varied trees + bushes).
    Bushes are short wide cylinders (h=0.8-1.2m) the drone can fly over at cruise altitude.
    LiDAR augmentation extended to mid-angle rays so downward-looking channels detect bushes.
    Varied trunk radii across all tree categories for visual realism.
    Stagnation penalty: growing penalty when drone fails to advance waypoints.
    All other config (smoothing, safety, rewards, architecture, DR) identical to v9.
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    obstacle_positions: list = [
        # ── Border trees (12) — loose perimeter, varied trunk radii ──
        # Top wall
        (-7.0, 10.0, 0.45, 5.0),
        (-2.0, 10.0, 0.55, 5.5),
        ( 3.0, 10.0, 0.40, 4.5),
        ...
        ( 9.0, -5.0, 0.58, 5.0),
        ( 9.0,  5.0, 0.44, 5.5),
        # ── Interior LARGE trees (5) — tall thick trunks, varied radii ──
        ( 0.0,  0.0, 0.75, 7.0),   # center — forces left/right split (thickest)
        (-4.5,  4.5, 0.65, 6.5),   # upper-left quadrant
        ( 4.5, -4.5, 0.55, 6.0),   # lower-right quadrant
        (-4.0, -3.0, 0.60, 5.5),   # lower-left quadrant
        ( 4.0,  3.0, 0.50, 5.5),   # upper-right quadrant
        # ── Interior MEDIUM trees (6) — varied corridor trees ──
        (-2.0,  2.5, 0.48, 4.5),   # upper-center-left
        ( 2.0, -2.5, 0.42, 4.5),   # lower-center-right
        (-7.0,  0.0, 0.38, 4.0),   # left midfield (thin)
        ( 7.0,  0.0, 0.52, 4.5),   # right midfield (thick)
        ( 0.0,  5.0, 0.46, 5.0),   # upper center
        ( 0.0, -5.0, 0.40, 4.0),   # lower center
        # ── Interior SMALL trees (4) — narrow tall trees, varied radii ──
        (-6.0,  2.5, 0.28, 4.0),   # left-upper corridor (thinnest)
        ( 6.0, -2.5, 0.35, 4.5),   # right-lower corridor
        ( 2.5,  5.5, 0.25, 3.5),   # upper-right gap (thinnest)
        (-2.5, -5.5, 0.32, 3.5),   # lower-left gap
        # ── BUSHES (8) — short wide cylinders, varied sizes ──
        (-5.5,  1.0, 0.85, 1.0),   # left corridor bush (wide)
        ( 5.5, -1.0, 0.70, 1.1),   # right corridor bush
        (-1.0,  4.0, 0.65, 0.8),   # upper-center bush (narrow)
        ( 1.0, -4.0, 0.80, 0.9),   # lower-center bush
        (-3.5, -6.0, 0.95, 1.2),   # lower-left bush (widest + tallest)
        ( 3.5,  6.0, 0.75, 1.0),   # upper-right bush
        ( 5.0,  2.0, 0.90, 1.1),   # right-upper bush (wide)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v10: dense forest (35 obstacles) + velocity-projection safety + deterministic proportional LL controller")
    if agent:
