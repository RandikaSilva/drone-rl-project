...
VERSION 9 — Domain Randomization for sim-to-real transfer.

v8 baseline: 96% success, 2% crash, 2% timeout.
v9 changes:  Domain randomization added on top of v8 for robust sim-to-real transfer.

Key changes from v8:
    1. Wind disturbance — smooth OU-process wind in world frame (up to 0.8 m/s)
    2. Velocity response scaling — per-env ±15% variation (simulates mass/motor differences)
    3. Drag coefficient randomization — per-env drag_tilt_coeff in [0.03, 0.10]
    4. LiDAR noise — additive Gaussian noise on normalized distances (std=0.05)
    5. Observation noise — additive noise on velocity/angular velocity readings (simulates noisy IMU)
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Obstacle Nav — SAC v9 (Domain Randomization)")
...
# v9: Domain randomization for sim-to-real transfer (built on v8 velocity-projection safety)
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v9: Domain randomization for sim-to-real. Built on v8 (96% success)."""
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print("[INFO] v9: domain randomization + v8 velocity-projection safety. Low-level: deterministic proportional controller (no checkpoint)")
        dr_features = []
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v9", "crazyflie_hierarchical_obstacle_sac_v9")
...
def play():
    ...
    # v9 smoothing for stable flight
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v9: domain randomization + velocity-projection safety (19 trees) + deterministic proportional LL controller")
    if agent:
...
def evaluate():
    ...
    # v9 eval: DR disabled (v6 checkpoint), smoothing enabled (v9 feature)
    env_cfg.dr_wind_enabled = False
    env_cfg.dr_vel_scale_enabled = False
    env_cfg.dr_drag_enabled = False
    env_cfg.dr_lidar_noise_enabled = False
