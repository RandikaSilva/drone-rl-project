...
parser.add_argument("--mode", type=str, default="train", choices=["train", "play", "eval", "debug"])
...
class LidarCNNExtractor(BaseFeaturesExtractor):
    """1D CNN for 450 LiDAR rays, MLP for state, fused together.

    Observation layout: [lidar_N, lin_vel_b(3), ang_vel_b(3), gravity_b(3),
                         goal_pos_b(3), phase_one_hot(5), nearest_obstacle(3)]
    """

    def __init__(self, observation_space, features_dim=128, lidar_dim=None, cnn_out_dim=32):
        ...
        # 1D CNN for LiDAR: (batch, 1, 450) -> (batch, cnn_out_dim)
        self.lidar_cnn = nn.Sequential(
            nn.Conv1d(1, 16, kernel_size=9, stride=4, padding=4),   # -> (16, 113)
            nn.ReLU(),
            nn.Conv1d(16, 32, kernel_size=5, stride=3, padding=2),  # -> (32, 38)
            nn.ReLU(),
            nn.Conv1d(32, cnn_out_dim, kernel_size=3, stride=2, padding=1),  # -> (32, 19)
            nn.ReLU(),
    ...
    def forward(self, observations):
        lidar = observations[:, :self.lidar_dim].unsqueeze(1)  # (batch, 1, 450)
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def _generate_waypoints(self, env_ids: torch.Tensor):
        ...
        # Obstacle clearance nudging (skip if no obstacles)
    ...
    def _init_hl_lidar_rays(self, n_rays: int):
        """Precompute high-level LiDAR ray directions (450 horizontal rays)."""
    ...
    def _get_observations(self) -> dict:
        ...
        # Layout: [lidar_450, lin_vel_b, ang_vel_b, gravity_b, goal_pos_b, phase, nearest_obstacle]
        obs = torch.cat([
            hl_lidar_norm,                          # 450
            self._robot.data.root_lin_vel_b,        # 3
            self._robot.data.root_ang_vel_b,        # 3
    ...
    def _debug_vis_callback(self, event):
        ...
        # Compute modified goal in body frame
        real_goal_b, _ = subtract_frame_transforms(root_pos, root_quat, self._goal_pos_w)
        ...
        # Rotate body-frame modified goal to world
        qw, qx, qy, qz = root_quat[:, 0], root_quat[:, 1], root_quat[:, 2], root_quat[:, 3]
        yaw = torch.atan2(2.0 * (qw * qz + qx * qy), 1.0 - 2.0 * (qy * qy + qz * qz))
        cos_yaw, sin_yaw = torch.cos(yaw), torch.sin(yaw)
...
# ═══════════════════════════════════════════════════════════════════════════════
# Debug  (dummy HL — verifies frozen LL + PID stack without a trained HL)
# ═══════════════════════════════════════════════════════════════════════════════

def debug():
    """Fly to a single waypoint ~3 m ahead using dummy HL actions (all zeros).

    The high-level outputs zeros → yaw_offset=0, speed_factor=mid, alt_offset=0
    so the frozen low-level sees the real goal directly.  No obstacles, no
    trained HL checkpoint required.  Use this to confirm that takeoff, the frozen
    LL, and PID all work before blaming the HL policy.
    """
    env_cfg = HierarchicalObstacleNavEnvCfg()
    env_cfg.scene.num_envs = 1
    ...
    # Single close waypoint directly ahead of the drone start position
    env_cfg.min_num_waypoints = 1
    env_cfg.max_num_waypoints = 1
    env_cfg.arena_x_range = (2.0, 4.0)   # 2–4 m ahead in X
    env_cfg.arena_y_range = (-0.5, 0.5)  # almost straight ahead
    env_cfg.cruise_altitude = 1.0        # 1 m nav height

    # Single dummy obstacle far outside the arena — keeps tensor shapes valid
    # but has zero effect on the flight
    env_cfg.obstacle_positions = [(50.0, 50.0, 0.5, 5.0)]
    ...
    # Dummy actions: zeros map to yaw_offset=0, speed_factor=mid-range, alt_offset=0
    dummy_actions = np.zeros((1, 3), dtype=np.float32)

    episode_count = 0
    step_count = 0
    playback_dt = env_cfg.sim.dt * env_cfg.decimation * 3.0  # 3× slower for visibility

    print(f"\n{'='*60}")
    print(f"[DEBUG] Dummy HL — frozen LL stack verification")
    print(f"[DEBUG] Single waypoint 2-4 m ahead at 1 m altitude")
    print(f"[DEBUG] No trained HL policy — actions are all zeros")
    ...
    try:
        while simulation_app.is_running():
            ...
            if step_count % 10 == 0:
                ...
                phase = unwrapped._phase[0].item()
                goal = unwrapped._goal_pos_w[0].cpu().numpy()
                phase_names = ["TAKEOFF", "STABILIZE", "NAVIGATE", "HOVER", "LAND"]
                dist = np.linalg.norm(goal[:2] - pos[:2])
                print(f"  Step {step_count:4d} | {phase_names[phase]:10s} | "
                      f"Pos ({pos[0]:5.2f},{pos[1]:5.2f},{pos[2]:5.2f}) | "
                      f"Goal ({goal[0]:5.2f},{goal[1]:5.2f},{goal[2]:5.2f}) | "
                      f"Dist {dist:.2f} m | "
                      f"Vel ({vel[0]:5.2f},{vel[1]:5.2f},{vel[2]:5.2f}) | "
                      f"R={rewards[0].item():.3f}")

            if dones.any():
                episode_count += 1
                print(f"\n  === Episode {episode_count} done ===\n")
