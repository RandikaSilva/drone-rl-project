"""UNSEEN ENV #3 — "NZ Fiordland Gorge Corridor" — play/eval ONLY for v10 checkpoint.

Purpose: test whether the v10 policy can navigate a gorge/valley corridor with
bush-covered cliff walls on either side — a scenario representative of NZ's
Fiordland and West Coast river valleys, where DOC-permitted conservation UAVs
operate for wildlife monitoring and pest control in inaccessible terrain.

The layout simulates a ~10m-wide gorge floor running west→east, walled in by
irregular podocarp tree lines (jittered y-coords, varied trunk radii), with
two emergent canopy rows further up the "slope" at y≈±7.5, fallen logs and
river-boulders scattered on the gorge floor (flyable-over at cruise alt), and
a couple of pinch-points where trees protrude into the corridor.

NZ relevance
------------
  * Fiordland / West Coast river gorges are the canonical terrain for NZ
    helicopter-based conservation ops (whio monitoring, stoat-trap deployment).
  * Department of Conservation permits UAV operations in such terrain
    (doc.govt.nz drone-use-on-conservation-land).
  * Under-canopy gorge navigation requires visual-inertial SLAM and dense
    obstacle avoidance — Tordesillas et al. (2024) arXiv:2403.09596.
  * Salmond et al. (2022), "Autonomous Surveying of Plantation Forests Using
    Multi-Rotor UAVs" (MDPI Drones 6(9):256) — under-canopy tree avoidance.

Environment details
-------------------
  Name             : NZ Fiordland Gorge Corridor
  Style            : Bush-walled river gorge (two cliff tree-walls + floor hazards)
  Total obstacles  : 60
    Cliff walls (24)    : 12 per side, y ≈ ±5 (jittered), radius 0.40-0.55, h 5-6m
    Shoulder emergents(12): 6 per side, y ≈ ±7.5, big trunks r=0.60-0.75, h 6.5-7m
    Gorge pinch-points(4): trees protruding into corridor at x=0, x=+7
    Floor hazards (10)  : fallen logs & boulders (bushes), flyable at cruise 1.5m
    Valley-rim scrub(6) : low bushes (y=±10), flyable
    Corner anchors (4)  : arena boundary
  Arena            : x in [-11, 11], y in [-11, 11]  (22m x 22m)
...
Usage (from IsaacLab directory):
    ...
    # Play (visual, 1 env):
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/v10_unseen3_s_corridor.py \
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    # ── UNSEEN env #3: Fiordland Gorge Corridor — 60 obstacles ───────────────
    # Two dense, naturally-jittered tree walls form a river valley (gorge) running
    # west→east. Drone must traverse the gorge floor (~10m wide corridor between
    # y≈-5 and y≈+5) while avoiding fallen logs and river-boulders on the floor.
    # Inspired by NZ Fiordland/West Coast river gorges where conservation UAVs
    # are deployed (DOC drone permits programme; Salmond et al. 2022).
    obstacle_positions: list = [
        # ── North-wall low cliff trees (12) — irregular y≈+5 ──
        ( -9.00,  5.00, 0.50, 5.5),
        ( -7.40,  5.30, 0.45, 5.0),
        ( -5.70,  4.80, 0.55, 5.5),
        ( -4.10,  5.50, 0.40, 5.0),
        ...
        (  7.40,  4.80, 0.40, 5.5),
        (  9.00,  5.10, 0.50, 5.0),
        # ── South-wall low cliff trees (12) — mirrored y≈-5 ──
        ( -9.00, -5.10, 0.50, 5.5),
        ( -7.40, -4.80, 0.45, 5.0),
        ( -5.70, -5.30, 0.55, 5.5),
        ( -4.10, -4.90, 0.40, 5.0),
        ...
        (  7.40, -5.30, 0.40, 5.5),
        (  9.00, -5.00, 0.50, 5.0),
        # ── North-shoulder emergent trees (6) — tall, deeper into slope y≈+7.5 ──
        ( -8.00,  7.50, 0.75, 7.0),
        ( -4.50,  7.80, 0.65, 6.5),
        ( -1.00,  7.20, 0.70, 7.0),
        (  2.50,  7.70, 0.65, 6.5),
        (  6.00,  7.40, 0.75, 7.0),
        (  8.50,  7.90, 0.60, 6.5),
        # ── South-shoulder emergent trees (6) — mirrored y≈-7.5 ──
        ( -8.00, -7.50, 0.75, 7.0),
        ( -4.50, -7.80, 0.65, 6.5),
        ( -1.00, -7.20, 0.70, 7.0),
        (  2.50, -7.70, 0.65, 6.5),
        (  6.00, -7.40, 0.75, 7.0),
        (  8.50, -7.90, 0.60, 6.5),
        # ── Gorge pinch-points (4) — trees protruding into corridor ──
        (  0.00,  3.00, 0.45, 5.0),   # mid pinch, north side
        (  0.00, -3.00, 0.45, 5.0),   # mid pinch, south side
        (  7.00,  2.50, 0.45, 5.0),   # east pinch, north
        (  7.00, -2.50, 0.45, 5.0),   # east pinch, south
        # ── Gorge-floor fallen logs & boulders (10) — low flyable obstacles ──
        ( -8.00,  1.00, 0.60, 0.8),
        ( -6.50, -1.50, 0.50, 0.7),
        ( -5.00,  0.50, 0.70, 1.0),
        ( -3.00, -1.00, 0.55, 0.8),
        ...
        (  5.50,  1.20, 0.50, 0.8),
        (  8.00, -0.50, 0.55, 0.9),
        # ── Arena corner anchors (4) ──
        (-10.5, -10.5, 0.50, 5.0),
        (-10.5,  10.5, 0.50, 5.0),
        ( 10.5, -10.5, 0.50, 5.0),
        ( 10.5,  10.5, 0.50, 5.0),
        # ── Valley-rim scrub (6 bushes, flyable) — outside the gorge ──
        ( -9.00,  9.50, 0.70, 1.2),
        ( -3.00, 10.00, 0.80, 1.0),
        (  4.00,  9.80, 0.75, 1.1),
        ( -9.00, -9.50, 0.70, 1.2),
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[UNSEEN ENV #3] NAME        : NZ Fiordland Gorge Corridor")
    print(f"[UNSEEN ENV #3] STYLE       : bush-walled river gorge, ~10m corridor + floor logs/boulders")
    print(f"[UNSEEN ENV #3] OBSTACLES   : {len(env_cfg.obstacle_positions)}  (24 cliff + 12 shoulder + 4 pinch + 10 floor + 6 rim + 4 corners)")
    print(f"[UNSEEN ENV #3] ARENA       : x in [-11,11], y in [-11,11]  |  START (-10, 0, 0.2)")
...
def evaluate():
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION -- UNSEEN ENV #3: NZ Fiordland Gorge Corridor")
    print(f"  Style    : bush-walled river gorge, ~10m-wide floor + log/boulder hazards")
    print(f"  Obstacles: {len(env_cfg.obstacle_positions)}  (24 cliff + 12 shoulder + 4 pinch + 10 floor + 6 rim + 4 corners)")
