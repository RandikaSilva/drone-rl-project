...
VERSION 2 — fixes global _hl_step_counter bug (see FINDINGS.md).

Two-level control architecture:
    HIGH-LEVEL (trained):  LiDAR 1347 rays (3 vert ch × 449 horiz) + state → goal modifier
...
Usage (from IsaacLab directory):
    ...
    # Train high-level:
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/hierarchical_obstacle_sac.py \
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    pid_att_kd: float = 1.0   # keep at training value — frozen LL was trained with this
    pid_vz_kp: float = 0.5
    pid_yaw_kp: float = 0.4
    pid_max_tilt: float = 0.5   # keep at training value — frozen LL was trained with this
    ...
    # Tightened ranges to keep frozen LL closer to its training distribution
    max_yaw_offset: float = 0.5    # radians (~28°) — enough lateral authority to clear 0.5m obstacles
    speed_factor_range: tuple = (0.6, 1.0)  # slow down to navigate, LL braking handled by EMA + clamp
    max_altitude_offset: float = 0.8  # meters — allows fly-over of 5m obstacles (cruise 1.5 + 0.8 = 2.3m)
    goal_modifier_smoothing: float = 0.15  # EMA alpha — more smoothing (lower = slower change)
    ...
    nav_xy_progress_scale: float = 5.0       # reduced — survival matters more than speed
    nav_velocity_align_scale: float = 6.0    # reduced
    nav_altitude_scale: float = -1.5
    obstacle_proximity_penalty_scale: float = -25.0   # 3x stronger — must avoid obstacles
    lidar_obstacle_penalty_scale: float = -25.0       # 3x stronger
    lidar_danger_distance: float = 3.5                # wider danger zone (was 2.5)
    subgoal_reachability_scale: float = 5.0           # bigger reward for safe sub-goals
    subgoal_magnitude_penalty: float = -0.3
    crash_penalty: float = -500.0                     # NEW: large negative on collision death
    intermediate_waypoint_bonus: float = 200.0        # bigger carrot for reaching waypoints
    speed_carry_bonus_scale: float = 5.0
    waypoint_progress_scale: float = 3.0
    nav_max_speed: float = 1.5                        # reduced — safer near obstacles
    nav_speed_penalty_scale: float = -3.0             # stronger speed cap
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Low-level velocity commands (direct, no smoothing — must match training pipeline)
        ...
        # Verify every parameter is truly frozen — crash loud if any slipped through
        ...
        training_mode = self._ll_actor.training  # should be False after .eval()
        print(f"[INFO] Low-level policy FROZEN: {total_params} params, "
              f"training={training_mode}, unfrozen_count={len(unfrozen)}")
        assert not training_mode, "LL actor must be in eval() mode"
    ...
    def _build_low_level_obs(self, use_modifier_mask: torch.Tensor) -> torch.Tensor:
        ...
        For NAVIGATE envs: goal_pos_b = real goal rotated by yaw_offset, scaled by speed_factor, with alt_offset
        For other phases: goal_pos_b = real waypoint in body frame (unmodified)
        """
        root_pos = self._robot.data.root_pos_w
        root_quat = self._robot.data.root_quat_w

        # Real goal in body frame
        ...
        goal_xy = real_goal_b[:, :2]  # (N, 2)
        yaw_off = self._hl_yaw_offset  # (N,)
        ...
        # Clamp goal_pos_b XY magnitude to keep LL within its training distribution.
        # During waypoint advance, the new goal can be 6-8m away — far outside what
        # the LL was trained on (~2-4m). Clamping direction is preserved; only magnitude
        # is limited. Applied to all phases (TAKEOFF, STABILIZE, NAVIGATE, HOVER, LAND).
        MAX_LL_GOAL_XY = 3.0  # m — upper bound of LL's effective training range
        ...
        # This makes the goal appear closer → LL naturally outputs smaller velocities.
        # Much safer than scaling velocity output, because the LL's learned coordination
        # between lateral and vertical channels is preserved.
        ...
        return torch.cat([
            ...
            goal_for_ll,                            # 3
            ll_lidar_norm,                          # 9 (matches training config)
            phase_one_hot,                          # 5
        ], dim=-1)  # Total: 26

    def _run_low_level_policy(self):
        """Query the frozen low-level SAC actor to get velocity commands."""
        is_navigate = (self._phase == self.NAVIGATE)
        ll_obs = self._build_low_level_obs(use_modifier_mask=is_navigate)

        # DEBUG: store last LL observation for diagnostics
        self._last_ll_obs = ll_obs

        with torch.no_grad():
            # SB3 SAC actor: deterministic=True returns mean action (no noise)
            ll_actions = self._ll_actor(ll_obs, deterministic=True)
            # Clamp to valid range
            ll_actions = ll_actions.clamp(-1.0, 1.0)

        # CRITICAL: Apply velocity commands EXACTLY as in multi_waypoint_sac training.
        # No EMA smoothing, no speed_factor scaling, no nav speed clamping.
        # The LL learned coordinated 4-channel output (lateral + vertical + yaw).
        # Any asymmetric post-processing (e.g., smoothing vx/vy but not vz) breaks
        # this coordination and causes rolling crashes.
        # Speed control is handled via goal distance scaling in _build_low_level_obs.
    ...
    def _run_pid(self):
        """PID controller: velocity commands → thrust/torques (runs every sim step)."""
        ...
        # Match native multi_waypoint_sac PID exactly — no tilt compensation.
        # The LL was trained with this formula and naturally compensates for tilt-induced
        # altitude loss via its vz_cmd output. Adding tilt compensation here creates a
        # training/inference mismatch that causes altitude overshoots during NAVIGATE.
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v0",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac", "crazyflie_hierarchical_obstacle_sac")
