...
VERSION 8 — Tree trunk takeoff & landing over v7 (94% success).

v7 baseline: 94% success with v6 checkpoint + hardcoded safety layer.

Key changes from v7:
    1. TAKEOFF from tree trunk — drone spawns 20cm from a random interior tree at 1.0m height,
       then moves away and ascends to cruise altitude
    2. LAND on tree trunk — instead of descending to ground, drone approaches the nearest
       interior tree to the final waypoint and holds 20cm from trunk surface at cruise altitude
       (simulating arm-grab behavior)
    3. Rewards redesigned for trunk approach (not ground descent)
    4. Collision disabled for target trunk during LAND phase
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Obstacle Nav — SAC v9")
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    episode_length_s = 50.0  # v9: longer episodes (was 40) — trunk hold + safer paths need more time
    ...
    trunk_approach_tolerance: float = 0.3 # tolerance for trunk landing success
    trunk_land_speed_threshold: float = 0.5  # max speed for trunk landing success
    # Interior trees suitable for takeoff/landing (indices 10-18, skip wall trees 0-9)
    interior_tree_indices: list = [10, 11, 12, 13, 14, 15, 16, 17, 18]

    # v9: Trunk approach rewards (replace ground descent rewards)
    trunk_approach_progress_scale: float = 10.0   # progress toward trunk point
    trunk_xy_precision_scale: float = 200.0       # precision near trunk
    trunk_altitude_hold_scale: float = -3.0       # penalize altitude deviation at trunk
    trunk_stability_scale: float = 20.0           # reward low speed near trunk
    trunk_reached_bonus: float = 1000.0           # bonus for reaching trunk position
    ...
    obstacle_safe_distance: float = 2.7  # v7: keep v6 value — lateral avoidance handles crashes without relearning
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # v9: Tree trunk takeoff & landing buffers
        ...
        self._land_trunk_pos_w = torch.zeros(self.num_envs, 3, device=self.device)  # 20cm from trunk surface
        ...
        print("[INFO] v9: v7 + tree trunk takeoff & landing. Low-level: deterministic proportional controller (no checkpoint)")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        reward = (
            ...
            + land_descent_control + land_controlled_descent + land_altitude_penalty
            + trunk_approach_progress + trunk_precision + trunk_altitude_hold
            + trunk_stability + trunk_reached_bonus + goal_bonus + crash_penalty
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Obstacle collision (v9: exclude start trunk during TAKEOFF/STABILIZE, landing trunk during LAND)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        num_reset = len(env_ids)
        interior_idx = torch.tensor(self.cfg.interior_tree_indices, device=self.device, dtype=torch.long)
        num_interior = len(interior_idx)

        # v9: Pick random interior tree for TAKEOFF start position
        random_tree_sel = torch.randint(0, num_interior, (num_reset,), device=self.device)
        start_tree_idx = interior_idx[random_tree_sel]
        ...
        # v9: Find nearest interior tree to final waypoint for LAND target
        final_wp_xy = self._final_goal_pos_w[env_ids, :2]  # (num_reset, 2)
        interior_obs_xy = self._obstacle_pos_w[env_ids][:, interior_idx, :]  # (num_reset, num_interior, 2)
        interior_radii = self._obstacle_radii[interior_idx]  # (num_interior,)
        delta_to_trees = final_wp_xy.unsqueeze(1) - interior_obs_xy  # (num_reset, num_interior, 2)
        dist_to_trees = torch.linalg.norm(delta_to_trees, dim=2)
        surface_dist_to_trees = dist_to_trees - interior_radii.unsqueeze(0)
        _, nearest_interior_sel = surface_dist_to_trees.min(dim=1)
        land_tree_idx = interior_idx[nearest_interior_sel]
        self._land_trunk_idx[env_ids] = land_tree_idx

        # Compute landing position: 20cm from nearest trunk surface toward the final waypoint
        land_tree_xy = self._obstacle_pos_w[env_ids][
            torch.arange(num_reset, device=self.device), land_tree_idx
        ]  # (num_reset, 2)
        land_tree_radii = self._obstacle_radii[land_tree_idx]  # (num_reset,)
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v9",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_trunk_sac_v2", "crazyflie_hierarchical_obstacle_trunk_sac_v2")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v9", cfg=env_cfg)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v9: v7 + tree trunk takeoff & landing (19 trees) + deterministic proportional LL controller")
    if agent:
