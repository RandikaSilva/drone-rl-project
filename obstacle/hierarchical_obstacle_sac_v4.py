...
VERSION 4 — Rich environment with dense obstacle field (19 trees).

Based on v3 deterministic proportional LL controller (no checkpoint needed).
Adds 5 extra interior trees with varied sizes for a denser, more challenging
navigation environment that requires real obstacle avoidance skill.
...
Two-level control architecture:
    ...
    PID        (fixed):    Velocity commands → thrust/torques (with hard pitch/roll limits)

The low-level is a deterministic proportional controller — no checkpoint needed.
...
Usage (from IsaacLab directory):
    ...
    # Train high-level:
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/hierarchical_obstacle_sac_v4.py \
...
# v4: No LL checkpoint needed — deterministic proportional controller
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v4: Rich env with 19 trees. High-level at 10 Hz, low-level at 50 Hz."""
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print("[INFO] v4: 19 trees (dense interior). Low-level: deterministic proportional controller (no checkpoint)")
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v4",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v4", "crazyflie_hierarchical_obstacle_sac_v4")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v4", cfg=env_cfg)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v4: Rich env (19 trees) + deterministic proportional LL controller")
    if agent:
