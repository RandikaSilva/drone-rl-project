"""UNSEEN ENV #2 — "NZ Native Podocarp-Broadleaf Bush" — play/eval ONLY for v10 checkpoint.

Purpose: test whether the v10 policy generalises to the UNSTRUCTURED, species-
rich stem arrangement characteristic of NZ native podocarp-broadleaf forest
(beech, rimu, miro, matai, kahikatea). Unlike the regular plantation of env1,
native bush has stems placed by natural regeneration — chaotic, mixed radii,
mixed canopy heights. Density here (60 trees in 484 m² ≈ 1,240 stems/ha) is
typical of intact mature podocarp-broadleaf stands recorded in NZ forest plot
data (Manaaki Whenua Landcare Research, Kāmahi-podocarp forest alliance).

NZ relevance
------------
  * Podocarp-broadleaf is the dominant native forest type on NZ's North Island
    and western South Island (Manaaki Whenua Landcare Research).
  * Species-rich: ~40 species per 400m² plot, uneven canopy 20-40m tall.
  * Under-canopy drone research directly addresses this biome:
    Tordesillas et al. (2024) "Scalable Autonomous Drone Flight in the Forest
    with Visual-Inertial SLAM" (arXiv 2403.09596) and Kuronen et al. (2025)
    "Towards autonomous photogrammetric forest inventory using a lightweight
    under-canopy robotic drone" (arXiv 2501.12073).
  * DOC permits drone use in native bush for pest monitoring and wildlife
    surveying (doc.govt.nz permits framework).

Environment details
-------------------
  Name             : NZ Native Podocarp-Broadleaf Bush
  Style            : Unstructured dense native forest, mixed stem sizes
  Total obstacles  : 60
    Trees (60)     : seeded random placement (seed=2026), min 1.7m separation
                     radius sampled U(0.30, 0.65) — mixed canopy/sub-canopy stems
                     height U(4.0, 6.5) — uneven canopy typical of podocarp bush
                     clearings kept around start (r >= 3m) and final-WP zone (x>7, |y|<1.5)
    Bushes (0)      (understory ferns omitted — see env5 Fern Gully for dense undergrowth)
  Arena            : x in [-11, 11], y in [-11, 11]  (22m x 22m ≈ 0.048 ha)
  Effective stocking: ~1,240 stems/ha — mature podocarp-broadleaf density
  Start position   : (-10, 0, 0.2)
  Waypoint range   : x in [-8, 8], y in [-6, 6]

NO training in this file — the v10 checkpoint is reused verbatim.
...
Usage (from IsaacLab directory):
    ...
    # Play (visual, 1 env):
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/v10_unseen2_random_scatter.py \
        --mode play --checkpoint /path/to/hl_sac_final.zip

    # Eval (headless, 50 episodes):
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/obstacle/v10_unseen2_random_scatter.py \
        --mode eval --checkpoint /path/to/hl_sac_final.zip --num_episodes 50 --headless
"""
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Obstacle Nav — SAC v10 (Dense Forest)")
parser.add_argument("--mode", type=str, default="eval", choices=["play", "eval"])
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v10: Dense forest with varied trees + bushes. Built on v9 (99% success)."""
    ...
    # ── UNSEEN env #2: Dense Random Scatter — 60 trees, seed=2026 ────────────
    # Format: (x, y, radius, height). Generated with numpy rng(seed=2026),
    # min 1.7m separation, start clearing 3m, final-WP clearing (x>7, |y|<1.5).
    obstacle_positions: list = [
        ( -6.10,  2.66, 0.464, 4.93),
        ( -2.76,  5.52, 0.617, 4.44),
        (  2.90, -3.83, 0.638, 6.30),
        (  2.58,  4.80, 0.480, 6.06),
        ( -0.98, -3.06, 0.397, 4.57),
        (  0.49, -1.31, 0.532, 4.03),
        ( -5.52,  7.12, 0.579, 5.52),
        ( -2.94,  8.49, 0.497, 5.08),
        (  7.61, -3.43, 0.544, 4.78),
        ( -4.53,  3.82, 0.380, 5.23),
        (  1.52, -5.91, 0.556, 5.37),
        ( -1.52, -0.10, 0.464, 5.69),
        ( -9.47,  5.59, 0.482, 4.82),
        ( -0.00, -7.72, 0.617, 6.47),
        ( -8.38, -2.69, 0.556, 4.79),
        (  5.21,  8.71, 0.611, 5.55),
        ( -0.16,  1.93, 0.497, 6.23),
        (  7.95, -5.92, 0.630, 5.97),
        (  2.64,  3.03, 0.491, 6.29),
        (  5.97, -6.94, 0.515, 5.01),
        (  2.03,  6.78, 0.520, 4.80),
        (  3.20, -7.06, 0.398, 4.08),
        ( -5.10,  0.31, 0.531, 6.20),
        ( -2.75, -3.45, 0.410, 4.29),
        (  8.71,  3.78, 0.561, 6.22),
        (  4.13,  6.50, 0.420, 5.72),
        ( -7.58, -4.42, 0.620, 4.80),
        (  5.51, -1.46, 0.364, 5.90),
        ( -2.56, -8.68, 0.638, 4.08),
        ( -5.55, -3.13, 0.499, 6.39),
        (  8.29, -8.10, 0.599, 4.42),
        (  5.92, -3.87, 0.551, 5.80),
        (  2.70, -0.03, 0.617, 4.44),
        (  5.21,  2.89, 0.372, 5.86),
        (  5.16,  5.13, 0.327, 4.53),
        (  7.51,  7.21, 0.372, 5.38),
        ( -4.51, -1.59, 0.307, 4.70),
        (  1.42,  9.04, 0.494, 6.22),
        ( -9.21, -5.64, 0.603, 5.42),
        ( -6.87, -9.03, 0.473, 4.19),
        ( -7.41,  7.90, 0.518, 5.85),
        ( -9.31, -7.44, 0.416, 5.03),
        (  0.42,  5.21, 0.446, 4.88),
        ( -7.37, -7.09, 0.479, 5.77),
        (  0.23,  7.32, 0.600, 4.48),
        ( -1.92,  3.47, 0.549, 5.00),
        (  4.58,  0.55, 0.485, 4.50),
        ( -6.45,  5.38, 0.352, 5.82),
        ( -9.04,  3.05, 0.403, 5.17),
        ( -8.08,  4.56, 0.375, 6.17),
        ( -3.98, -5.09, 0.416, 4.15),
        (  4.85, -5.33, 0.513, 5.50),
        ( -3.21, -7.06, 0.613, 5.83),
        ( -5.27, -7.69, 0.595, 5.42),
        (  8.10,  1.65, 0.423, 5.69),
        (  8.45,  9.08, 0.399, 6.34),
        (  3.27,  8.24, 0.551, 6.39),
        (  3.40, -8.96, 0.357, 5.54),
        (  0.63, -3.77, 0.647, 4.06),
        (  9.50,  5.94, 0.572, 5.43),
    ]
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print("[INFO] v10: dense forest (35 obstacles, varied trunks) + stagnation penalty + v9 smoothing + v8 velocity-projection safety. Low-level: deterministic proportional controller (no checkpoint)")
        dr_features = []
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v10",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v10", "crazyflie_hierarchical_obstacle_sac_v10")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v10", cfg=env_cfg)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[UNSEEN ENV #2] NAME        : NZ Native Podocarp-Broadleaf Bush")
    print(f"[UNSEEN ENV #2] STYLE       : unstructured native forest (species-rich, mixed stems)")
    print(f"[UNSEEN ENV #2] OBSTACLES   : {len(env_cfg.obstacle_positions)}  (~1,240 stems/ha, seed=2026, min 1.7m sep)")
    print(f"[UNSEEN ENV #2] ARENA       : x in [-11,11], y in [-11,11]  |  START (-10, 0, 0.2)")
    print(f"[INFO] v10 checkpoint + velocity-projection safety + deterministic proportional LL controller")
    if agent:
...
def evaluate():
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION -- UNSEEN ENV #2: NZ Native Podocarp-Broadleaf Bush")
    print(f"  Style    : unstructured native forest (~1,240 stems/ha)")
    print(f"  Obstacles: {len(env_cfg.obstacle_positions)}  (all trees, seed=2026)")
    print(f"  Episodes : {args_cli.num_episodes}  |  Num envs: {args_cli.num_envs}")
