"""Hierarchical Crazyflie obstacle avoidance — Dense Forest Return-to-Start with SAC.
VERSION 11 — Dense forest, drone spawns at center, must return to start.

v10 baseline: Dense forest with 35 obstacles, varied trees/bushes.
v11 changes:
    - Drone spawns at CENTER of a very dense forest (~50 obstacles)
    - Trees arranged in concentric rings around spawn — drone is "trapped"
    - Mission: navigate outward through forest corridors to waypoints, then RETURN to start
    - Final waypoint is ALWAYS the starting position (return-to-start)
    - Path feasibility detection: if all LiDAR sectors are blocked and drone is
      stagnating, skip remaining waypoints and fly home immediately
    - New "return mode" observation bit so the agent knows when it's heading home
    - Return-to-start bonus reward for successfully getting back
...
parser.add_argument("--max_waypoints", type=int, default=4)
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v11: Dense forest return-to-start. Drone spawns at center, navigates out, returns home."""

    episode_length_s = 240.0  # v11: longer — drone must navigate out AND back
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(
        num_envs=256,
        env_spacing=80.0,  # v11: larger spacing for bigger forest
    ...
    # Steady-state drag tilt
    drag_tilt_coeff: float = 0.06

    # Movement noise — Ornstein-Uhlenbeck random walk
    ...
    # ── Multi-Waypoint + Return-to-Start (v11) ──────────────────────────────
    # Drone spawns at center of forest
    static_start_pos: tuple = (0.0, 0.0, 0.2)
    min_num_waypoints: int = 2
    max_num_waypoints: int = 4   # v11: includes return waypoint (always appended)
    # v11: waypoints are generated in a ring around center, then return WP added
    waypoint_min_radius: float = 5.0   # min distance from center for intermediate WPs
    waypoint_max_radius: float = 10.0  # max distance from center for intermediate WPs
    arena_x_range: tuple = (-12.0, 12.0)
    arena_y_range: tuple = (-12.0, 12.0)
    min_y_change: float = 2.0
    intermediate_waypoint_tolerance: float = 6.0
    final_waypoint_tolerance: float = 5.0
    waypoint_obstacle_clearance: float = 3.0

    # ── Path Blocked Detection (v11) ─────────────────────────────────────────
    # Divide LiDAR into 8 sectors of 45°. If ALL sectors have min distance
    # below threshold AND drone is stagnating → trigger return-to-start
    num_lidar_sectors: int = 8
    blocked_sector_threshold: float = 3.0  # meters — sector considered blocked if min < this
    blocked_min_sectors: int = 6           # at least this many sectors must be blocked
    blocked_stagnation_timeout: float = 10.0  # seconds of stagnation before triggering return

    # ── Obstacles (v11: ~50 obstacles — dense forest around center) ──────────
    # Format: (x, y, radius, height)
    # Drone spawns at (0, 0) — trees form concentric rings around spawn
    obstacle_positions: list = [
        # ═══ INNER RING (r ≈ 2.5-3.5m) — tight around spawn, forces awareness ═══
        ( 2.5,  1.5, 0.50, 6.0),   # NE inner
        ( 0.5,  3.0, 0.45, 5.5),   # N inner
        (-2.0,  2.0, 0.55, 6.5),   # NW inner
        (-3.0, -0.5, 0.48, 5.5),   # W inner
        (-1.5, -2.5, 0.52, 6.0),   # SW inner
        ( 1.5, -2.8, 0.42, 5.0),   # SE inner (thinner — gap corridor)

        # ═══ MIDDLE RING (r ≈ 5-7m) — creates maze corridors ═══
        ( 5.0,  3.0, 0.60, 7.0),   # NE mid
        ( 2.0,  5.5, 0.55, 6.5),   # N mid
        (-2.5,  5.0, 0.50, 6.0),   # NW mid
        (-5.5,  2.0, 0.65, 7.0),   # W mid (thick — hard to pass)
        (-5.0, -2.5, 0.48, 5.5),   # SW mid
        (-2.0, -5.0, 0.55, 6.5),   # S mid
        ( 2.5, -5.5, 0.45, 5.5),   # SE mid
        ( 5.5, -1.5, 0.58, 6.5),   # E mid

        # ═══ OUTER RING (r ≈ 8-10m) — forest edge ═══
        ( 8.0,  4.0, 0.55, 7.0),   # NE outer
        ( 4.0,  8.5, 0.50, 6.5),   # N outer
        (-4.0,  8.0, 0.60, 7.0),   # NW outer
        (-8.5,  3.0, 0.52, 6.0),   # W outer
        (-8.0, -3.5, 0.58, 6.5),   # SW outer
        (-3.5, -8.0, 0.45, 5.5),   # S outer
        ( 3.5, -8.5, 0.55, 6.0),   # SE outer
        ( 8.5, -2.0, 0.50, 6.5),   # E outer

        # ═══ GAP FILLERS — block easy straight-line escapes ═══
        ( 3.8,  0.5, 0.40, 5.0),   # blocks direct E path
        ( 0.5,  4.5, 0.42, 5.5),   # blocks direct N path
        (-4.2,  0.0, 0.38, 5.0),   # partially blocks W (thin — tight squeeze)
        ( 0.3, -4.0, 0.45, 5.5),   # blocks direct S path
        ( 4.0,  6.0, 0.48, 6.0),   # NE gap filler
        (-6.0, -5.0, 0.52, 6.5),   # SW gap filler
        ( 6.5,  1.0, 0.44, 5.5),   # E gap filler
        (-1.0, -7.0, 0.40, 5.0),   # S gap filler

        # ═══ EXTRA CORRIDOR TREES — narrows passages between rings ═══
        ( 1.0,  2.0, 0.35, 4.5),   # NE inner corridor
        (-1.0,  3.5, 0.38, 5.0),   # N inner-mid gap
        (-3.5, -3.5, 0.40, 5.5),   # SW inner-mid gap
        ( 3.5, -3.5, 0.36, 4.5),   # SE inner-mid gap
        ( 7.0,  6.5, 0.50, 6.0),   # NE mid-outer gap
        (-7.0,  6.0, 0.48, 5.5),   # NW mid-outer gap
        (-7.0, -6.5, 0.52, 6.0),   # SW mid-outer gap
        ( 7.0, -5.5, 0.46, 5.5),   # SE mid-outer gap

        # ═══ BOUNDARY TREES (r ≈ 12-14m) — arena perimeter ═══
        (12.0,  0.0, 0.55, 6.0),
        ( 0.0, 12.0, 0.60, 6.5),
        (-12.0,  0.0, 0.50, 5.5),
        ( 0.0,-12.0, 0.55, 6.0),
        ...
        # ═══ BUSHES (short wide cylinders — drone can fly over) ═══
        ( 1.0,  0.8, 0.80, 1.0),   # near center
        (-0.8, -1.0, 0.70, 0.9),   # near center
        ( 4.0, -3.0, 0.85, 1.1),   # SE
        (-3.0,  4.0, 0.75, 1.0),   # NW
        ( 6.0,  5.0, 0.90, 1.2),   # NE outer
        (-6.0, -6.0, 0.65, 0.8),   # SW outer
        ( 0.0,  7.0, 0.80, 1.0),   # N
        (-7.0,  0.0, 0.70, 0.9),   # W
    ]
    bush_height_threshold: float = 1.5
    obstacle_collision_radius: float = 0.5
    obstacle_wall_radius: float = 0.65
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    env.step() runs at 50 Hz (decimation=2).
    High-level goal modifier updates every 5 env.steps (10 Hz).
    Low-level deterministic controller runs every env.step (50 Hz).
    """
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        max_wps = self.cfg.max_num_waypoints + 1  # v11: +1 for return-to-start waypoint

        # ── Buffers ──────────────────────────────────────────────────────────
        self._thrust = torch.zeros(self.num_envs, 1, 3, device=self.device)
        self._moment = torch.zeros(self.num_envs, 1, 3, device=self.device)

        # Multi-waypoint storage (v11: +1 slot for return WP)
        ...
        # v11: Path blocked detection
        self._blocked_timer = torch.zeros(self.num_envs, device=self.device)  # time continuously blocked
        ...
        print(f"[INFO] v11: dense forest return-to-start ({len(self.cfg.obstacle_positions)} obstacles). "
              f"Drone spawns at center. Low-level: deterministic proportional controller")
        dr_features = []
        ...
        print(f"[INFO]   pid_max_tilt={self.cfg.pid_max_tilt:.2f} rad ({self.cfg.pid_max_tilt * 180 / 3.14159:.1f}deg)")
    ...
    def _setup_scene(self):
        ...
        # Spawn tree obstacles with varied colors
        trunk_colors = [
            (0.36, 0.25, 0.13), (0.30, 0.20, 0.10), (0.42, 0.30, 0.15),
            (0.35, 0.22, 0.08), (0.38, 0.28, 0.16),
        ]
        canopy_colors = [
            (0.15, 0.40, 0.12), (0.10, 0.35, 0.08), (0.20, 0.45, 0.15),
            (0.12, 0.30, 0.10), (0.18, 0.42, 0.18),
        ]
        bush_colors = [
            (0.22, 0.45, 0.15), (0.18, 0.38, 0.12),
            (0.28, 0.50, 0.18), (0.20, 0.42, 0.10),
        ]
        ...
        self.sim.set_camera_view(eye=[0.0, -30.0, 15.0], target=[0.0, 0.0, 2.0])

    # ── Waypoint Generation (v11: radial + return-to-start) ─────────────────

    def _generate_waypoints(self, env_ids: torch.Tensor):
        """Generate waypoints in a ring around center, then append return-to-start WP.

        v11: Intermediate waypoints are placed at random angles around the spawn center
        at distances between waypoint_min_radius and waypoint_max_radius. The final
        waypoint is always the starting position (return home).
        """
        num_reset = len(env_ids)
        device = self.device
        max_wps = self.cfg.max_num_waypoints + 1  # +1 for return WP

        # Number of intermediate waypoints (before return WP)
        ...
        # Generate intermediate waypoints in a ring around center
        r_min = self.cfg.waypoint_min_radius
        r_max = self.cfg.waypoint_max_radius

        waypoints_local = torch.zeros(num_reset, max_wps, 3, device=device)

        for wi in range(self.cfg.max_num_waypoints):
            # Random angle, random radius
            angle = torch.rand(num_reset, device=device) * 2 * math.pi
            # Spread angles: offset each WP by roughly 360/max_wps degrees
            angle = angle + wi * (2 * math.pi / self.cfg.max_num_waypoints)
            radius = r_min + torch.rand(num_reset, device=device) * (r_max - r_min)

            wp_x = radius * torch.cos(angle)
            wp_y = radius * torch.sin(angle)

            # Clamp to arena
            x_min, x_max = self.cfg.arena_x_range
            y_min, y_max = self.cfg.arena_y_range
            wp_x = wp_x.clamp(x_min, x_max)
            wp_y = wp_y.clamp(y_min, y_max)

            waypoints_local[:, wi, 0] = wp_x
            waypoints_local[:, wi, 1] = wp_y
            waypoints_local[:, wi, 2] = self.cfg.cruise_altitude

        # Obstacle clearance nudging for intermediate waypoints
        obs_xy = self._obstacle_local
        obs_r = self._obstacle_radii
        clearance = self.cfg.waypoint_obstacle_clearance

        for wi in range(self.cfg.max_num_waypoints):
            wp_xy = waypoints_local[:, wi, :2]
            ...
            if too_close.any():
                ...
                waypoints_local[too_close, wi, 0] = (waypoints_local[too_close, wi, 0] + nudge[:, 0]).clamp(x_min, x_max)
                waypoints_local[too_close, wi, 1] = (waypoints_local[too_close, wi, 1] + nudge[:, 1]).clamp(y_min, y_max)

        # v11: Append return-to-start as FINAL waypoint
        # The return WP is at slot [num_intermediate] for each env
        # Fill all remaining slots with start position too
        start_local = torch.tensor(self.cfg.static_start_pos, dtype=torch.float32, device=device)
        for wi in range(max_wps):
            mask = wi >= num_intermediate
            if mask.any():
                waypoints_local[mask, wi, 0] = start_local[0]
                waypoints_local[mask, wi, 1] = start_local[1]
                waypoints_local[mask, wi, 2] = self.cfg.cruise_altitude

        # Total waypoints = intermediate + 1 (return)
        total_wps = num_intermediate + 1
        self._num_waypoints[env_ids] = total_wps
        ...
        # Final goal = start position (for landing)
        batch_idx = torch.arange(num_reset, device=device)
        final_wp_idx = (total_wps - 1).long()
        self._final_goal_pos_w[env_ids] = waypoints_w[batch_idx, final_wp_idx].clone()
        self._final_goal_pos_w[env_ids, 2] = 0.2  # landing altitude
    ...
    def _check_waypoint_advancement(self, xy_dist: torch.Tensor, speed: torch.Tensor) -> torch.Tensor:
        ...
        # Auto-advance stuck intermediate waypoints
    ...
    def _check_path_blocked(self):
        """Check if LiDAR sectors are blocked, triggering goal change to start position.

        Divides horizontal LiDAR rays into sectors. If most sectors have
        min distance below threshold and drone is stagnating, skip remaining
        waypoints and set goal = start position. The existing v10 rewards
        (progress, velocity alignment, etc.) naturally guide the drone back.
        """
        if self._hl_lidar_horiz_indices is None:
            return

        is_navigate = self._phase == self.NAVIGATE
        # Only trigger for drones not already heading to the return WP (last WP = start)
        is_intermediate = self._current_wp_idx < (self._num_waypoints - 1)
        can_trigger = is_navigate & is_intermediate

        if not can_trigger.any():
            return

        # Get horizontal LiDAR distances
        horiz_idx = self._hl_lidar_horiz_indices
        if not hasattr(self, '_last_hl_lidar_distances'):
            return
        horiz_dists = self._last_hl_lidar_distances[:, horiz_idx]  # (N, n_horiz)
        n_horiz = horiz_dists.shape[1]
        n_sectors = self.cfg.num_lidar_sectors
        rays_per_sector = n_horiz // n_sectors

        if rays_per_sector == 0:
            return

        # Compute min distance per sector
        sectors_blocked = torch.zeros(self.num_envs, dtype=torch.int32, device=self.device)
        for s in range(n_sectors):
            start_ray = s * rays_per_sector
            end_ray = start_ray + rays_per_sector
            sector_min = horiz_dists[:, start_ray:end_ray].min(dim=1).values
            sectors_blocked += (sector_min < self.cfg.blocked_sector_threshold).int()

        # Path is considered blocked if enough sectors are blocked
        is_blocked = sectors_blocked >= self.cfg.blocked_min_sectors

        # Update blocked timer
        dt = self.step_dt
        self._blocked_timer[is_blocked & can_trigger] += dt
        self._blocked_timer[~is_blocked] = 0.0  # reset if any sector opens

        # Trigger goal-change-to-start when blocked AND stagnating
        time_since_advance = self._episode_time - self._last_wp_advance_time
        stagnating = time_since_advance > self.cfg.blocked_stagnation_timeout
        trigger_return = can_trigger & (self._blocked_timer > 2.0) & stagnating

        if trigger_return.any():
            trigger_ids = torch.where(trigger_return)[0]
            # Skip to return waypoint (last WP = start position)
            return_wp_idx = (self._num_waypoints[trigger_ids] - 1).long()
            self._current_wp_idx[trigger_ids] = return_wp_idx.int()
            self._update_current_goal(trigger_ids)
            # Reset distance tracking for new goal
            new_delta = self._goal_pos_w[trigger_ids] - self._robot.data.root_pos_w[trigger_ids]
            self._prev_xy_dist[trigger_ids] = torch.linalg.norm(new_delta[:, :2], dim=1)
            self._last_wp_advance_time[trigger_ids] = self._episode_time[trigger_ids]
            self._blocked_timer[trigger_ids] = 0.0
    ...
    def _run_low_level_policy(self):
        ...
        # Multi-obstacle velocity-projection safety + predictive avoidance
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # v11: Check if path is blocked → trigger return-to-start
    ...
    def _get_observations(self) -> dict:
        ...
        # Noisy velocity/angular velocity
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        is_at_final_wp_now = (self._phase == self.NAVIGATE) & (self._current_wp_idx >= (self._num_waypoints - 1))
        time_at_final = self._episode_time - self._last_wp_advance_time
        force_hover = is_at_final_wp_now & (time_at_final > self.cfg.final_wp_force_hover_timeout)
        normal_hover = is_at_final_wp_now & (current_xy_dist < self.cfg.final_waypoint_tolerance)
        to_hover = (normal_hover | force_hover) & (speed < 3.0) & (altitude_now > 0.3)
        # v11: When force-hovering far from goal during return, land at current position
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Generate waypoints (with return-to-start WP appended) & set start
        ...
        # v11: Reset blocked detection timer
        self._blocked_timer[env_ids] = 0.0

        # Domain randomization re-sample
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        if debug_vis:
            ...
            # v11: Start/home marker (larger, distinct)
            if not hasattr(self, "home_visualizer"):
                home_marker_cfg = CUBOID_MARKER_CFG.copy()
                home_marker_cfg.markers["cuboid"].size = (0.6, 0.6, 0.6)
                home_marker_cfg.prim_path = "/Visuals/Command/home_position"
                self.home_visualizer = VisualizationMarkers(home_marker_cfg)
            ...
            self.home_visualizer.set_visibility(True)
            ...
            if hasattr(self, "home_visualizer"):
                self.home_visualizer.set_visibility(False)
    ...
    def _debug_vis_callback(self, event):
        ...
        max_wps = self.cfg.max_num_waypoints + 1
        for wp_idx in range(max_wps):
        ...
        # v11: Show home/start position marker
        home_pos = self._start_pos_w.clone()
        home_pos[:, 2] = self.cfg.cruise_altitude  # show at cruise height for visibility
        self.home_visualizer.visualize(home_pos)
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v11",
...
def train():
    ...
    print(f"[INFO] Low-level: deterministic proportional controller")
    print(f"[INFO] Waypoints: {env_cfg.min_num_waypoints}-{env_cfg.max_num_waypoints} + return-to-start")
    print(f"[INFO] Obstacles: {len(env_cfg.obstacle_positions)} (dense forest around center)")
    print(f"[INFO] Mission: navigate outward through forest -> return to start")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v11", cfg=env_cfg)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v11: dense forest return-to-start ({len(env_cfg.obstacle_positions)} obstacles)")
    print(f"[INFO] Mission: start at center -> navigate outward -> return home")
    if agent:
...
def evaluate():
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION: Dense Forest Return-to-Start SAC v11 | {args_cli.num_episodes} episodes")
    print(f"HL Checkpoint: {os.path.basename(args_cli.checkpoint)}")
    print(f"LL: deterministic proportional controller")
    print(f"Mission: center-start -> navigate outward -> return home")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        print(f"\n{'='*60}", flush=True)
        print(f"RESULTS -- Dense Forest Return-to-Start SAC v11 ({n} episodes)", flush=True)
