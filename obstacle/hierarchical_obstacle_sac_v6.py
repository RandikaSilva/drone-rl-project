...
VERSION 6 — Minimal targeted fixes over v5 (74% success).

v5 baseline: 74% success, 18% crash, 68% waypoint completion.
v5 crashes are from tight corridor passages between interior trees.

Key changes from v5 (MINIMAL — only 3 targeted fixes):
    1. Reactive safety layer at 0.6m — last-resort speed reduction near obstacles
    2. Slightly stronger crash penalty (-800 -> -900)
    3. Slightly earlier safe_distance (2.5 -> 2.7m)
...
# ── Argument Parser ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Hierarchical Crazyflie Obstacle Nav — SAC v6")
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    """v6: Minimal targeted fixes over v5. Reactive safety + stronger crash penalty + wider safe dist."""
    ...
    # v6: Reactive safety — last-resort speed reduction very close to obstacles
    reactive_safety_distance: float = 0.6  # very tight — only near-collision
    reactive_safety_min_speed: float = 0.4  # never go below 40% speed
    ...
    obstacle_safe_distance: float = 2.7  # v6: slightly earlier trigger (was 2.5 in v5)
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HierarchicalObstacleNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print("[INFO] v6: v5 + reactive safety + stronger crash penalty. Low-level: deterministic proportional controller (no checkpoint)")
    ...
    def _run_low_level_policy(self):
        ...
        # v6: Reactive safety — last-resort speed reduction very close to obstacles (NAVIGATE only)
        ...
        if nav_mask.any():
            ...
            min_surf, _ = obs_surface.min(dim=1)
            ...
            if apply_mask.any():
                self._ll_vx_cmd[apply_mask] *= safety_scale[apply_mask]
                self._ll_vy_cmd[apply_mask] *= safety_scale[apply_mask]
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v6",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v6", "crazyflie_hierarchical_obstacle_sac_v6")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v6", cfg=env_cfg)
...
def play():
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] v6: v5 + reactive safety (19 trees) + deterministic proportional LL controller")
    if agent:
