...
class MultiWaypointNavEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # TEST 9: Simulate random HL policy — update goal modifiers every 5 steps
        # during NAVIGATE phase (matching v2's 10 Hz HL update rate)
        self._hl_step_counter += 1
        update_mask = (self._hl_step_counter % 5 == 0) & (self._phase == self.NAVIGATE)
        if update_mask.any():
            n_update = update_mask.sum()
            # Random values within v2's ranges: yaw ±0.5, speed 0.6-1.0, alt ±0.8
            self._hl_yaw_offset[update_mask] = (torch.rand(n_update, device=self.device) * 2 - 1) * 0.5
            self._hl_speed_factor[update_mask] = torch.rand(n_update, device=self.device) * 0.4 + 0.6
            self._hl_alt_offset[update_mask] = (torch.rand(n_update, device=self.device) * 2 - 1) * 0.8
    ...
    def _get_observations(self) -> dict:
        ...
        # Step 4: XY magnitude clamping (ALL phases, max 3.0m)
        MAX_LL_GOAL_XY = 3.0
