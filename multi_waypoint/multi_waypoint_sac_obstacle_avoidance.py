...
Built on top of multi_waypoint_sac_debug_test10_stabilized_hl.py (stable LL base).
...
Stabilization fixes from test10 (culprit #3):
    - Tightened goal modifier ranges (yaw +/-0.15, speed 0.85-1.0, alt +/-0.2)
    - EMA smoothing on goal modifiers (alpha=0.3)
    - XY goal clamp at 8.0m (matching LL training distribution)
    - Actor x100 fix for frozen LL (culprit #2)

Decision frequencies:
    High-level:  10 Hz  (one sub-goal per 5 env.steps)
...
Usage (from IsaacLab directory):
    ...
    # Train high-level:
    python ~/Desktop/Lasitha/drone_rl_project/drone-rl-project/multi_waypoint/multi_waypoint_sac_obstacle_avoidance.py \
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    # ── Goal Modifier (test10 stabilized ranges) ────────────────────────────
    max_yaw_offset: float = 0.15       # radians — tightened from 0.5 (test10 fix)
    speed_factor_range: tuple = (0.85, 1.0)  # tightened from (0.6, 1.0) (test10 fix)
    max_altitude_offset: float = 0.2   # meters — tightened from 0.8 (test10 fix)
    goal_modifier_smoothing: float = 0.3  # EMA alpha (test10 fix)
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        HL goal modifier updates every 5 steps (10 Hz).
        LL policy runs every step (50 Hz).
        PID runs here (same pattern as multi_waypoint training).
        """
        # Update high-level goal modifier every 5 steps per env (10 Hz)
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac", "crazyflie_obstacle_avoidance_sac")
...
def play():
    """Play a trained hierarchical SAC policy -- runs continuously until Ctrl+C."""
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"[INFO] Hierarchical SAC policy loaded!", flush=True)
    print(f"[INFO] High-level: 10 Hz sub-goal planner", flush=True)
    print(f"[INFO] Low-level: frozen multi-waypoint SAC (50 Hz)", flush=True)
        ...
        while simulation_app.is_running():
            ...
            if step_count % 50 == 0:
                ...
                print(f"  Step {step_count} | Phase: {phase_names[phase]} | WP: {wp_idx}/{num_wps} | "
                      f"Pos: ({pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.2f}) | Reward: {rewards[0].item():.3f}", flush=True)
