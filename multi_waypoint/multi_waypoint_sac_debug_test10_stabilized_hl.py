...
class MultiWaypointNavEnvCfg(DirectRLEnvCfg):
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=512, env_spacing=70.0, replicate_physics=True)
    ...
    # Second raycaster matching HL lidar from obstacle env (not used for obs, just present in scene)
    ...
    # Obstacle geometry (same as hierarchical_obstacle_sac_v2.py)
...
class MultiWaypointNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MultiWaypointNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # TEST 9: Goal modifier buffers + step counter for random HL simulation
    ...
    def _setup_scene(self):
        ...
        # Second raycaster (HL lidar) — not used for obs, testing if it affects LL
        self._hl_lidar = RayCaster(self.cfg.hl_lidar)
        self.scene.sensors["hl_lidar"] = self._hl_lidar

        # Spawn tree obstacles (same geometry as obstacle env)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # TEST 10: Stabilized random HL — fixes from incremental debug:
        # 1. Tightened modifier ranges (yaw ±0.15, speed 0.85-1.0, alt ±0.2)
        #    Test 9 used full v2 ranges which crashed the LL before HL could learn.
        # 2. EMA smoothing on goal modifiers (alpha=0.3) for gradual transitions
        #    (NOT on velocity commands — that was culprit #1)
        # 3. XY clamp increased to 8.0m (see below) to match training distribution
        self._hl_step_counter += 1
        update_mask = (self._hl_step_counter % 5 == 0) & (self._phase == self.NAVIGATE)
        if update_mask.any():
            n_update = update_mask.sum()
            # Tightened ranges — safe for untrained/early HL policy
            target_yaw = (torch.rand(n_update, device=self.device) * 2 - 1) * 0.15
            target_speed = torch.rand(n_update, device=self.device) * 0.15 + 0.85
            target_alt = (torch.rand(n_update, device=self.device) * 2 - 1) * 0.2
            # EMA smoothing on modifiers — gradual transitions prevent LL goal whiplash
            alpha = 0.3
            self._hl_yaw_offset[update_mask] = (1 - alpha) * self._hl_yaw_offset[update_mask] + alpha * target_yaw
            self._hl_speed_factor[update_mask] = (1 - alpha) * self._hl_speed_factor[update_mask] + alpha * target_speed
            self._hl_alt_offset[update_mask] = (1 - alpha) * self._hl_alt_offset[update_mask] + alpha * target_alt
    ...
    def _get_observations(self) -> dict:
        ...
        # TEST 8: Goal modification pipeline (matching v2's _build_low_level_obs)
        use_modifier = (self._phase == self.NAVIGATE)

        # Step 1: Yaw rotation of goal XY (NAVIGATE only)
        ...
        # Step 2: Altitude offset (NAVIGATE only)
        modified_goal = torch.stack([
            rotated_x,
            rotated_y,
            goal_pos_b[:, 2] + self._hl_alt_offset,
        ], dim=1)

        # Step 3: Conditional selection
        goal_for_ll = torch.where(
            use_modifier.unsqueeze(1),
            modified_goal,
            goal_pos_b,
        )

        # Step 4: XY magnitude clamping (ALL phases)
        # FIX: 3.0 → 8.0 — LL trained with 6-8m distances, clamping broke speed regulation
        MAX_LL_GOAL_XY = 8.0
        goal_xy_now = goal_for_ll[:, :2]
        xy_dist = goal_xy_now.norm(dim=-1, keepdim=True).clamp(min=1e-6)
        ...
        # Step 5: Speed factor scaling on XY (NAVIGATE only)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # TEST 9: Reset goal modifier buffers and step counter
...
def play():
    ...
    # Test 5 fix: extract actor (same as obstacle env) but replicate predict() scaling
    actor = agent.policy.actor
    actor.eval()
    for param in actor.parameters():
        param.requires_grad = False
    print(f"[INFO] Using extracted actor with x100 scaling fix (Test 5 fix)", flush=True)
    ...
    try:
        while simulation_app.is_running():
            ...
            # Actor extraction + scaling fix: actor outputs [-1,1] (tanh),
            # multiply by 100 (action_space scale) then clamp to [-1,1]
            # to replicate what predict() + _pre_physics_step did during training
            obs_tensor = torch.as_tensor(obs, dtype=torch.float32, device="cuda:0")
            with torch.no_grad():
                actions = actor(obs_tensor, deterministic=True)
                actions = (actions * 100.0).clamp(-1.0, 1.0)
