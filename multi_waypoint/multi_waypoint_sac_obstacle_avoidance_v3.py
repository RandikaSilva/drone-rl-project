"""Hierarchical Crazyflie obstacle avoidance + multi-waypoint navigation with SAC — v3.

Built on v2. Fixes roll-crash in narrow corridors (both-side lidar red → yaw oscillation → roll death).

v3 fixes (vs v2 which rolled when both sides detected obstacles):
    1. Anti-oscillation: max yaw rate of change = 0.1 rad per HL update (prevents whiplash)
    2. Corridor detection: when both left+right lidar < danger dist, suppress yaw → go straight
    3. Corridor escape: auto altitude boost (+0.3m/s vz) when trapped in tight corridor
    4. Gentler emergency brake: min factor 0.4 (v2 had 0.2 → too aggressive → altitude loss)
    5. Adaptive EMA: alpha drops to 0.2 when in danger zone (smoother in tight spaces)
    6. Roll penalty doubled in NAVIGATE: discourage aggressive banking near obstacles
    7. Reduced yaw range: ±0.5 -> ±0.35 rad (20°) — enough to steer, less likely to destabilize
...
class HierarchicalObstacleNavEnvCfg(DirectRLEnvCfg):
    ...
    # ── LiDAR Noise (v3-fix: robustness to sensor imperfections) ────────────
    hl_lidar_noise_std: float = 0.02   # Gaussian noise σ on normalized [0,1] readings
    ll_lidar_noise_std: float = 0.01   # smaller noise for LL (fewer rays, more critical)

    # ── Goal Modifier (v3: balanced ranges + anti-oscillation) ──────────────
    max_yaw_offset: float = 0.35       # radians (~20°) — v2 had 0.5 (too aggressive → roll)
    speed_factor_range: tuple = (0.3, 1.0)   # can slow down near obstacles
    max_altitude_offset: float = 0.5   # meters — can fly over lower canopy
    goal_modifier_smoothing: float = 0.5  # EMA alpha (base, reduced in danger)
    goal_modifier_danger_smoothing: float = 0.2  # v3: slower EMA when in danger (anti-oscillation)
    hl_update_interval: int = 3        # env.steps between HL updates (16.7 Hz)
    max_yaw_rate_per_update: float = 0.1  # v3: max yaw change per HL update (anti-whiplash)
    corridor_escape_vz: float = 0.3    # v3: upward velocity when corridor trapped (m/s)

    # ── LL Dynamic Speed Cap (v3-fix: prevent high-speed crashes near obstacles) ─
    # Speed is progressively reduced near obstacles — policy learns to work with this
    obstacle_speed_cap_min: float = 0.3  # minimum speed ratio near obstacles (30% of max)
    ...
    emergency_brake_distance: float = 3.5  # v3-fix: start braking earlier (was 2.5)
    emergency_brake_min_factor: float = 0.15  # v3-fix: harder braking at close range (was 0.4)
    corridor_danger_distance: float = 4.0  # v3: both-side detection threshold
    # ── PID Dynamic Tilt (v3-fix: reduce max tilt near obstacles to prevent flipping) ─
    obstacle_min_tilt: float = 0.25  # minimum max tilt near obstacles (~14°)
    ...
    ang_vel_reward_scale: float = -1.5   # v3: stronger (v2: -1.0) — penalize roll harder
    yaw_rate_penalty_scale: float = -2.0  # v3: stronger (v2: -1.5) — discourage spinning
    upright_reward_scale: float = 6.0    # v3: stronger (v2: 4.0) — reward staying level
...
class HierarchicalObstacleNavEnv(DirectRLEnv):
    ...
    def _detect_corridor(self) -> torch.Tensor:
        """Detect if drone is in a narrow corridor (obstacles on both left AND right).

        Splits horizontal lidar rays into left half (90°-270°) and right half (270°-90°).
        If min distance on BOTH sides < corridor_danger_distance → corridor detected.

        Returns: (num_envs,) bool tensor
        """
        if self._hl_lidar_ray_dirs is None or not hasattr(self, '_last_hl_lidar_distances'):
        ...
        # Ray directions in local frame: positive Y = left, negative Y = right
        ray_dirs = self._hl_lidar_ray_dirs[horiz_idx]
        left_mask = ray_dirs[:, 1] > 0.1   # rays pointing left
        right_mask = ray_dirs[:, 1] < -0.1  # rays pointing right

        horiz_dists = self._last_hl_lidar_distances[:, horiz_idx] if hasattr(self, '_last_hl_lidar_distances') else \
            torch.full((self.num_envs, len(horiz_idx)), self.cfg.hl_lidar_max_distance, device=self.device)

        threshold = self.cfg.corridor_danger_distance

        # Min distance on left side and right side
        if left_mask.any():
            left_min = horiz_dists[:, left_mask].min(dim=1).values
        else:
            left_min = torch.full((self.num_envs,), self.cfg.hl_lidar_max_distance, device=self.device)

        if right_mask.any():
            right_min = horiz_dists[:, right_mask].min(dim=1).values
        else:
            right_min = torch.full((self.num_envs,), self.cfg.hl_lidar_max_distance, device=self.device)

        # Both sides close = corridor
        return (left_min < threshold) & (right_min < threshold)
    ...
    def _build_low_level_obs(self, use_modifier_mask: torch.Tensor) -> torch.Tensor:
        ...
        # v3 fix: Clamp LATERAL displacement from yaw rotation
        # Without this, 0.35 rad on a 5m goal = 1.7m lateral jump → roll crash
        MAX_LATERAL_SHIFT = 0.8  # max meters the goal can shift sideways
        lateral_shift = rotated_y - goal_xy[:, 1]
        lateral_shift = lateral_shift.clamp(-MAX_LATERAL_SHIFT, MAX_LATERAL_SHIFT)
        rotated_y = goal_xy[:, 1] + lateral_shift
        ...
        # v3-fix: Add Gaussian noise to LL LiDAR for robustness
        if self.cfg.ll_lidar_noise_std > 0.0:
            ll_lidar_norm = ll_lidar_norm + torch.randn_like(ll_lidar_norm) * self.cfg.ll_lidar_noise_std
            ll_lidar_norm = ll_lidar_norm.clamp(0.0, 1.0)
    ...
    def _run_low_level_policy(self):
        ...
        v3-fix: Progressive speed cap based on obstacle proximity.
        The ×100 scaling saturates all LL outputs to ±1.0 (max velocity always).
        Instead of fighting the policy direction, we cap the MAGNITUDE of velocity
        proportional to obstacle distance, and reduce max tilt to prevent flipping.
        """
        is_navigate = (self._phase == self.NAVIGATE)
        ...
        # v3-fix: Compute dynamic max velocity based on obstacle proximity
        # This scales DOWN the max speed as obstacles get closer
        # The LL policy direction is preserved — only the magnitude is capped
        dynamic_max_xy = torch.full_like(self._ll_vx_cmd, self.cfg.max_velocity_xy)
        dynamic_max_yaw = torch.full_like(self._ll_vx_cmd, self.cfg.max_yaw_rate)

        if is_navigate.any():
            lidar_dist = self._min_horiz_lidar_dist
            # Progressive speed reduction: full speed at 5m+, minimum at 1m
            speed_ratio = ((lidar_dist - 1.0) / (self.cfg.lidar_danger_distance - 1.0)).clamp(0.0, 1.0)
            # Smooth curve: sqrt gives more speed at medium distance
            speed_ratio = speed_ratio.sqrt()
            # Min speed ratio from config (enough to maneuver, not enough to crash)
            min_speed_ratio = self.cfg.obstacle_speed_cap_min
            nav_speed_ratio = min_speed_ratio + (1.0 - min_speed_ratio) * speed_ratio

            dynamic_max_xy = torch.where(is_navigate,
                nav_speed_ratio * self.cfg.max_velocity_xy, dynamic_max_xy)
            dynamic_max_yaw = torch.where(is_navigate,
                nav_speed_ratio * self.cfg.max_yaw_rate, dynamic_max_yaw)

        # Apply velocity commands with dynamic limits
        self._ll_vx_cmd = (ll_actions[:, 0] * dynamic_max_xy)
        self._ll_vy_cmd = (ll_actions[:, 1] * dynamic_max_xy)
        self._ll_vz_cmd = (ll_actions[:, 2] * self.cfg.max_velocity_z)
        self._ll_yaw_rate_cmd = (ll_actions[:, 3] * dynamic_max_yaw)

        # v3: Corridor escape — push upward when trapped between obstacles
        in_corridor = self._detect_corridor() & is_navigate
        if in_corridor.any():
            self._ll_vz_cmd[in_corridor] = torch.clamp(
                self._ll_vz_cmd[in_corridor] + self.cfg.corridor_escape_vz,
                max=self.cfg.max_velocity_z,
            )

    def _run_pid(self):
        """PID controller: velocity commands -> thrust/torques.
        v3-fix: Reduce max tilt near obstacles to prevent flipping.
        """
        ...
        # v3-fix: Dynamic max tilt — reduce near obstacles to prevent flipping
        is_navigate = (self._phase == self.NAVIGATE)
        lidar_dist = self._min_horiz_lidar_dist
        # At 5m+: full tilt (0.5 rad). At 1m: reduced tilt
        tilt_ratio = ((lidar_dist - 1.0) / (self.cfg.lidar_danger_distance - 1.0)).clamp(0.0, 1.0)
        dynamic_max_tilt = torch.where(
            is_navigate,
            self.cfg.obstacle_min_tilt + (self.cfg.pid_max_tilt - self.cfg.obstacle_min_tilt) * tilt_ratio,
            torch.full_like(lidar_dist, self.cfg.pid_max_tilt),
        )

        desired_roll = (self.cfg.pid_vel_kp * vy_err).clamp(-dynamic_max_tilt, dynamic_max_tilt)
        desired_pitch = (-self.cfg.pid_vel_kp * vx_err).clamp(-dynamic_max_tilt, dynamic_max_tilt)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        HL goal modifier updates every 3 steps (16.7 Hz).
        v3: anti-oscillation, corridor detection, adaptive smoothing.
        """
        ...
        if update_mask.any():
            ...
            # v3: Corridor detection — check if obstacles on BOTH sides
            # Split lidar into left half and right half (horizontal rays only)
            in_corridor = self._detect_corridor()

            # v3: In corridor → suppress yaw (go straight), force slow speed
            target_yaw = torch.where(in_corridor, target_yaw * 0.2, target_yaw)
            target_spd = torch.where(in_corridor, torch.clamp(target_spd, max=0.5), target_spd)
            # In corridor → push altitude up to escape over obstacles
            target_alt = torch.where(
                in_corridor,
                torch.clamp(target_alt, min=0.2),  # bias upward
                target_alt,
            )

            # v3: Adaptive EMA — slower smoothing when in danger zone
            is_danger = self._min_horiz_lidar_dist < self.cfg.lidar_danger_distance
            alpha = torch.where(
                is_danger,
                torch.full_like(self._min_horiz_lidar_dist, self.cfg.goal_modifier_danger_smoothing),
                torch.full_like(self._min_horiz_lidar_dist, self.cfg.goal_modifier_smoothing),
            )

            new_yaw = ((1 - alpha) * self._hl_yaw_offset + alpha * target_yaw)
            new_spd = ((1 - alpha) * self._hl_speed_factor + alpha * target_spd)
            new_alt = ((1 - alpha) * self._hl_alt_offset + alpha * target_alt)

            # v3: Anti-oscillation — clamp yaw rate of change per update
            max_dyaw = self.cfg.max_yaw_rate_per_update
            yaw_delta = (new_yaw - self._hl_yaw_offset).clamp(-max_dyaw, max_dyaw)
            new_yaw = self._hl_yaw_offset + yaw_delta

            self._hl_yaw_offset[update_mask] = new_yaw[update_mask]
            self._hl_speed_factor[update_mask] = new_spd[update_mask]
            self._hl_alt_offset[update_mask] = new_alt[update_mask]
    ...
    def _get_observations(self) -> dict:
        ...
        # v3-fix: Add Gaussian noise to LiDAR for robustness
        if self.cfg.hl_lidar_noise_std > 0.0:
            hl_lidar_norm = hl_lidar_norm + torch.randn_like(hl_lidar_norm) * self.cfg.hl_lidar_noise_std
            hl_lidar_norm = hl_lidar_norm.clamp(0.0, 1.0)
...
gym.register(
    id="Isaac-HierarchicalObstacle-SAC-v3",
...
def train():
    ...
    log_root = os.path.join(SCRIPT_DIR, "logs", "hierarchical_sac_v3", "crazyflie_obstacle_avoidance_sac_v3")
    ...
    env = gym.make("Isaac-HierarchicalObstacle-SAC-v3", cfg=env_cfg)
