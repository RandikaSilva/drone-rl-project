...
def play():
    ...
    # Test 5: Extract actor directly (same as obstacle env does)
    actor = agent.policy.actor
    actor.eval()
    for param in actor.parameters():
        param.requires_grad = False
    print(f"[INFO] Using extracted actor directly (Test 5)", flush=True)
    ...
    try:
        while simulation_app.is_running():
            ...
            with torch.no_grad():
                actions = actor(obs_tensor, deterministic=True).clamp(-1.0, 1.0)
