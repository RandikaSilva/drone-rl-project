...
class MultiWaypointNavEnvCfg(DirectRLEnvCfg):
    ...
    ema_smoothing_alpha: float = 0.6  # EMA on lateral velocity commands (matching obstacle env)
...
class MultiWaypointNavEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MultiWaypointNavEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # EMA state for lateral velocity smoothing
        self._ema_vx = torch.zeros(self.num_envs, device=self.device)
        self._ema_vy = torch.zeros(self.num_envs, device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        raw_vx = self._actions[:, 0] * self.cfg.max_velocity_xy
        raw_vy = self._actions[:, 1] * self.cfg.max_velocity_xy
        vz_cmd = self._actions[:, 2] * self.cfg.max_velocity_z
        yaw_rate_cmd = self._actions[:, 3] * self.cfg.max_yaw_rate

        # ── EMA smoothing on lateral velocity (Test 3) ──
        alpha = self.cfg.ema_smoothing_alpha
        self._ema_vx = alpha * raw_vx + (1 - alpha) * self._ema_vx
        self._ema_vy = alpha * raw_vy + (1 - alpha) * self._ema_vy
        vx_cmd = self._ema_vx
        vy_cmd = self._ema_vy
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._ema_vx[env_ids] = 0.0
        self._ema_vy[env_ids] = 0.0
